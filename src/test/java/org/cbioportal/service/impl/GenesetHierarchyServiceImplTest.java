...
public class GenesetHierarchyServiceImplTest extends BaseServiceImplTest {
  ...
  @Mock private MolecularDataService geneticDataService;
  ...
  @Mock private GenesetHierarchyRepository genesetHierarchyRepository;
  ...
  public void setUp() throws Exception {
    ...
    Mockito.when(geneticProfileService.getMolecularProfile(MOLECULAR_PROFILE_ID))
    ...
    Mockito.when(
            genesetDataService.fetchGenesetData(
                MOLECULAR_PROFILE_ID, Arrays.asList(SAMPLE_ID1, SAMPLE_ID2, SAMPLE_ID3), null))
    ...
    Mockito.when(
            genesetDataService.fetchGenesetData(
                PVALUE_GENETIC_PROFILE_ID, Arrays.asList(SAMPLE_ID1, SAMPLE_ID2, SAMPLE_ID3), null))
  ...
  private GenesetMolecularData getSimpleFlatGenesetDataItem(
      String sampleStableId, String genesetId, String value) {
  ...
  public void fetchCorrelatedGenes() throws Exception {
    ...
    List<GenesetHierarchyInfo> result =
        genesetHierarchyService.fetchGenesetHierarchyInfo(
            MOLECULAR_PROFILE_ID, 50, 0.4, 0.05, Arrays.asList(SAMPLE_ID1, SAMPLE_ID2, SAMPLE_ID3));

    // what we expect: at threshold 0.4 only GENESET_ID1 will qualify, so only the hierarchy related
    // to this
    ...
    result =
        genesetHierarchyService.fetchGenesetHierarchyInfo(
            MOLECULAR_PROFILE_ID, 90, 0.3, 0.05, Arrays.asList(SAMPLE_ID1, SAMPLE_ID2, SAMPLE_ID3));

    // what we expect: at threshold 0.3 both GENESET_ID1 & 2 will qualify, so the hierarchy related
    // to these
    // genesets should return. That is:
    //   Root node ->  sub node A -> parent node 1 -> GENESET_ID1, with representative score=0.499
    // and p-value=0.0359
    //                                             -> GENESET_ID2, with representative score=-0.35
    // and p-value=0.046
    ...
    result =
        genesetHierarchyService.fetchGenesetHierarchyInfo(
            MOLECULAR_PROFILE_ID, 40, 0.1, 0.01, Arrays.asList(SAMPLE_ID1, SAMPLE_ID2, SAMPLE_ID3));

    // what we expect: at threshold 0.1 both GENESET_ID1 & 2 will initially qualify, but GENESET_ID1
    // will finally not
    // make it because of its p-values. So result should be:
    //   Root node ->  sub node A -> parent node 1 -> GENESET_ID2, with representative score=0.12
    // and p-value=0.0019

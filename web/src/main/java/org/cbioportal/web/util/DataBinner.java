...
public class DataBinner {
    ...
    public List<DataBin> calculateClinicalDataBins(
        ClinicalDataBinFilter clinicalDataBinFilter,
    ...
    public List<DataBin> recalcBinCount(
        ...
        List<Range<BigDecimal>> ranges = clinicalData == null
            ? Collections.emptyList()
            : filterSpecialRanges(clinicalData);
        ...
        for (DataBin dataBin : clinicalDataBins) {
            ...
            if ("NA".equalsIgnoreCase(dataBin.getSpecialValue())) {
                dataBin.setCount(
                    countNAs(clinicalData, clinicalDataType, ids).intValue()
                );
            }
        }
    ...
    public List<DataBin> calculateClinicalDataBins(
        ...
        Range<BigDecimal> range = clinicalDataBinFilter.getStart() == null &&
            clinicalDataBinFilter.getEnd() == null
            ? Range.all()
            : studyViewFilterUtil.calcRange(
                clinicalDataBinFilter.getStart(),
                true,
                clinicalDataBinFilter.getEnd(),
                true
            );

        if (range.hasUpperBound()) {
            clinicalData =
                filterSmallerThanUpperBound(
                    clinicalData,
                    range.upperEndpoint()
                );
            numericalOnly = true;
        ...
        if (range.hasLowerBound()) {
            clinicalData =
                filterBiggerThanLowerBound(clinicalData, range.lowerEndpoint());
            numericalOnly = true;
        }

        DataBin upperOutlierBin = calcUpperOutlierBin(
            attributeId,
            clinicalData
        );
        DataBin lowerOutlierBin = calcLowerOutlierBin(
            attributeId,
            clinicalData
        );
        Collection<DataBin> numericalBins = calcNumericalClinicalDataBins(
            attributeId,
            clinicalData,
            clinicalDataBinFilter.getCustomBins(),
            lowerOutlierBin,
            upperOutlierBin,
        ...
        if (!numericalOnly) {
            ...
            dataBins.addAll(
                calcNonNumericalClinicalDataBins(attributeId, clinicalData)
            );

            DataBin naDataBin = calcNaDataBin(
                attributeId,
                clinicalData,
                clinicalDataType,
                ids
    ...
    public List<Range<BigDecimal>> filterSpecialRanges(
        List<ClinicalData> clinicalData
    ) {
        return clinicalData
            .stream()
    ...
    public Collection<DataBin> calcNonNumericalClinicalDataBins(
        String attributeId,
        List<ClinicalData> clinicalData
    ) {
        return calcNonNumericalDataBins(
            attributeId,
            filterNonNumericalValues(clinicalData)
        );
    }

    public List<String> filterNonNumericalValues(
        List<ClinicalData> clinicalData
    ) {
        ...
        return clinicalData
            ...
            .filter(
                s ->
                    !NumberUtils.isCreatable(dataBinHelper.stripOperator(s)) &&
                    !dataBinHelper.isNA(s)
            )
            .collect(Collectors.toList());
    }

    public Collection<DataBin> calcNonNumericalDataBins(
        String attributeId,
        ...
        for (String value : nonNumericalValues) {
            DataBin dataBin = map.computeIfAbsent(
    ...
    public Collection<DataBin> calcNumericalClinicalDataBins(
        String attributeId,
        List<ClinicalData> clinicalData,
    ...
    public List<BigDecimal> filterNumericalValues(
        List<ClinicalData> clinicalData
    ) {
    ...
    public Collection<DataBin> calcNumericalDataBins(
        String attributeId,
        ...
        Range<BigDecimal> boxRange = dataBinHelper.calcBoxRange(
            sortedNumericalValues
        );

        // remove initial outliers
        List<BigDecimal> withoutOutliers = sortedNumericalValues
            .stream()
            .filter(isNotOutlier)
            .collect(Collectors.toList());
        ...
        if (0 < uniqueValues.size() && uniqueValues.size() <= 5) {
            ...
            dataBins =
                discreteDataBinner.calculateDataBins(
                    attributeId,
                    withoutOutliers,
                    uniqueValues
                );
            ...
            if (lowerOutlierBin.getEnd() == null) {
                BigDecimal end = dataBins != null && dataBins.size() > 0
                    ? dataBins.get(0).getStart()
                    : boxRange.lowerEndpoint();
                ...
                if (
                    dataBins
                        .get(0)
                        .getStart()
                        .compareTo(lowerOutlierBin.getEnd()) ==
                    1
                ) {
            ...
            if (upperOutlierBin.getStart() == null) {
                BigDecimal start = dataBins != null && dataBins.size() > 0
                    ? dataBins.get(dataBins.size() - 1).getEnd()
                    : boxRange.upperEndpoint();
                ...
                if (
                    dataBins
                        .get(dataBins.size() - 1)
                        .getEnd()
                        .compareTo(upperOutlierBin.getStart()) ==
                    -1
                ) {
                    upperOutlierBin.setStart(
                        dataBins.get(dataBins.size() - 1).getStart()
                    );
                } else {
                    dataBins
                        .get(dataBins.size() - 1)
                        .setEnd(upperOutlierBin.getStart());
                }
            }
        }

        // update upper and lower outlier counts
        List<BigDecimal> upperOutliers = sortedNumericalValues
            .stream()
            .filter(isUpperOutlier)
            .collect(Collectors.toList());
        List<BigDecimal> lowerOutliers = sortedNumericalValues
            .stream()
            .filter(isLowerOutlier)
            .collect(Collectors.toList());

        if (upperOutliers.size() > 0) {
            upperOutlierBin.setCount(
                upperOutlierBin.getCount() + upperOutliers.size()
            );
        }

        if (lowerOutliers.size() > 0) {
            lowerOutlierBin.setCount(
                lowerOutlierBin.getCount() + lowerOutliers.size()
            );
        }
    ...
    public List<ClinicalData> filterSmallerThanUpperBound(
        List<ClinicalData> clinicalData,
        BigDecimal value
    ) {
        return (
    ...
    public List<ClinicalData> filterBiggerThanLowerBound(
        List<ClinicalData> clinicalData,
        BigDecimal value
    ) {
        return (
        ...
        if (clinicalData != null) {
            uniqueClinicalDataIds =
                clinicalDataType == ClinicalDataType.PATIENT
                    ? clinicalData
                        .stream()
                        .map(ClinicalData::getPatientId)
                        .filter(Objects::nonNull)
                        .collect(Collectors.toSet())
                    : clinicalData
                        .stream()
                        .map(ClinicalData::getSampleId)
                        .filter(Objects::nonNull)
                        .collect(Collectors.toSet());
        } else {

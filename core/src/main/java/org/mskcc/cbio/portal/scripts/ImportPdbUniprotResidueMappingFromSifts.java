...
public final class ImportPdbUniprotResidueMappingFromSifts {
     ...
     */
    public static void importSiftsData(
        File mappingFile,
        Set<String> humanChains,
        String pdbCacheDir,
        double identp_threhold
    )
    ...
    private static boolean processPdbUniprotAlignment(
        PdbUniprotAlignment pdbUniprotAlignment,
        List<PdbUniprotResidueMapping> pdbUniprotResidueMappings,
        int alignId,
        String pdbId,
        String chainId,
        String uniprotAcc,
        ...
        throws DaoException {
        String uniprotSeq = getUniprotSequence(
            uniprotAcc,
            uniprotResBeg,
            uniprotResEnd
        );
        ...
        List<Group> pdbResidues = getPdbResidues(
            atomCache,
            pdbId,
            chainId,
            pdbSeqResBeg,
        ...
        String uniprotId = DaoUniProtIdMapping.mapFromUniprotAccessionToUniprotId(
            uniprotAcc
        );
        if (uniprotId == null) {
        ...
        pdbUniprotAlignment.setPdbFrom(
            Integer.toString(startRes.getSeqNum()) +
            (startRes.getInsCode() == null ? "" : startRes.getInsCode())
        );
        pdbUniprotAlignment.setPdbTo(
            Integer.toString(endRes.getSeqNum()) +
    ...
    private static List<Group> getPdbResidues(
        AtomCache atomCache,
        String pdbId,
        String chainId,
        int start,
    ...
    private static String getUniprotSequence(
        String uniportAcc,
        int start,
        int end
    ) {
        try {
            UniprotProxySequenceReader<AminoAcidCompound> uniprotSequence = new UniprotProxySequenceReader<AminoAcidCompound>(
                uniportAcc,
    ...
    public static void main(String[] args) throws Exception {
        if (args.length < 2) {
            System.out.println(
                "command line usage:  importPdbUniprotResidueMapping.pl <pdb_chain_uniprot.tsv> <pdb_chain_human.tsv> <pdb-cache-dir>"
            );
            return;
        }

        String pdbCacheDir = args.length > 2
            ? args[2]

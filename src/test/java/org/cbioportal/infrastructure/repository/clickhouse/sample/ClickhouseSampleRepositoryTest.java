...
public class ClickhouseSampleRepositoryTest {
    ...
    public void getAllSamplesInStudy() {
        var sortedByStableIdDesc = getAllTcgaPubSamplesPaginatedAndSorted(
            "stableId",
            "DESC"
        );
        ...
        var sortedByStableIdAsc = getAllTcgaPubSamplesPaginatedAndSorted(
            "stableId",
            "ASC"
        );
    ...
    private List<List<Sample>> getAllTcgaPubSamplesPaginatedAndSorted(
        ...
        pages.add(repository().getAllSamplesInStudy(
            STUDY_TCGA_PUB,
    ...
    // this is a workaround. cannot autowire the repository for some reason.
    private ClickhouseSampleRepository repository() {
        if (repository == null) {
            if (mapper != null) {
                repository = new ClickhouseSampleRepository(mapper);
            }
        }

...
public class ProxyController {
    ...
    public ResponseEntity<String> proxyOncokb(
        @RequestBody(required = false) String body,
        HttpMethod method,
        HttpServletRequest request
    )
        ...
        if (!this.showOncokb) {
            return new ResponseEntity<>(
                "OncoKB service is disabled.",
                HttpStatus.NOT_FOUND
            );
        } else if (StringUtils.isEmpty(oncokbToken)) {
            return new ResponseEntity<>(
                "No OncoKB access token is provided.",
                HttpStatus.NOT_FOUND
            );
        }
    ...
    private URI buildUri(
        ...
        throws URISyntaxException {
        return buildUri(
            (useSecureProtocol ? "https" : "http") + ":/" + path,
            queryString
        );
    }
    ...
    private ResponseEntity<String> exchangeData(
        String body,
        URI uri,
        HttpMethod method,

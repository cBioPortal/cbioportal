...
public class ExportDataForDownload {
    ...
    public static void main(String[] args) {
        ...
        OptionSpec<String> htmlIndexFile = parser
            .accepts(
                "htmlIndexFile",
                "output file to store html " + "index of files produced"
            )
            .withRequiredArg()
            .describedAs("htmlIndexFile")
            .ofType(String.class);
        OptionSpec<String> directory = parser
            .accepts("directory", "directory to hold download files produced")
            .withRequiredArg()
            .describedAs("directory")
            .ofType(String.class);
        OptionSpec<String> baseURL = parser
            .accepts("baseURL", "base for html links")
            .withRequiredArg()
            .describedAs("baseURL")
            .ofType(String.class);
        ...
        if (
            !options.has(baseURL) ||
            !options.has(directory) ||
            !options.has(htmlIndexFile)
        ) {
        ...
        // TODO: *.seg files
        generateExportProfileData(
            options.valueOf(htmlIndexFile),
     ...
     *
     */
    public static void generateExportProfileData(
        String htmlIndexFile,
        String directory,
        String baseURL
    ) {
        /*
        ...
        try {
            ...
            for (CanonicalGene aCanonicalGene : allGenes) {
                geneSymbolMap.put(
                    aCanonicalGene.getEntrezGeneId(),
                    aCanonicalGene.getHugoGeneSymbolAllCaps()
                );
            }
            ...
            for (CancerStudy aCancerStudy : theCancerStudies) {
                ...
                htmlIndexFilePrintWriter.print(
                    "<LI>" + aCancerStudy.getName() + "</LI>\n"
                );

                ArrayList<GeneticProfile> theGeneticProfiles = DaoGeneticProfile.getAllGeneticProfiles(
                    aCancerStudy.getInternalId()
                );
                htmlIndexFilePrintWriter.print("<UL>");
                ...
                for (GeneticProfile aGeneticProfile : theGeneticProfiles) {
                    ...
                    monitorOutput.append(
                        "\tProcessing " + aGeneticProfile.getProfileName()
                    );

                    ArrayList<Integer> internalSampleIds = DaoSampleProfile.getAllSampleIdsInProfile(
                        aGeneticProfile.getGeneticProfileId()
                    );
                    ...
                    monitorOutput.append(
                        ", with " + internalSampleIds.size() + " cases"
                    );
                    ...
                    monitorOutput.append(
                        " and " + theGenes.size() + " genes.\n"
                    );

                    long start = System.currentTimeMillis();

                    String filename =
                        aGeneticProfile.getStableId() + TABBED_FILE_SUFFIX;
                    ...
                    out.println(
                        ...
                        ", cgds dbms source, documentation of " +
                        "data encoding, etc."
                    );
                    ...
                    for (Long aGeneID : theGenes) {
                        HashMap<Integer, String> theGeneticAlterationMap = aDaoGeneticAlteration.getGeneticAlterationMap(
                            aGeneticProfile.getGeneticProfileId(),
                            aGeneID.longValue()
                        );
                        ...
                        // output data
                        for (Integer sampleId : internalSampleIds) {
                            String value = theGeneticAlterationMap.get(
                                sampleId
                            );
                            out.print("\t");
                            if (null == value) {

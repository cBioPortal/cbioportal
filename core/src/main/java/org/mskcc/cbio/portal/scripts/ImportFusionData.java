...
public class ImportFusionData {
    ...
    public void importData() throws IOException, DaoException {
        ...
        if (genePanelID != null) {
            ...
            List<GenePanel> genePanels = genePanelRepositoryLegacy.getGenePanelByStableId(
                genePanelID
            );
        ...
        while ((line = buf.readLine()) != null) {
            ...
            if (!line.startsWith("#") && line.trim().length() > 0) {
                ...
                if (sample == null) {
                    if (StableIdUtil.isNormal(barCode)) {
                        ...
                        throw new RuntimeException(
                            ...
                            this.fusionFile.getCanonicalPath()
                        );
                    }
                }
                ...
                if (
                    genePanel != null &&
                    !GeneticProfileUtil.geneInPanel(gene, genePanel)
                ) {
                    ProgressMonitor.logWarning(
                        "Gene is not profiled in gene panel:  " +
                        geneSymbol +
                        " [" +
                        entrezGeneId +
                        "] " +
                        genePanel.getStableId() +
                        ". Ignoring it " +
                        "and all fusion data associated with it!"
                    );
                } else {
                    ...
                    mutation.setMutationStatus(
                        record.getFusionStatus().toUpperCase()
                    );
                    ...
                    if (
                        ...
                        Integer panelId = (genePanel == null)
                            ? null

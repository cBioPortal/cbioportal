...
public class AlterationDriverAnnotationServiceImplTest {
  ...
  @Mock private AlterationDriverAnnotationRepository alterationDriverAnnotationRepository;
  ...
  public void getCustomDriverAnnotationProps() {
    ...
    List<AlterationDriverAnnotation> annotationList =
        Arrays.asList(alterationDriverAnnotation1, alterationDriverAnnotation2);
    when(alterationDriverAnnotationRepository.getAlterationDriverAnnotations(isNull()))
  ...
  public void getCustomDriverAnnotationPropsWithoutNATiers() {
    ...
    List<AlterationDriverAnnotation> annotationList =
        Arrays.asList(alterationDriverAnnotation1, naTierAnnotation);
    ...
    Assert.assertFalse(
        "NA has to be removed from the set of tiers", props.getTiers().contains("NA"));
  }
  ...
  public void getCustomDriverAnnotationPropsWithoutEmptyTiers() {
    ...
    List<String> molecularProfileIds = Arrays.asList("test_1", "test_2");
    List<AlterationDriverAnnotation> annotationList =
        Arrays.asList(alterationDriverAnnotation1, emptyTierAnnotation);
    when(alterationDriverAnnotationRepository.getAlterationDriverAnnotations(molecularProfileIds))
        .thenReturn(annotationList);
    ...
    Assert.assertFalse(
        "Empty string tier has to be removed from the set", props.getTiers().contains(""));
  }
}

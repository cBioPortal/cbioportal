...
public class ImportGeneData extends ConsoleRunnable {
     ...
     * @throws DaoException
     */
    public static void importData(File geneFile, String genomeBuild)
        ...
        try (FileReader reader = new FileReader(geneFile)) {
            ...
            while ((line = buf.readLine()) != null) {
                ...
                int referenceGenomeId = DaoReferenceGenome
                    .getReferenceGenomeByBuildName(genomeBuild)
                ...
                if (
                    geneSymbol.startsWith("MIR") &&
                    type.equalsIgnoreCase("miscRNA")
                ) {
                    line = buf.readLine();
                ...
                if (!mainSymbol.equals("-")) {
                    ...
                    Set<CanonicalGene> genes = genesWithSymbolFromNomenClatureAuthority.get(
                        mainSymbol
                    );
                    Set<ReferenceGenomeGene> refGenes = refGenesWithSymbolFromNomenClatureAuthority.get(
                        mainSymbol
                    );

                    if (genes == null) {
                        genes = new HashSet<CanonicalGene>();
                        genesWithSymbolFromNomenClatureAuthority.put(
                            mainSymbol,
                            genes
                        );
                    }
                    if (refGenes == null) {
                        ...
                        refGenesWithSymbolFromNomenClatureAuthority.put(
                            mainSymbol,
                            refGenes
                        );
                    }
                    ...
                    Set<CanonicalGene> genes = genesWithoutSymbolFromNomenClatureAuthority.get(
                        geneSymbol
                    );
                    if (genes == null) {
                        genes = new HashSet<CanonicalGene>();
                        genesWithoutSymbolFromNomenClatureAuthority.put(
                            geneSymbol,
                            genes
                        );
                    }
                    genes.add(gene);
                    Set<ReferenceGenomeGene> refGenes = refGenesWithoutSymbolFromNomenClatureAuthority.get(
                        geneSymbol
                    );
                    if (refGenes == null) {
                        refGenes = new HashSet<ReferenceGenomeGene>();
                        refGenesWithoutSymbolFromNomenClatureAuthority.put(
                            geneSymbol,
                            refGenes
                        );
                    }
            ...
            addGenesToDB(
                genesWithSymbolFromNomenClatureAuthority,
                genesWithoutSymbolFromNomenClatureAuthority
            );
            addReferenceGenomeGenesToDB(
                refGenesWithSymbolFromNomenClatureAuthority,
     ...
     * @throws DaoException
     */
    private static void addGenesToDB(
        Map<String, Set<CanonicalGene>> genesWithSymbolFromNomenClatureAuthority,
        Map<String, Set<CanonicalGene>> genesWithoutSymbolFromNomenClatureAuthority
    )
        throws DaoException {
            ...
            if (genes.size() == 1) {
                ...
                if (daoGene.getGene(gene.getEntrezGeneId()) != null) {
                    ProgressMonitor.logWarning(
                        "Gene is already in table. Updates are not allowed. If you need to update the `gene` table, please remove all studies first and empty the `gene` table."
                    );
                    nrExisting++;
                    continue;
                }
                ...
                ProgressMonitor.logWarning(
                    "New gene with official symbol added"
                );
            } else {
        ...
        if (genesWithoutSymbolFromNomenClatureAuthority.keySet().size() > 0) {
            ...
            for (Map.Entry<String, Set<CanonicalGene>> entry : genesWithoutSymbolFromNomenClatureAuthority.entrySet()) {
                ...
                if (genes.size() == 1) {
                    ...
                    if (
                        !genesWithSymbolFromNomenClatureAuthority.containsKey(
                            symbol
                        )
                    ) {
                        ...
                        daoGene.addGene(gene);
                        ProgressMonitor.logWarning(
                            "New gene with *no* official symbol added"
                        );
                        nrImported++;
                    } else {
                        // ignore entries with a symbol that have the same value as stardard one
                        ProgressMonitor.logWarning(
                            "Ignored line with entrez gene id " +
                            gene.getEntrezGeneId() +
                            " because its 'unnoficial' symbol " +
                            symbol +
                            " is already an 'official symbol' of another gene"
                        );
                        nrSkipped++;
                        continue;
                    }
            ...
            ProgressMonitor.logWarning(
                "There were " +
                genesWithoutSymbolFromNomenClatureAuthority.keySet().size() +
                " genes names in this file without an official symbol from nomenclature authority. Imported: " +
                nrImported +
                ". Gene names skipped (because of duplicate symbol entry or because symbol is an 'official symbol' of another gene): " +
                nrSkipped
            );
        }
        ...
        if (nrExisting > 0) {
            ProgressMonitor.logWarning(
                "Number of records skipped because the gene was already in the gene table (updates are not allowed - see specific warnings above): " +
                nrExisting
            );
        }
    }
     ...
     * @throws DaoException
     */
    private static void addReferenceGenomeGenesToDB(
        Map<String, Set<ReferenceGenomeGene>> refGenesWithSymbolFromNomenClatureAuthority,
        Map<String, Set<ReferenceGenomeGene>> refGenesWithoutSymbolFromNomenClatureAuthority
    )
        throws DaoException {
        ...
        for (Map.Entry<String, Set<ReferenceGenomeGene>> entry : refGenesWithSymbolFromNomenClatureAuthority.entrySet()) {
            ...
            if (refGenes.size() == 1) {
                ...
                // first check if gene exists. If exists, give warning and skip record:
                if (
                    daoReferenceGenomeGene.getGene(
                        refGene.getEntrezGeneId(),
                        refGene.getReferenceGenomeId()
                    ) !=
                    null
                ) {
                    ProgressMonitor.logWarning(
                        "Reference Genome Gene is already in table. Updates are not allowed. If you need to update the `gene` table, please remove all studies first and empty the `gene` table."
                    );
                    nrExisting++;
                    continue;
                }
                try {
                    ...
                    ProgressMonitor.logWarning(
                        "chromosome too large" + refGene.getChr()
                    );
                    continue;
                }
                ProgressMonitor.logWarning(
                    "New reference genome gene with official symbol added"
                );
            } else {
                //TODO - is unexpected for official symbols...raise Exception instead?
                logDuplicateReferenceGenomeGeneSymbolWarning(
                    entry.getKey(),
                    refGenes,
                    true
                );
        ...
        // Add genes without symbol from nomenclature authority
        if (
            refGenesWithoutSymbolFromNomenClatureAuthority.keySet().size() > 0
        ) {
            int nrImported = 0;
            int nrSkipped = 0;
            for (Map.Entry<String, Set<ReferenceGenomeGene>> entry : refGenesWithoutSymbolFromNomenClatureAuthority.entrySet()) {
                ...
                if (refGenes.size() == 1) {
                    ...
                    //only add the gene if it does not conflict with an official symbol:
                    if (
                        !refGenesWithSymbolFromNomenClatureAuthority.containsKey(
                            symbol
                        )
                    ) {
                        //first check if gene exists. If exists, give warning and skip record since we don't allow updates in this process:
            ...
            ProgressMonitor.logWarning(
                "There were " +
                refGenesWithoutSymbolFromNomenClatureAuthority.keySet().size() +
                " genes names in this file without an official symbol from nomenclature authority. Imported: " +
                nrImported +
                ". Gene names skipped (because of duplicate symbol entry or because symbol is an 'official symbol' of another gene): " +
                nrSkipped
    ...
    private static void logDuplicateGeneSymbolWarning(
        String symbol,
        Set<CanonicalGene> genes,
        boolean isOfficialSymbol
    ) {
        ...
        for (CanonicalGene gene : genes) {
            sb.append(" ").append(gene.getEntrezGeneId()).append(". Ignore...");
        }
        ProgressMonitor.logWarning(sb.toString());
    }

    private static void logDuplicateReferenceGenomeGeneSymbolWarning(
        String symbol,
        Set<ReferenceGenomeGene> refGenes,
        boolean isOfficialSymbol
    ) {
        StringBuilder sb = new StringBuilder();
        if (isOfficialSymbol) {
    ...
    public static void importGeneLength(
        File geneFile,
        String genomeBuild,
        String species,
        boolean hasGenes
     ...
     * @throws DaoException
     */
    public static boolean updateLength(
        String symbol,
        String chromosome,
        List<long[]> loci,
        int refreneceGenomeId
    )
        throws IOException, DaoException {
        ...
        if (gene == null) {
            ProgressMonitor.logWarning(
                "Unable to retrieve gene by symbol: " + symbol
            );
        ...
        ReferenceGenomeGene refGene = daoReferenceGenomeGene.getGene(
            gene.getEntrezGeneId(),
            refreneceGenomeId
        );
        ...
        if (refGene != null) {
            ...
            System.out.println(
                " updateLength--> cytoband of the gene:  " + cbChr
            );
            if (cbChr.equals(chromosome)) { //Update the length only if the chromosome matches
                // update reference genome gene
                System.out.println(
                    " updateLength--> update reference genome gene:  " +
                    gene.getHugoGeneSymbolAllCaps()
                );
                ...
                System.out.println(
                    " updateLength--> exonic length of the gene:  " +
                    daoReferenceGenomeGene
                        .getGene(gene.getEntrezGeneId(), refreneceGenomeId)
                        .getExonicLength()
                );
                lengthUpdated = true;
            } else {
                ProgressMonitor.logWarning(
                    "Cytoband does not match, gene not saved (likely another version of gene in gtf has correct chr and is saved)"
                );
            }
        }
        return lengthUpdated;
    ...
    static void importSuppGeneData(
        File suppGeneFile,
        String referenceGenomeBuild
    )
        throws IOException, DaoException {
        ...
        ReferenceGenome refGenome = DaoReferenceGenome.getReferenceGenomeByBuildName(
    ...
    public void run() {
        try {
            ...
            parser
                .accepts("genes", "ncbi genes file")
                .withRequiredArg()
                .describedAs("ncbi_genes.txt")
                .ofType(String.class);
            parser
                .accepts("supp-genes", "alternative genes file")
                .withRequiredArg()
                .describedAs("supp-genes.txt")
                .ofType(String.class);
            parser
                .accepts("microrna", "microrna file")
                .withRequiredArg()
                .describedAs("microrna.txt")
                .ofType(String.class);
            parser
                .accepts(
                    "gtf",
                    "gtf file for calculating and storing gene lengths"
                )
                .withRequiredArg()
                .describedAs("gencode.<version>.annotation.gtf")
                .ofType(String.class);
            parser
                .accepts("genome-build", "genome build eg GRCh38")
                .withRequiredArg()
                .describedAs("genome build")
                .ofType(String.class);
            parser
                .accepts("species", "different kinds of organisms eg. humna")
                .withRequiredArg()
                .describedAs("species")
                .ofType(String.class);
            ...
            if (options.has("genes")) {
                ...
                System.out.println(
                    "Reading gene data from:  " + geneFile.getAbsolutePath()
                );
                numLines = FileUtil.getNumLines(geneFile);
                System.out.println(" --> total number of lines:  " + numLines);
                ProgressMonitor.setMaxValue(numLines);
                MySQLbulkLoader.bulkLoadOn();
                ImportGeneData.importData(
                    geneFile,
            ...
            if (options.has("supp-genes")) {
                ...
                System.out.println(
                    "Reading supp. gene data from:  " +
                    suppGeneFile.getAbsolutePath()
                );
                numLines = FileUtil.getNumLines(suppGeneFile);
                System.out.println(" --> total number of lines:  " + numLines);
                ProgressMonitor.setMaxValue(numLines);
                ImportGeneData.importSuppGeneData(
                    suppGeneFile,
                    (String) options.valueOf("genome-build")
                );
            }
            ...
            if (options.has("microrna")) {
                ...
                System.out.println(
                    "Reading miRNA data from:  " + miRNAFile.getAbsolutePath()
                );
            ...
            if (options.has("gtf")) {
                ...
                System.out.println(
                    "Reading loci data from:  " + lociFile.getAbsolutePath()
                );
                numLines = FileUtil.getNumLines(lociFile);
                System.out.println(" --> total number of lines:  " + numLines);
                ProgressMonitor.setMaxValue(numLines);
                ImportGeneData.importGeneLength(
                    lociFile,
                    (String) options.valueOf("genome-build"),
                    species,
                    options.has("genes")

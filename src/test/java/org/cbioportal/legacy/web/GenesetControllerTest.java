...
public class GenesetControllerTest {
  ...
  @MockitoBean private GenesetService genesetService;
  ...
  public void getAllGenesets() throws Exception {
    ...
    Mockito.when(
            genesetService.getAllGenesets(Mockito.anyString(), Mockito.anyInt(), Mockito.anyInt()))
        .thenReturn(genesetList);

    mockMvc
        .perform(MockMvcRequestBuilders.get("/api/genesets").accept(MediaType.APPLICATION_JSON))
  ...
  public void getGeneset() throws Exception {
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.get("/api/genesets/test_geneset_id")

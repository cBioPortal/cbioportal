...
public class ImportGenesetData extends ConsoleRunnable {
    ...
    public void run() {
        try {
            ...
            OptionSpec<String> data = parser
                .accepts("data", "Geneset data file")
                .withRequiredArg()
                .ofType(String.class);
            OptionSpec<String> supp = parser
                .accepts("supp", "Option gene set supplemental data file")
                .withRequiredArg()
                .ofType(String.class);
            OptionSpec<String> newVersionArgument = parser
                .accepts(
                    "new-version",
                    "New version of gene sets. This will REMOVE GSVA data and hierarchy."
                )
                .withRequiredArg()
                .ofType(String.class);
            parser.accepts(
                "update-info",
                "Only update the info of gene sets (Ref Link, name, description)"
            );

            // For testing purposes, we don't want the prompt, so make extra argument.
            parser.accepts(
                "confirm-delete-all-genesets-hierarchy-genesetprofiles",
                "This will avoid the command prompt being shown."
            );
            ...
            boolean confirmEmptyGenesets = options.has(
                "confirm-delete-all-genesets-hierarchy-genesetprofiles"
            );
            ...
            if ((!updateInfo && !newVersion) || (updateInfo && newVersion)) {
                ...
                if (databaseGenesetVersion == null) {
                    ProgressMonitor.setCurrentMessage(
                        "New version of gene sets specified. Loading to empty database."
                    );
                    ...
                    if (!confirmEmptyGenesets) {
                        ProgressMonitor.setCurrentMessage(
                            "Are you sure you want to remove all previous gene sets, gene set hierarchy and gene set genetic profiles?"
                        );
                        ProgressMonitor.setCurrentMessage(
                        ...
                        try (Scanner scanner = new Scanner(System.in)) {
                            String confirmEmptyingGenesets = scanner
                                .next()
                                .toLowerCase();
                            if (confirmEmptyingGenesets.equals("yes")) {
                                ...
                                ProgressMonitor.setCurrentMessage(
                                    "Replacing existing gene sets not confirmed; aborting."
                                );
                                return;
                            }
                        }
                        ...
                        ProgressMonitor.setCurrentMessage(
                            "Auto confirm to empty gene sets, gene set hierarchy and gene set genetic profiles is set."
                        );
                    }

                    // Empty gene set tables  and import new gene sets
                    ProgressMonitor.setCurrentMessage(
                        "New version of gene sets specified.\nRemoving old gene sets, gene set hierarchy and gene set genetic profiles."
                    );
                ...
                ProgressMonitor.setCurrentMessage(
                    "It is now possible to import gene set hierarchy data and gene set genetic profiles such as GSVA scores."
                );
            }
    ...
    public static int importData(
        ...
        throws Exception {
        ProgressMonitor.setCurrentMessage(
            "Reading data from: " + genesetFile.getCanonicalPath()
        );
        ...
        while (line != null) {
            ...
            Geneset existingGeneset = DaoGeneset.getGenesetByExternalId(
            ...
            if (updateInfo) {
                if (existingGeneset == null) {
                    ProgressMonitor.logWarning(
                        "Could not find gene set " +
                        parts[0] +
                        " in DB. Record will be skipped."
                    );
                } else {
                    ProgressMonitor.setCurrentMessage(
                        "Updating gene set: " + geneset.getExternalId()
                    );
                ...
                ProgressMonitor.setCurrentMessage(
                    "Adding gene set: " + geneset.getExternalId()
                );
        ...
        if (skippedGenes > 0) {
            ProgressMonitor.setCurrentMessage(
                "\n" +
                skippedGenes +
                " times a gene was not found in local gene table. Possible reasons:\n\n" +
    ...
    static void importSuppGenesetData(File suppFile) throws Exception {
        ProgressMonitor.setCurrentMessage(
            "Reading data from: " + suppFile.getCanonicalPath()
        );
        ...
        ProgressMonitor.setCurrentMessage(
            "Finished loading supplementary gene set info.\n"
        );
    }

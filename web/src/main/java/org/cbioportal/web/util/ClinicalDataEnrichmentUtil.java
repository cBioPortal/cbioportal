...
public class ClinicalDataEnrichmentUtil {
    ...
    public List<ClinicalDataEnrichment> createEnrichmentsForNumericData(
        ...
        List<ClinicalAttribute> filteredAttributes = attributes
            .stream()
        ...
        List<Map<String, List<Double>>> dataByGroupAndByAttribute = groupedSamples
            .stream()
            .map(
                groupSamples ->
            ...
            clinicalAttribute -> {
                ...
                for (Map<String, List<Double>> entry : dataByGroupAndByAttribute) {
                    if (entry.containsKey(attributeId)) {
                        Collection<Object> values = entry
                            .get(attributeId)
                            .stream()
                            .collect(Collectors.toList());
                ...
                if (
                    ...
                    double pValue = KruskalWallis.getPvalue(
                    ...
                    if (!Double.isNaN(pValue)) { // this happens when all the values are zero
                        ...
                        clinicalEnrichment.setClinicalAttribute(
                            clinicalAttribute
                        );
                        clinicalEnrichment.setpValue(
    ...
    public List<ClinicalDataEnrichment> createEnrichmentsForCategoricalData(
        ...
        List<String> filteredAttributeIds = filteredAttributes
            .stream()
            .map(ClinicalAttribute::getAttrId)
            .collect(Collectors.toList());

        // ClinicalDataCountItem for all STRING datatype attributes and for all sample groups
        List<Map<String, ClinicalDataCountItem>> dataCountsByGroupAndByAttribute = groupedSamples
            .stream()
            .map(
                groupSamples ->
                    getClinicalDataCounts(filteredAttributeIds, groupSamples)
            )
            .collect(Collectors.toList());
        ...
        filteredAttributes.forEach(
            clinicalAttribute -> {
                ...
                List<Map<String, Integer>> categoryCountsByGroup = dataCountsByGroupAndByAttribute
                    .stream()
                    .map(
                        e -> {
                ...
                Set<String> allPossibleCategories = categoryCountsByGroup
                    .stream()
                ...
                if (allPossibleCategories.size() > 1) {
                    ...
                    for (Map<String, Integer> groupCategoryCounts : categoryCountsByGroup) {
                        Map<String, Integer> allCategoryCounts = allPossibleCategories
                            .stream()
                            .collect(
                                Collectors.toMap(
                                    category -> category,
                                    category -> {
                                        return groupCategoryCounts.containsKey(
                                                category
                                            )
                        ...
                        if (
                            isValidGroupdData(
                                new ArrayList<>(allCategoryCounts.values())
                            )
                        ) {
                    ...
                    if (dataTable.size() > 1 && dataTable.isValid()) {
                        ...
                        clinicalEnrichment.setScore(
                            BigDecimal.valueOf(
                                (Double) Chisquare.getScoreValue(dataTable)
                            )
                        );
            ...
            clinicalDatas
                ...
                .collect(
                    Collectors.groupingBy(
                        x -> x.getAttrId(),
                        Collectors.mapping(
                            x -> Double.valueOf(x.getAttrValue()),
                            Collectors.toList()
                        )
                    )
                );
    ...
    private Map<String, ClinicalDataCountItem> getClinicalDataCounts(
        ...
        List<ClinicalDataCountItem> clinicalDataCountItems = clinicalDataService.fetchClinicalDataCounts(
            studyIds,
            sampleIds,
            attributeIds
        );

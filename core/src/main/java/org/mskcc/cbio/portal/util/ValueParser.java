...
public class ValueParser {
    ...
    public ValueParser(String str, double zScoreThreshold) {
        ...
        if (zScoreThreshold < 0.0) {
            throw new IllegalArgumentException(
                "zScoreThreshold must be greater than 0"
            );
        }
        ParsedFullDataTypeSpec aParsedFullDataTypeSpec = createParsedFullDataTypeSpecFromZscore(
            zScoreThreshold
        );
        ...
        this.theOncoPrintGeneDisplaySpec =
     ...
     * @return
     */
    private ParsedFullDataTypeSpec createParsedFullDataTypeSpecFromZscore(
     ...
     */
    public static ValueParser generateValueParser(
        String gene,
        String value,
        double zScoreThreshold,
        ...
        GeneWithSpec theGeneWithSpec = theOncoPrintSpecification.getGeneWithSpec(
            gene
        );
    ...
    private void determineExpressionThresholds(
        ...
        ResultDataTypeSpec theResultDataTypeSpec = theOncoPrintGeneDisplaySpec.getResultDataTypeSpec(
            theGeneticDataType
        );
        if (
            null != theResultDataTypeSpec &&
            null ==
            theResultDataTypeSpec.getCombinedLesserContinuousDataTypeSpec() &&
            null ==
            theResultDataTypeSpec.getCombinedGreaterContinuousDataTypeSpec()
        ) {
            ParsedFullDataTypeSpec aParsedFullDataTypeSpec = createParsedFullDataTypeSpecFromZscore(
                zScoreThreshold
            );
            ...
            ResultDataTypeSpec expressionResultDataTypeSpec = aParsedFullDataTypeSpec
                .cleanUpInput()
     ...
     */
    public boolean isDiscreteTypeThisLevel(
        GeneticDataTypes theDiscreteGeneticDataType,
        ...
        if (theDiscreteGeneticDataType.equals(GeneticDataTypes.Mutation)) {
            throw new IllegalArgumentException(
                "isDiscreteTypeThisLevel cannot handle Mutations"
            );
        }
        ...
        if (
            ...
            throw new IllegalArgumentException(
                "theDiscreteGeneticDataType is not a " + "discrete datatype"
            );
        }
        ...
        return this.theOncoPrintGeneDisplaySpec.satisfy(
                theDiscreteGeneticDataType,
                theGeneticTypeLevel
            );
    }
     ...
     * @return
     */
    public boolean isDiscreteTypeAltered(
        GeneticDataTypes theDiscreteGeneticDataType
    ) {
        ...
        if (theDiscreteGeneticDataType.equals(GeneticDataTypes.Mutation)) {
            throw new IllegalArgumentException(
                "isDiscreteTypeAltered cannot handle Mutations"
            );
        }
        ...
        String measurement = getValue(
            convertGeneticType(theDiscreteGeneticDataType)
        );
        if (null == measurement) {
            return false;
        }
    ...
    public boolean isCnaAmplified() {
        return isDiscreteTypeThisLevel(
            GeneticDataTypes.CopyNumberAlteration,
    ...
    private boolean doesContinuousValueExceedThreshold(
        GeneticDataTypes theContinuousGeneticDataType,
        ...
        if (
            !theContinuousGeneticDataType
                .getTheDataTypeCategory()
                .equals(DataTypeCategory.Continuous)
        ) {
            throw new IllegalArgumentException(
                "theContinuousGeneticDataType is " + "not a continuous datatype"
            );
        }

        String value = getValue(
            convertGeneticType(theContinuousGeneticDataType)
        );
        // out.println(value);
        if (null == value) {
    ...
    public boolean isMRNAWayUp() {
        return doesContinuousValueExceedThreshold(
            GeneticDataTypes.Expression,
            Direction.higher
        );
    }
    ...
    public boolean isMutated() {
        ...
        if (mutationValue != null) {
            ...
            if (
                mutationValue.equalsIgnoreCase(MUTATION_VALUE_NAN) ||
    ...
    public String getMutationType() {
        String toReturn = datatypeToValueMap.get(
            GeneticAlterationType.MUTATION_EXTENDED.name()
        );
    ...
    public boolean isGeneAltered() {
        return (
            isMutated() ||
            this.isDiscreteTypeAltered(GeneticDataTypes.CopyNumberAlteration) ||
            isMRNAWayUp() ||
            isMRNAWayDown() ||
            isRPPAWayUp() ||
            isRPPAWayDown()
    ...
    private static GeneticAlterationType convertGeneticType(
        GeneticDataTypes theDiscreteGeneticDataType
    ) {

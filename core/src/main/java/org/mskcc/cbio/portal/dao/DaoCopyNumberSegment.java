...
public final class DaoCopyNumberSegment {
    ...
    private static final String FRACTION_GENOME_ALTERED_ATTR_ID =
        "FRACTION_GENOME_ALTERED";

    private DaoCopyNumberSegment() {}

    public static int addCopyNumberSegment(CopyNumberSegment seg)
        throws DaoException {
        if (!MySQLbulkLoader.isBulkLoad()) {
            ...
            MySQLbulkLoader
                .getMySQLbulkLoader("copy_number_seg")
                .insertRecord(
    ...
    public static void createFractionGenomeAlteredClinicalData(
        int cancerStudyId
    )
        throws DaoException {
        Connection con = null;
        ...
        try {
            ...
            for (Map.Entry<Integer, String> fractionGenomeAltered : fractionGenomeAltereds.entrySet()) {
                DaoClinicalData.addSampleDatum(
                    fractionGenomeAltered.getKey(),
                    FRACTION_GENOME_ALTERED_ATTR_ID,
                    fractionGenomeAltered.getValue()
                );
            }
    ...
    public static List<CopyNumberSegment> getSegmentForASample(
        ...
        throws DaoException {
        return getSegmentForSamples(
            Collections.singleton(sampleId),
            cancerStudyId
        );
    }
    ...
    public static double getCopyNumberActeredFraction(
        int sampleId,
        int cancerStudyId,
        double cutoff
    )
        throws DaoException {
        Double d = getCopyNumberActeredFraction(
                Collections.singleton(sampleId),
                cancerStudyId,
                cutoff
            )
    ...
    public static Map<Integer, Double> getCopyNumberActeredFraction(
        Collection<Integer> sampleIds,
        int cancerStudyId,
        double cutoff
    )
        throws DaoException {
        Map<Integer, Long> alteredLength = getCopyNumberAlteredLength(
            sampleIds,
            cancerStudyId,
            cutoff
        );
        Map<Integer, Long> measuredLength = getCopyNumberAlteredLength(
            sampleIds,
            cancerStudyId,
            0
        );
        Map<Integer, Double> fraction = new HashMap<Integer, Double>(
            alteredLength.size()
        );
    ...
    private static Map<Integer, Long> getCopyNumberAlteredLength(
        Collection<Integer> sampleIds,
        int cancerStudyId,
        double cutoff
    )
        ...
        try {
            ...
            if (cutoff > 0) {
                sql =
                    "SELECT  `SAMPLE_ID`, SUM(`END`-`START`)" +
                    " FROM `copy_number_seg`" +
                    " WHERE `CANCER_STUDY_ID`=" +
                    cancerStudyId +
                    " AND ABS(`SEGMENT_MEAN`)>=" +
                    cutoff +
                    " AND `SAMPLE_ID` IN ('" +
                    StringUtils.join(sampleIds, "','") +
                    "')" +
     ...
     * @throws DaoException
     */
    public static boolean segmentDataExistForCancerStudy(int cancerStudyId)
        throws DaoException {
        Connection con = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        try {
            ...
            pstmt =
                con.prepareStatement(
                    "SELECT EXISTS (SELECT 1 FROM `copy_number_seg` WHERE `CANCER_STUDY_ID`=?)"
                );
            pstmt.setInt(1, cancerStudyId);
            rs = pstmt.executeQuery();
     ...
     * @throws DaoException
     */
    public static boolean segmentDataExistForSample(
        int cancerStudyId,
        int sampleId
    )
        throws DaoException {
        ...
        try {
            ...
            pstmt =
                con.prepareStatement(
                    "SELECT EXISTS(SELECT 1 FROM `copy_number_seg`" +

...
public class ScientificSmallDataBinner {
    ...
    public List<DataBin> calculateDataBins(
        ...
        Range<BigDecimal> exponentBoxRange = dataBinHelper.calcBoxRange(
            exponents
        );
        ...
        BigDecimal exponentRange = exponentBoxRange == null
            ...
            : exponentBoxRange
                .upperEndpoint()
                .subtract(exponentBoxRange.lowerEndpoint());
        ...
        if (exponentRange == null) {
            ...
            intervals.add(
                sortedNumericalValues.get(sortedNumericalValues.size() - 1)
            );
        } else if (exponentRange.compareTo(new BigDecimal("1")) == 1) {
            Integer interval = Math.round(exponentRange.floatValue() / 4);

            for (
                int i = exponentBoxRange.lowerEndpoint().intValue() - interval;
            ...
            intervals.add(
                BigDecimal.valueOf(
                    Math.pow(
                        10,
                        exponentBoxRange.lowerEndpoint().doubleValue()
                    ) /
                    3
                )
            );
            ...
            for (
                int i = exponentBoxRange.lowerEndpoint().intValue();
            ...
            BigDecimal interval = BigDecimal.valueOf(
                2 * Math.pow(10, exponentBoxRange.lowerEndpoint().doubleValue())
            );

            for (
                BigDecimal d = BigDecimal.valueOf(
                    Math.pow(10, exponentBoxRange.lowerEndpoint().intValue())
                );
                d.doubleValue() <=
                Math.pow(
                    10,
                    exponentBoxRange.upperEndpoint().doubleValue() + 1
                );
                d = d.add(interval)
            ) {
                intervals.add(d);

...
public class ClickhouseSampleMapperTest {
    ...
    public void getMetaSamples() {
        var allAccSamplesMeta = mapper.getMetaSamples(
            Collections.singletonList(STUDY_ACC_TCGA),
            null, null, null
        );
        assertEquals( 4, allAccSamplesMeta.getTotalCount().intValue());
        
        var allTcgaPubSamplesMeta = mapper.getMetaSamples(
            Collections.singletonList(STUDY_TCGA_PUB),
            null, null, null
        );
        assertEquals( 15, allTcgaPubSamplesMeta.getTotalCount().intValue());

        var allSamplesMetaForAccAndTcga = mapper.getMetaSamples(
            List.of(STUDY_ACC_TCGA, STUDY_TCGA_PUB),
            null, null, null
        );
        assertEquals( 19, allSamplesMetaForAccAndTcga.getTotalCount().intValue());
        
        var tcgaPubSamplesMetaForPatientA1A0SB = mapper.getMetaSamples(
            Collections.singletonList(STUDY_TCGA_PUB), 
            "tcga-a1-a0sb",
            null, null
        );
        ...
        var specificSamplesMetaForAccAndTcga = mapper.getMetaSamples(
            List.of(
                "invalid_study",
                "invalid_study",
                STUDY_ACC_TCGA,
            ...
            List.of(
                ...
                "tcga-a1-a0sq-01",
                "tcga-a1-a0si-01",
                // samples from study_genie_pub (should be filtered out because we didn't include this study)
                "GENIE-TEST-302-01",
                "GENIE-TEST-309-01"
            ),
            null
        ...
        var keywordFilteredSamplesMetaForAccAndTcgaAndGenie =  mapper.getMetaSamples(
    ...
    public void getMetaSamplesBySampleListIds() {
        var studyTcgaPubAll = mapper.getMetaSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_all"));
        assertEquals(14, studyTcgaPubAll.getTotalCount().intValue());
        
        var studyTcgaPubAcgh = mapper.getMetaSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_acgh"));
        assertEquals(14, studyTcgaPubAcgh.getTotalCount().intValue());
        
        var studyTcgaPubCnaseq = mapper.getMetaSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_cnaseq"));
        assertEquals(7, studyTcgaPubCnaseq.getTotalCount().intValue());
        
        var studyTcgaPubComplete = mapper.getMetaSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_complete"));
        assertEquals(7, studyTcgaPubComplete.getTotalCount().intValue());
        
        var studyTcgaPubLog2cna = mapper.getMetaSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_log2cna"));
        assertEquals(14, studyTcgaPubLog2cna.getTotalCount().intValue());
        
        var studyTcgaPubMethylationHm27 = mapper.getMetaSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_methylation_hm27"));
        assertEquals(1, studyTcgaPubMethylationHm27.getTotalCount().intValue());
        
        var studyTcgaPubMrna = mapper.getMetaSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_mrna"));
        assertEquals(8, studyTcgaPubMrna.getTotalCount().intValue());
        
        var studyTcgaPubSequenced = mapper.getMetaSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_sequenced"));
        assertEquals(7, studyTcgaPubSequenced.getTotalCount().intValue());
        
        var studyTcgaPubCna = mapper.getMetaSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_cna"));
        assertEquals(7, studyTcgaPubCna.getTotalCount().intValue());
        
        var studyTcgaPubRnaSeqV2Mrna = mapper.getMetaSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_rna_seq_v2_mrna"));
        assertEquals(7, studyTcgaPubRnaSeqV2Mrna.getTotalCount().intValue());
        
        var studyTcgaPubMicrorna = mapper.getMetaSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_microrna"));
        assertEquals(0, studyTcgaPubMicrorna.getTotalCount().intValue());
        
        var studyTcgaPubRppa = mapper.getMetaSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_rppa"));
        assertEquals(0, studyTcgaPubRppa.getTotalCount().intValue());
        
        var studyTcgaPub3wayComplete = mapper.getMetaSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_3way_complete"));
        assertEquals(7, studyTcgaPub3wayComplete.getTotalCount().intValue());
        
        var accTcgaAll = mapper.getMetaSamplesBySampleListIds(Collections.singletonList("acc_tcga_all"));
        assertEquals(1, accTcgaAll.getTotalCount().intValue());
    }

    @Test
    public void getSamples() {
        var allAccSamples = mapper.getSamples(
            Collections.singletonList(STUDY_ACC_TCGA),
        ...
        var allTcgaPubSamples = mapper.getSamples(
            Collections.singletonList(STUDY_TCGA_PUB),
            null, null, null, null, null, null, null
        );
        ...
        var allSamplesForAccAndTcga = mapper.getSamples(
            List.of(STUDY_ACC_TCGA, STUDY_TCGA_PUB),
            null, null, null, null, null, null, null
        );
        ...
        var tcgaPubSamplesForPatientA1A0SB = mapper.getSamples(
            Collections.singletonList(STUDY_TCGA_PUB),
            "tcga-a1-a0sb",
        ...
        var keywordFilteredSamplesForAccAndTcgaAndGenie =  mapper.getSamples(
            List.of(STUDY_ACC_TCGA, STUDY_TCGA_PUB, STUDY_GENIE_PUB),
            null,
            null,
            "-02",
    ...
    public void getSamplesByStudyAndSampleIds() {
        var userDefinedSamplesForAccAndTcga = mapper.getSamples(
            List.of(
                "invalid_study",
                "invalid_study",
                STUDY_ACC_TCGA,
        ...
        var userDefinedSamplesForGenie = mapper.getSamples(
            ...
            List.of(
                ...
                "GENIE-TEST-302-01",
                "GENIE-TEST-309-01",
                // samples from acc_tcga or study_tcga_pub (should be filtered out because we didn't include this study)
                "tcga-a1-b0sq-01",
                "tcga-a1-a0sq-01",
                "tcga-a1-a0si-01"
            ),
    ...
    public void getSummarySamples() {
        var allAccSamples = mapper.getSummarySamples(
            Collections.singletonList(STUDY_ACC_TCGA),
            null, null, null, null, null, null, null
        );
        ...
        var allTcgaPubSamples = mapper.getSummarySamples(
            Collections.singletonList(STUDY_TCGA_PUB),
            null, null, null, null, null, null, null
        );
        ...
        var allSamplesForAccAndTcga = mapper.getSummarySamples(
            List.of(STUDY_ACC_TCGA, STUDY_TCGA_PUB),
            null, null, null, null, null, null, null
        );
        ...
        var tcgaPubSamplesForPatientA1A0SB = mapper.getSummarySamples(
            Collections.singletonList(STUDY_TCGA_PUB),
            "tcga-a1-a0sb",
            null, null, null, null, null, null
        );
        ...
        var keywordFilteredSamplesForAccAndTcgaAndGenie =  mapper.getSummarySamples(
            List.of(STUDY_ACC_TCGA, STUDY_TCGA_PUB, STUDY_GENIE_PUB),
            null,
            null,
            "-02",
    ...
    public void getSummarySamplesByStudyAndSampleIds() {
        var userDefinedSamplesForAccAndTcga = mapper.getSummarySamples(
            List.of(
                "invalid_study",
                "invalid_study",
                STUDY_ACC_TCGA,
        ...
        var userDefinedSamplesForGenie = mapper.getSummarySamples(
            Collections.singletonList(
                STUDY_GENIE_PUB
            ),
            null,
    ...
    public void getDetailedSamples() {
        var allAccSamples = mapper.getDetailedSamples(
            Collections.singletonList(STUDY_ACC_TCGA),
            null, null, null, null, null, null, null
        );
        assertAccTcgaSamplesDetailed(allAccSamples);
        
        var allTcgaPubSamples = mapper.getDetailedSamples(
            Collections.singletonList(STUDY_TCGA_PUB), 
            null, null, null, null, null, null, null
        );
        assertTcgaPubSamplesDetailed(allTcgaPubSamples);
        
        var allSamplesForAccAndTcga = mapper.getDetailedSamples(
            List.of(STUDY_ACC_TCGA, STUDY_TCGA_PUB),
            null, null, null, null, null, null, null
        );
        assertMultiStudySamplesDetailed(allSamplesForAccAndTcga);

        var tcgaPubSamplesForPatientA1A0SB = mapper.getDetailedSamples(
            Collections.singletonList(STUDY_TCGA_PUB),
            "tcga-a1-a0sb",
            null, null, null, null, null, null
        );
        ...
        var keywordFilteredSamplesForAccAndTcgaAndGenie =  mapper.getDetailedSamples(
            List.of(STUDY_ACC_TCGA, STUDY_TCGA_PUB, STUDY_GENIE_PUB),
            null,
            null,
            "-02",
    ...
    public void getDetailedSamplesByStudyAndSampleIds() {
        var userDefinedSamplesForAccAndTcga = mapper.getDetailedSamples(
            List.of(
                "invalid_study",
                "invalid_study",
                STUDY_ACC_TCGA,
        ...
        var userDefinedSamplesForGenie = mapper.getDetailedSamples(
            Collections.singletonList(
                STUDY_GENIE_PUB
            ),
            null,
    ...
    private void assertAccTcgaSamplesSummary(List<Sample> allAccSamples) {
        assertAccTcgaSampleIds(allAccSamples);
        var a0sb01 = allAccSamples.stream().filter(s -> s.stableId().equals("tcga-a1-a0sb-01")).findFirst().get();
    ...
    private void assertTcgaPubSamplesSummary(List<Sample> allTcgaPubSamples) {
        assertTcgaPubSampleIds(allTcgaPubSamples);
        var a0sb02 = allTcgaPubSamples.stream().filter(s -> s.stableId().equals("tcga-a1-a0sb-02")).findFirst().get();
    ...
    private void assertMultiStudySampleIds(List<Sample> allSamplesForAccAndTcga) {
        ...
        var allSamplesIdsForAccAndTcga = allSamplesForAccAndTcga.stream().map(Sample::stableId).toList();
        // we have tcga-a1-a0sb-01 in both studies, so the count should be 2
        assertEquals(2, allSamplesIdsForAccAndTcga.stream().filter(s -> s.equals("tcga-a1-a0sb-01")).toList().size());
    ...
    private void assertMultiStudySamplesDetailed(List<Sample> allSamplesForAccAndTcga) {
        assertMultiStudySamplesSummary(allSamplesForAccAndTcga);
        assertNotNull(allSamplesForAccAndTcga.getFirst().patient().cancerStudy().cancerStudyIdentifier());
    }
    ...
    private void assertSampleIdsForPatientA1A0SB(List<Sample> tcgaPubSamplesForPatientA1A0SB) {
        ...
        var sampleIdsForPatientA1A0SB = tcgaPubSamplesForPatientA1A0SB.stream().map(Sample::stableId).toList();
    ...
    private void assertSamplesSummaryForPatientA1A0SB(List<Sample> tcgaPubSamplesForPatientA1A0SB) {
        ...
        assertEquals(SampleType.PRIMARY_SOLID_TUMOR, tcgaPubSamplesForPatientA1A0SB.getFirst().sampleType());
        assertEquals(1, tcgaPubSamplesForPatientA1A0SB.getLast().patientId().intValue());
        assertEquals(SampleType.PRIMARY_SOLID_TUMOR, tcgaPubSamplesForPatientA1A0SB.getLast().sampleType());
    }
    ...
    private void assertSamplesDetailedForPatientA1A0SB(List<Sample> tcgaPubSamplesForPatientA1A0SB) {
        ...
        assertEquals(STUDY_TCGA_PUB, tcgaPubSamplesForPatientA1A0SB.getFirst().patient().cancerStudy().cancerStudyIdentifier());
        assertEquals("tcga-a1-a0sb", tcgaPubSamplesForPatientA1A0SB.getLast().patient().stableId());
        assertEquals(STUDY_TCGA_PUB, tcgaPubSamplesForPatientA1A0SB.getLast().patient().cancerStudy().cancerStudyIdentifier());
    }
    ...
    private void assertUserDefinedSampleIds(List<Sample> userDefinedSamplesForAccAndTcga) {
        ...
        var specificSampleIdsForAccAndTcga = userDefinedSamplesForAccAndTcga.stream().map(Sample::stableId).toList();
    ...
    private void assertUserDefinedSamplesSummary(List<Sample> userDefinedSamplesForAccAndTcga) {
        assertUserDefinedSampleIds(userDefinedSamplesForAccAndTcga);
        assertEquals(SampleType.PRIMARY_SOLID_TUMOR, userDefinedSamplesForAccAndTcga.get(0).sampleType());
        assertEquals(SampleType.PRIMARY_SOLID_TUMOR, userDefinedSamplesForAccAndTcga.get(1).sampleType());
        assertEquals(SampleType.PRIMARY_SOLID_TUMOR, userDefinedSamplesForAccAndTcga.get(2).sampleType());
    }

    private void assertUserDefinedSamplesDetailed(List<Sample> userDefinedSamplesForAccAndTcga) {
        assertUserDefinedSamplesSummary(userDefinedSamplesForAccAndTcga);
        assertNotNull(userDefinedSamplesForAccAndTcga.get(0).patient().cancerStudy().cancerStudyIdentifier());
        assertNotNull(userDefinedSamplesForAccAndTcga.get(1).patient().cancerStudy().cancerStudyIdentifier());
        assertNotNull(userDefinedSamplesForAccAndTcga.get(2).patient().cancerStudy().cancerStudyIdentifier());
    }
    ...
    private void assertUserDefinedGenieSampleIds(List<Sample> userDefinedSamplesForGenie) {
        ...
        var specificSampleIdsForGenie = userDefinedSamplesForGenie.stream().map(Sample::stableId).toList();
    ...
    private void assertUserDefinedGenieSamplesDetailed(List<Sample> userDefinedSamplesForGenie) {
        assertUserDefinedGenieSamplesSummary(userDefinedSamplesForGenie);
        assertNotNull(userDefinedSamplesForGenie.get(0).patient().cancerStudy().cancerStudyIdentifier());
        assertNotNull(userDefinedSamplesForGenie.get(1).patient().cancerStudy().cancerStudyIdentifier());
    }

    private void assertKeywordFilteredSampleIds(List<Sample> keywordFilteredSamplesForAccAndTcgaAndGenie) {
        assertEquals( 4, keywordFilteredSamplesForAccAndTcgaAndGenie.size());
        var keywordFilteredSamplesIds = keywordFilteredSamplesForAccAndTcgaAndGenie.stream().map(Sample::stableId).toList();
    ...
    private void assertKeywordFilteredSamplesSummary(List<Sample> keywordFilteredSamplesForAccAndTcgaAndGenie) {
        assertKeywordFilteredSampleIds(keywordFilteredSamplesForAccAndTcgaAndGenie);
        assertEquals(SampleType.PRIMARY_SOLID_TUMOR, keywordFilteredSamplesForAccAndTcgaAndGenie.get(0).sampleType());
        assertEquals(SampleType.PRIMARY_SOLID_TUMOR, keywordFilteredSamplesForAccAndTcgaAndGenie.get(1).sampleType());
        assertEquals(SampleType.PRIMARY_SOLID_TUMOR, keywordFilteredSamplesForAccAndTcgaAndGenie.get(2).sampleType());
        assertEquals(SampleType.PRIMARY_SOLID_TUMOR, keywordFilteredSamplesForAccAndTcgaAndGenie.get(3).sampleType());
    }

    private void assertKeywordFilteredSamplesDetailed(List<Sample> keywordFilteredSamplesForAccAndTcgaAndGenie) {
        assertKeywordFilteredSamplesSummary(keywordFilteredSamplesForAccAndTcgaAndGenie);
        assertNotNull(keywordFilteredSamplesForAccAndTcgaAndGenie.get(0).patient().cancerStudy().cancerStudyIdentifier());
        assertNotNull(keywordFilteredSamplesForAccAndTcgaAndGenie.get(1).patient().cancerStudy().cancerStudyIdentifier());
        assertNotNull(keywordFilteredSamplesForAccAndTcgaAndGenie.get(2).patient().cancerStudy().cancerStudyIdentifier());
        assertNotNull(keywordFilteredSamplesForAccAndTcgaAndGenie.get(3).patient().cancerStudy().cancerStudyIdentifier());
    }
    
    @Test
    public void getSamplesBySampleListIds() {
        var studyTcgaPubAll = mapper.getSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_all"));
        assertEquals(14, studyTcgaPubAll.size());

        var studyTcgaPubAcgh = mapper.getSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_acgh"));
        assertEquals(14, studyTcgaPubAcgh.size());

        var studyTcgaPubCnaseq = mapper.getSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_cnaseq"));
        assertEquals(7, studyTcgaPubCnaseq.size());

        var studyTcgaPubComplete = mapper.getSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_complete"));
        assertEquals(7, studyTcgaPubComplete.size());

        var studyTcgaPubLog2cna = mapper.getSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_log2cna"));
        assertEquals(14, studyTcgaPubLog2cna.size());

        var studyTcgaPubMethylationHm27 = mapper.getSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_methylation_hm27"));
        assertEquals(1, studyTcgaPubMethylationHm27.size());

        var studyTcgaPubMrna = mapper.getSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_mrna"));
        assertEquals(8, studyTcgaPubMrna.size());

        var studyTcgaPubSequenced = mapper.getSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_sequenced"));
        assertEquals(7, studyTcgaPubSequenced.size());

        var studyTcgaPubCna = mapper.getSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_cna"));
        assertEquals(7, studyTcgaPubCna.size());

        var studyTcgaPubRnaSeqV2Mrna = mapper.getSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_rna_seq_v2_mrna"));
        assertEquals(7, studyTcgaPubRnaSeqV2Mrna.size());

        var studyTcgaPubMicrorna = mapper.getSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_microrna"));
        assertEquals(0, studyTcgaPubMicrorna.size());

        var studyTcgaPubRppa = mapper.getSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_rppa"));
        assertEquals(0, studyTcgaPubRppa.size());

        var studyTcgaPub3wayComplete = mapper.getSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_3way_complete"));
    ...
    public void getSummarySamplesBySampleListIds() {
        var studyTcgaPubAll = mapper.getSummarySamplesBySampleListIds(Collections.singletonList("study_tcga_pub_all"));
        assertEquals(14, studyTcgaPubAll.size());
        assertEquals(SampleType.PRIMARY_SOLID_TUMOR, studyTcgaPubAll.getFirst().sampleType());

        var studyTcgaPubAcgh = mapper.getSummarySamplesBySampleListIds(Collections.singletonList("study_tcga_pub_acgh"));
        assertEquals(14, studyTcgaPubAcgh.size());
        assertEquals(SampleType.PRIMARY_SOLID_TUMOR, studyTcgaPubAcgh.getFirst().sampleType());

        var studyTcgaPubCnaseq = mapper.getSummarySamplesBySampleListIds(Collections.singletonList("study_tcga_pub_cnaseq"));
        assertEquals(7, studyTcgaPubCnaseq.size());
        assertEquals(SampleType.PRIMARY_SOLID_TUMOR, studyTcgaPubCnaseq.getFirst().sampleType());

        var studyTcgaPubComplete = mapper.getSummarySamplesBySampleListIds(Collections.singletonList("study_tcga_pub_complete"));
        assertEquals(7, studyTcgaPubComplete.size());
        assertEquals(SampleType.PRIMARY_SOLID_TUMOR, studyTcgaPubComplete.getFirst().sampleType());

        var studyTcgaPubLog2cna = mapper.getSummarySamplesBySampleListIds(Collections.singletonList("study_tcga_pub_log2cna"));
        assertEquals(14, studyTcgaPubLog2cna.size());
        assertEquals(SampleType.PRIMARY_SOLID_TUMOR, studyTcgaPubLog2cna.getFirst().sampleType());

        var studyTcgaPubMethylationHm27 = mapper.getSummarySamplesBySampleListIds(Collections.singletonList("study_tcga_pub_methylation_hm27"));
        assertEquals(1, studyTcgaPubMethylationHm27.size());
        assertEquals(SampleType.PRIMARY_SOLID_TUMOR, studyTcgaPubMethylationHm27.getFirst().sampleType());

        var studyTcgaPubMrna = mapper.getSummarySamplesBySampleListIds(Collections.singletonList("study_tcga_pub_mrna"));
        assertEquals(8, studyTcgaPubMrna.size());
        assertEquals(SampleType.PRIMARY_SOLID_TUMOR, studyTcgaPubMrna.getFirst().sampleType());

        var studyTcgaPubSequenced = mapper.getSummarySamplesBySampleListIds(Collections.singletonList("study_tcga_pub_sequenced"));
        assertEquals(7, studyTcgaPubSequenced.size());
        assertEquals(SampleType.PRIMARY_SOLID_TUMOR, studyTcgaPubSequenced.getFirst().sampleType());

        var studyTcgaPubCna = mapper.getSummarySamplesBySampleListIds(Collections.singletonList("study_tcga_pub_cna"));
        assertEquals(7, studyTcgaPubCna.size());
        assertEquals(SampleType.PRIMARY_SOLID_TUMOR, studyTcgaPubCna.getFirst().sampleType());

        var studyTcgaPubRnaSeqV2Mrna = mapper.getSummarySamplesBySampleListIds(Collections.singletonList("study_tcga_pub_rna_seq_v2_mrna"));
        assertEquals(7, studyTcgaPubRnaSeqV2Mrna.size());
        assertEquals(SampleType.PRIMARY_SOLID_TUMOR, studyTcgaPubRnaSeqV2Mrna.getFirst().sampleType());

        var studyTcgaPub3wayComplete = mapper.getSummarySamplesBySampleListIds(Collections.singletonList("study_tcga_pub_3way_complete"));
        assertEquals(7, studyTcgaPub3wayComplete.size());
        assertEquals(SampleType.PRIMARY_SOLID_TUMOR, studyTcgaPub3wayComplete.getFirst().sampleType());

        var accTcgaAll = mapper.getSummarySamplesBySampleListIds(Collections.singletonList("acc_tcga_all"));
        assertEquals(1, accTcgaAll.size());
    ...
    public void getDetailedSamplesBySampleListIds() {
        var studyTcgaPubAll = mapper.getDetailedSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_all"));
        assertEquals(14, studyTcgaPubAll.size());

        var studyTcgaPubAcgh = mapper.getDetailedSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_acgh"));
        assertEquals(14, studyTcgaPubAcgh.size());

        var studyTcgaPubCnaseq = mapper.getDetailedSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_cnaseq"));
        assertEquals(7, studyTcgaPubCnaseq.size());

        var studyTcgaPubComplete = mapper.getDetailedSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_complete"));
        assertEquals(7, studyTcgaPubComplete.size());

        var studyTcgaPubLog2cna = mapper.getDetailedSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_log2cna"));
        assertEquals(14, studyTcgaPubLog2cna.size());

        var studyTcgaPubMethylationHm27 = mapper.getDetailedSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_methylation_hm27"));
        assertEquals(1, studyTcgaPubMethylationHm27.size());

        var studyTcgaPubMrna = mapper.getDetailedSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_mrna"));
        assertEquals(8, studyTcgaPubMrna.size());

        var studyTcgaPubSequenced = mapper.getDetailedSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_sequenced"));
        assertEquals(7, studyTcgaPubSequenced.size());
        // all samples should be sequenced
        assertEquals(7, studyTcgaPubSequenced.stream().filter(Sample::sequenced).count());

        var studyTcgaPubCna = mapper.getDetailedSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_cna"));
        assertEquals(7, studyTcgaPubCna.size());
        ...
        var studyTcgaPubRnaSeqV2Mrna = mapper.getDetailedSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_rna_seq_v2_mrna"));
        assertEquals(7, studyTcgaPubRnaSeqV2Mrna.size());

        var studyTcgaPubMicrorna = mapper.getDetailedSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_microrna"));
        assertEquals(0, studyTcgaPubMicrorna.size());

        var studyTcgaPubRppa = mapper.getDetailedSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_rppa"));
        assertEquals(0, studyTcgaPubRppa.size());

        var studyTcgaPub3wayComplete = mapper.getDetailedSamplesBySampleListIds(Collections.singletonList("study_tcga_pub_3way_complete"));
        assertEquals(7, studyTcgaPub3wayComplete.size());

        var accTcgaAll = mapper.getDetailedSamplesBySampleListIds(Collections.singletonList("acc_tcga_all"));
        assertEquals(1, accTcgaAll.size());
    }

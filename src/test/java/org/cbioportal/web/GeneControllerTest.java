...
public class GeneControllerTest {
  ...
  public void getAllGenesDefaultProjection() throws Exception {
    ...
    mockMvc
        .perform(MockMvcRequestBuilders.get("/api/genes").accept(MediaType.APPLICATION_JSON))
  ...
  public void getAllGenesMetaProjection() throws Exception {
    ...
    mockMvc
        .perform(MockMvcRequestBuilders.get("/api/genes").param("projection", "META"))
        .andExpect(MockMvcResultMatchers.status().isOk())
  ...
  public void getGeneNotFound() throws Exception {
    ...
    Mockito.when(geneService.getGene(eq("test_gene_id")))
        .thenThrow(new GeneNotFoundException("test_gene_id"));

    mockMvc
        .perform(
            MockMvcRequestBuilders.get("/api/genes/test_gene_id")
                .accept(MediaType.APPLICATION_JSON))
        .andExpect(MockMvcResultMatchers.status().isNotFound())
        .andExpect(
            MockMvcResultMatchers.jsonPath("$.message").value("Gene not found: test_gene_id"));
  }
  ...
  public void getAliasesOfGene() throws Exception {
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.get("/api/genes/test_gene_id/aliases")
                .accept(MediaType.APPLICATION_JSON))
        .andExpect(MockMvcResultMatchers.status().isOk())
        .andExpect(
            MockMvcResultMatchers.content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
  ...
  public void fetchGenesDefaultProjection() throws Exception {
    ...
    Mockito.when(
            geneService.fetchGenes(Mockito.anyList(), Mockito.anyString(), Mockito.anyString()))
        .thenReturn(geneList);
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.post("/api/genes/fetch")
                .with(csrf())
                .accept(MediaType.APPLICATION_JSON)

...
public class GenesetCorrelationControllerTest {
  ...
  @MockitoBean public GenesetCorrelationService genesetCorrelationService;
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.post(
                    "/api/genesets/" + QUERY_GENESET_ID + "/expression-correlation/fetch")
                .with(csrf())
        ...
        .andExpect(
            MockMvcResultMatchers.jsonPath("$[0].expressionGeneticProfileId").value(EXPR_PROF_ID))
        .andExpect(
            MockMvcResultMatchers.jsonPath("$[0].zScoreGeneticProfileId").value(ZSCORE_PROF_ID))
        ...
        .andExpect(
            MockMvcResultMatchers.jsonPath("$[1].expressionGeneticProfileId").value(EXPR_PROF_ID))
        .andExpect(
            MockMvcResultMatchers.jsonPath("$[1].zScoreGeneticProfileId").value(ZSCORE_PROF_ID));
  }

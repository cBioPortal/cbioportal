...
public class GeneController {
  ...
  public ResponseEntity<List<Gene>> getAllGenes(
      @Parameter(description = "Search keyword that applies to hugo gene symbol of the genes")
          ...
          String keyword,
      @Parameter(description = "Alias of the gene") @RequestParam(required = false) String alias,
    ...
    if (projection == Projection.META) {
      ...
      responseHeaders.add(
          ...
          geneService.getMetaGenes(keyword, alias).getTotalCount().toString());
  ...
  @RequestMapping(
      value = "/api/genes/{geneId}/aliases",
      method = RequestMethod.GET,
      produces = MediaType.APPLICATION_JSON_VALUE)
  ...
  public ResponseEntity<List<Gene>> fetchGenes(
    ...
    if (projection == Projection.META) {
      ...
      responseHeaders.add(
          HeaderKeyConstants.TOTAL_COUNT,
          geneService.fetchMetaGenes(geneIds, geneIdType.name()).getTotalCount().toString());
      return new ResponseEntity<>(responseHeaders, HttpStatus.OK);
    } else {
      return new ResponseEntity<>(
          geneService.fetchGenes(geneIds, geneIdType.name(), projection.name()), HttpStatus.OK);
    }
  }
}

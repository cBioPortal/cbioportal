...
public class GeneController {
    ...
    public ResponseEntity<List<Gene>> getAllGenes(
        @ApiParam(
            "Search keyword that applies to hugo gene symbol of the genes"
        ) @RequestParam(required = false) String keyword,
        @ApiParam("Alias of the gene") @RequestParam(
            required = false
        ) String alias,
        @ApiParam("Level of detail of the response") @RequestParam(
            defaultValue = "SUMMARY"
        ) Projection projection,
        @ApiParam("Page size of the result list") @Max(GENE_MAX_PAGE_SIZE) @Min(
            PagingConstants.MIN_PAGE_SIZE
    ...
    public ResponseEntity<Gene> getGene(
        @ApiParam(
            required = true,
            value = "Entrez Gene ID or Hugo Gene Symbol e.g. 1 or A1BG"
    ...
    public ResponseEntity<List<String>> getAliasesOfGene(
        ...
        throws GeneNotFoundException, GeneWithMultipleEntrezIdsException {
        return new ResponseEntity<>(
            geneService.getAliasesOfGene(geneId),
            HttpStatus.OK
        );
    }
    ...
    public ResponseEntity<List<Gene>> fetchGenes(
        @ApiParam("Type of gene ID") @RequestParam(
            defaultValue = "ENTREZ_GENE_ID"
        ) GeneIdType geneIdType,
        @ApiParam(
            required = true,
            value = "List of Entrez Gene IDs or Hugo Gene Symbols"
        ) @Size(
            min = 1,
        ...
        if (projection == Projection.META) {
            ...
            responseHeaders.add(
                HeaderKeyConstants.TOTAL_COUNT,
                geneService
                    .fetchMetaGenes(geneIds, geneIdType.name())
                    .getTotalCount()
                    .toString()
            );
            return new ResponseEntity<>(responseHeaders, HttpStatus.OK);

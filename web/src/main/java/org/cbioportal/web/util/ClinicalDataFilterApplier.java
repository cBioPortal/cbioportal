...
public abstract class ClinicalDataFilterApplier {
    ...
    public List<SampleIdentifier> apply(
        ...
        if (!clinicalDataFilters.isEmpty() && !sampleIdentifiers.isEmpty()) {
            ...
            List<ClinicalData> sampleClinicalDataList = clinicalDataService.fetchClinicalData(
                studyIds,
                sampleIds,
                attributeIds,
                "SAMPLE",
            ...
            List<ClinicalData> patientClinicalDataList = clinicalDataService.getPatientClinicalDataDetailedToSample(
                studyIdsOfPatients,
                patientIds,
                attributeIds
            );
            ...
            clinicalDataList.forEach(
                c -> {
                    ...
                    if (
                        c.getAttrValue().equals("NAN") ||
                        c.getAttrValue().equals("N/A")
                    ) {
                        c.setAttrValue("NA");
                    }
                }
            ...
            for (ClinicalData clinicalData : clinicalDataList) {
                if (
                    clinicalDataMap.containsKey(
                        clinicalData.getSampleId(),
                        clinicalData.getStudyId()
                    )
                ) {
                    (
                        (List<ClinicalData>) clinicalDataMap.get(
                            clinicalData.getSampleId(),
                            clinicalData.getStudyId()
                        )
                    ).add(clinicalData);

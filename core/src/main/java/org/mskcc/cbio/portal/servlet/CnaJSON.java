...
public class CnaJSON extends HttpServlet {
    ...
    private void processGetCnaRequest(
        HttpServletRequest request,
        HttpServletResponse response
    )
        throws ServletException, IOException {
        ...
        Boolean filterByCbioGene = Boolean.parseBoolean(
            request.getParameter(CBIO_GENES_FILTER)
        );
                ...
                if (cnaProfile != null) {
                    ...
                    if (
                        ...
                        if (!cnaEvents.isEmpty()) {
                            String concatEventIds = getConcatEventIds(
                                cnaEvents
                            );
                            int profileId = cnaProfile.getGeneticProfileId();
                            //                            drugs = getDrugs(cnaEvents, fdaOnly, cancerDrug);
                            contextMap =
                                DaoCnaEvent.countSamplesWithCnaEvents(
                                    concatEventIds,
                                    profileId
                                );
                            if (
                                mrnaProfileId != null && sampleIds.length == 1
                            ) {
                                mrnaContext =
                                    getMrnaContext(
                                        internalSampleIds.get(0),
                                        cnaEvents,
                                        mrnaProfileId
                                    );
                            }
    ...
    private void processGetSegmentsRequest(
        HttpServletRequest request,
        HttpServletResponse response
    )
        throws ServletException, IOException {
        String[] sampleIds = request
        ...
        try {
            if (cancerStudyId != null) {
                ...
                if (
                    ...
                    segs =
                        DaoCopyNumberSegment.getSegmentForSamples(
                            internalSampleIds,
                            cancerStudy.getInternalId()
                        );
                }
        ...
        for (CopyNumberSegment seg : segs) {
            String stableId = DaoSample
                .getSampleById(seg.getSampleId())
                .getStableId();
    ...
    private void processCnaFractionsRequest(
        HttpServletRequest request,
        HttpServletResponse response
    )
        throws ServletException, IOException {
        ...
        try {
            out.write(
                mapper.writeValueAsString(
                    replaceInternalSampleIdsWithStableIds(fraction)
                )
            );
        } finally {
            out.close();
    ...
    private Map<String, Double> replaceInternalSampleIdsWithStableIds(
        Map<Integer, Double> fraction
    ) {
        Map<String, Double> toReturn = new HashMap<String, Double>();
        for (Integer internalId : fraction.keySet()) {
            toReturn.put(
                DaoSample.getSampleById(internalId).getStableId(),
    ...
    private Map<Long, Set<String>> getDrugs(
        ...
        for (CnaEvent cnaEvent : cnaEvents) {
            ...
            if (
                cnaEvent.getAlteration() == CNA.AMP ||
                cnaEvent.getAlteration() == CNA.GAIN
            ) { // since drugs are usually intibiting
                genes.add(gene);
            }

            // Temporary way of handling cases such as akt inhibitor for pten loss
            Set<Long> targets = daoDrugInteraction.getMoreTargets(
                gene,
                cnaEvent.getAlteration().name()
            );
    ...
    private void exportCnaEvent(
        ...
        try {
            gistic =
                getGistic(
                    cancerStudy.getInternalId(),
                    cnaEvent.getGeneSymbol(),
                    cnaEvent.getAlteration()
                );
        } catch (DaoException ex) {
            throw new ServletException(ex);
        }
    ...
    private static final Map<Integer, Map<String, Map<CNA, List>>> gisticMap = new HashMap<Integer, Map<String, Map<CNA, List>>>(); // map from cancer study id // to map from gene to a list of params

    private static List getGistic(int cancerStudyId, String gene, CNA cna)
        throws DaoException {
        ...
        synchronized (gisticMap) {
            ...
            if (mapGeneGistic == null) {
                ...
                List<Gistic> gistics = DaoGistic.getAllGisticByCancerStudyId(
                    cancerStudyId
                );
                for (Gistic g : gistics) {
                    List<String> genes = new ArrayList<String>(
                        g.getGenes_in_ROI().size()
                    );

...
public class UuidDataAccessTokenServiceImplTestConfiguration {
  ...
  private List<DataAccessToken> dataAccessTokenListForMockUserWithOneToken =
      makeDataAccessTokenListForMockUserWithOneToken();
  private List<DataAccessToken> dataAccessTokenListForMockUserWithFiveTokens =
      makeDataAccessTokenListForMockUserWithFiveTokens();
  ...
  public DataAccessTokenRepository dataAccessTokenRepository() {
    Answer<Void> dataAccessTokenRepositoryCreateTokenAnswer =
        new Answer<Void>() {
    ...
    Answer<Void> dataAccessTokenRepositoryDeleteTokenAnswer =
        new Answer<Void>() {
    ...
    DataAccessTokenRepository dataAccessTokenRepository =
        Mockito.mock(DataAccessTokenRepository.class);
    Mockito.when(dataAccessTokenRepository.getDataAccessToken(FAIL_TO_GET_TOKEN_STRING))
        .thenThrow(new RuntimeException("Fail to get this token"));
    Mockito.when(dataAccessTokenRepository.getDataAccessToken(NONEXISTENT_TOKEN_STRING))
        .thenReturn(null);
    Mockito.when(dataAccessTokenRepository.getDataAccessToken(EXPIRED_TOKEN_STRING))
        .thenReturn(makeExpiredDataAccessToken());
    Mockito.when(dataAccessTokenRepository.getDataAccessToken(VALID_TOKEN_STRING))
    ...
    Mockito.when(
        ...
        .thenReturn(sortByExpiration(dataAccessTokenListForMockUserWithFiveTokens));
    Mockito.doAnswer(dataAccessTokenRepositoryDeleteTokenAnswer)
        .when(dataAccessTokenRepository)
        .removeDataAccessToken(ArgumentMatchers.anyString());
    Mockito.doAnswer(dataAccessTokenRepositoryCreateTokenAnswer)
        .when(dataAccessTokenRepository)
        .addDataAccessToken(ArgumentMatchers.any(DataAccessToken.class));
  ...
  private List<DataAccessToken> makeDataAccessTokenListForMockUserWithOneToken() {
    List<DataAccessToken> dataAccessTokenListForMockUserWithOneToken =
        new ArrayList<DataAccessToken>();
    dataAccessTokenListForMockUserWithOneToken.add(
        makeDataAccessToken(OLDEST_TOKEN_UUID, MOCK_USERNAME_WITH_ONE_TOKEN, -150000));
  ...
  private List<DataAccessToken> makeDataAccessTokenListForMockUserWithFiveTokens() {
    List<DataAccessToken> dataAccessTokenListForMockUserWithFiveTokens =
        new ArrayList<DataAccessToken>();
    ...
    dataAccessTokenListForMockUserWithFiveTokens.add(
        makeDataAccessToken(OLDEST_TOKEN_UUID, MOCK_USERNAME_WITH_FIVE_TOKENS, 45000));
    dataAccessTokenListForMockUserWithFiveTokens.add(
        makeDataAccessToken(NEWEST_TOKEN_UUID, MOCK_USERNAME_WITH_FIVE_TOKENS, 1000000));

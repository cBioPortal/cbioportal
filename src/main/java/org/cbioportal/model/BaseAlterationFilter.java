...
public class BaseAlterationFilter implements Serializable {
  ...
  @JsonIgnore protected Select<String> tiersSelect = Select.all();
  ...
  public void setTiersBooleanMap(Map<String, Boolean> tiersBooleanMap) {
    if (tiersBooleanMap == null) {
      ...
      this.tiersSelect =
          Select.byValues(
              tiersBooleanMap.entrySet().stream().filter(e -> e.getValue()).map(e -> e.getKey()));
  ...
  public Select<String> getSelectedTiers() {
    if (tiersSelect == null) return Select.none();
    return tiersSelect;
  }

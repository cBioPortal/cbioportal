...
public class ImportGenericAssayEntity extends ConsoleRunnable {
    ...
    public ImportGenericAssayEntity(
        File dataFile,
    ...
    public void run() {
        try {
            ...
            String description =
                "Import generic assay records from generic assay response files.";

            OptionParser parser = new OptionParser();
            OptionSpec<String> data = parser
                .accepts("data", "Generic assay data file")
                .withRequiredArg()
                .ofType(String.class);

            // require entity type
            OptionSpec<String> entityType = parser
                .accepts("entity-type", "Entity type")
                .withRequiredArg()
                .ofType(String.class);

            // don't require additional properties
            OptionSpec<String> additionalProperties = parser
                .accepts(
                    "additional-properties",
                    "Additional properties need to be imported"
                )
                .withOptionalArg()
                .ofType(String.class);
            ...
            ProgressMonitor.setCurrentMessage(
                "Adding new generic assay to the database\n"
            );
            startImport(
                options,
                data,
    ...
    public static void startImport(
        ...
        OptionSpec<String> geneticAlterationType,
        OptionSpec<String> additionalProperties,
        boolean updateInfo
    )
        throws Exception {
        if (
            options.hasArgument(data) &&
            options.hasArgument(geneticAlterationType)
        ) {
            File genericAssayFile = new File(options.valueOf(data));
            GeneticAlterationType geneticAlterationTypeArg = GeneticAlterationType.valueOf(
                options.valueOf(geneticAlterationType)
            );
            String additionalPropertiesArg = options.valueOf(
                additionalProperties
            );
            importData(
                genericAssayFile,
                geneticAlterationTypeArg,
    ...
    public static void importData(
        ...
        throws Exception {
        ProgressMonitor.setCurrentMessage(
            "Reading data from: " + dataFile.getCanonicalPath()
        );
        ...
        while (currentLine != null) {
            ...
            if (additionalProperties != null) {
                String[] columnNameList = additionalProperties
                    .trim()
                    .split(",");
                for (String columnName : columnNameList) {
                    int indexAdditionalField = getColIndexByName(
                        headerNames,
                        columnName
                    );
            ...
            if (genericAssayMeta != null) {
                ProgressMonitor.setCurrentMessage(
                    "Cannot add new entity, entity exists: " +
                    genericAssayMetaStableId
                );
            }
            // create a new generic assay meta and add to the database
            else {
                GeneticEntity newGeneticEntity = new GeneticEntity(
                    geneticAlterationType.name(),
                    stableId
                );
                GeneticEntity createdGeneticEntity = DaoGeneticEntity.addNewGeneticEntity(
                    newGeneticEntity
                );

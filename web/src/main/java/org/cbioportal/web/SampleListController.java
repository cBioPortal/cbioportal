...
public class SampleListController {
    ...
    public ResponseEntity<List<SampleList>> getAllSampleListsInStudy(
        ...
        if (projection == Projection.META) {
            ...
            responseHeaders.add(
                HeaderKeyConstants.TOTAL_COUNT,
                sampleListService
                    .getMetaSampleListsInStudy(studyId)
                    .getTotalCount()
                    .toString()
            );
            return new ResponseEntity<>(responseHeaders, HttpStatus.OK);
    ...
    public ResponseEntity<List<String>> getAllSampleIdsInSampleList(
        ...
        throws SampleListNotFoundException {
        return new ResponseEntity<>(
            sampleListService.getAllSampleIdsInSampleList(sampleListId),
            HttpStatus.OK
        );
    }

    @PreAuthorize(
        "hasPermission(#sampleListIds, 'Collection<SampleListId>', 'read')"
    )
    @RequestMapping(
    ...
    public ResponseEntity<List<SampleList>> fetchSampleLists(
        @ApiParam(required = true, value = "List of sample list IDs") @Size(
            min = 1,

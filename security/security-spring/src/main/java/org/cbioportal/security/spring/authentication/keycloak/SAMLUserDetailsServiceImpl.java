...
public class SAMLUserDetailsServiceImpl implements SAMLUserDetailsService {
    ...
    public Object loadUserBySAML(SAMLCredential credential) {
        ...
        for (Attribute cAttribute : credential.getAttributes()) {
            ...
            if (
                userId == null &&
                attrName.equals(SAML_IDP_METADATA_EMAIL_ATTR_NAME)
            ) {
        ...
        try {
            if (userId == null) {
                String errorMsg =
                    "loadUserBySAML(), can not instantiate PortalUserDetails from SAML assertion." +
            ...
            if (userRoles.size() > 0) userDetailsObj =
            ...
            throw new RuntimeException(
                "Error occurs during authentication: " + e.getMessage()
            );
        }
    }
}

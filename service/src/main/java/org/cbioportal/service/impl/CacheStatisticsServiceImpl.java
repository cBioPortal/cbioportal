...
public class CacheStatisticsServiceImpl implements CacheStatisticsService {
    ...
    private void checkIfCacheStatisticsEndpointEnabled() {
        if (!cacheStatisticsEndpointEnabled) {
            throw new AccessDeniedException(
                "Cache statistics is not enabled for this instance of the portal."
            );
        }
    }

    @Override
    public List<String> getKeyCountsPerClass(String cacheName)
        ...
        while (iterator.hasNext()) {
            ...
            int keyCount = classToKeyCount.containsKey(className)
    ...
    @Override
    public List<String> getKeysInCache(String cacheName)
        throws CacheNotFoundException {
        checkIfCacheStatisticsEndpointEnabled();
        Cache cache = cacheManager.getCache(cacheName);
        if (cache == null) {

...
public class DataAccessTokenController {
    private final List<String> SUPPORTED_DAT_METHODS = Arrays.asList(
        "uuid",
        "jwt",
        "none"
    );
    ...
    public void postConstruct() {
        if (datMethod == null || !SUPPORTED_DAT_METHODS.contains(datMethod)) {
            throw new RuntimeException(
                "Specified data access token method, " +
                datMethod +
                " is not supported"
            );
        } else {
            this.tokenService =
                this.dataAccessTokenServiceFactory.getDataAccessTokenService(
                        this.datMethod
                    );
        }
    }
    ...
    private void initializeUsersWhoCannotUseTokenSet() {
        usersWhoCannotUseTokenSet =
            new HashSet<>(Arrays.asList(USERS_WHO_CANNOT_USE_TOKENS));
    }

    @RequestMapping(
        method = RequestMethod.POST,
        value = "/data-access-tokens",
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    public ResponseEntity<DataAccessToken> createDataAccessToken(
        ...
        if (allowRevocationOfOtherTokens != null) {
            createdToken =
                tokenService.createDataAccessToken(
    ...
    public ResponseEntity<List<DataAccessToken>> getAllDataAccessTokens(
        ...
        List<DataAccessToken> allDataAccessTokens = tokenService.getAllDataAccessTokens(
            getAuthenticatedUser(authentication)
        );
    ...
    public void revokeAllDataAccessTokens(Authentication authentication) {
        tokenService.revokeAllDataAccessTokens(
            getAuthenticatedUser(authentication)
        );
    }

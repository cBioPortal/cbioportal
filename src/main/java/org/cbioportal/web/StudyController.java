...
public class StudyController {
  ...
  public ResponseEntity<List<CancerStudy>> fetchStudies(
    ...
    if (projection == Projection.META) {
      ...
      responseHeaders.add(
          HeaderKeyConstants.TOTAL_COUNT,
          studyService.fetchMetaStudies(studyIds).getTotalCount().toString());
      return new ResponseEntity<>(responseHeaders, HttpStatus.OK);
    } else {
      return new ResponseEntity<>(
  ...
  @ApiResponse(
      ...
      description = "OK",
      content =
          @Content(array = @ArraySchema(schema = @Schema(implementation = CancerStudyTags.class))))
  ...
  private AccessLevel getAccessLevel() {
    return PortalSecurityConfig.userAuthorizationEnabled(authenticate)
            && showUnauthorizedStudiesOnHomePage
        ? AccessLevel.LIST
        : AccessLevel.READ;
  }
}

...
public class DiscreteDataTypeSpec extends DataTypeSpecInequality {
    ...
    public DiscreteDataTypeSpec(
        ...
        if (
            theGeneticDataType == GeneticDataTypes.CopyNumberAlteration &&
            !theGeneticDataType.equals(
                ((GeneticTypeLevel) threshold).getTheGeneticDataType()
            )
        ) {
            // assumes that levels are organized in increasing order
            throw new IllegalArgumentException(
                "threshold is not a level within theGeneticDataType"
            );
        }
    }
     ...
     */
    public static DiscreteDataTypeSpec discreteDataTypeSpecGenerator(
        String theGeneticDataTypeString,
        String comparisonOpString,
        String levelString
    ) {
        try {
            GeneticDataTypes theGeneticDataType = DiscreteDataTypeSpec.findDataType(
            ...
            return new DiscreteDataTypeSpec(
                theGeneticDataType,
                theComparisonOp,
                threshold
            );
    ...
    public boolean satisfy(Object value) {
        if (value instanceof GeneticTypeLevel) {
            ...
            return (
                ...
                value.equals(threshold.toString())
            );
        }
        // keep compiler happy
    ...
    public DiscreteDataTypeSetSpec convertToDiscreteDataTypeSetSpec() {
        if (threshold instanceof GeneticTypeLevel) {
            ...
            threshold instanceof String
        ) {
            return DiscreteDataTypeSetSpec.specificMutationDataTypeSetSpecGenerator(
                (String) threshold
            );
        }

...
public class OncotatorParser {
    ...
    public static OncotatorRecord parseJSON(String key, String json)
        ...
        if (!errorNode.isMissingNode()) {
            System.out.println(
                "JSON parse error for " + key + ": " + errorNode.getTextValue()
            );
            return null;
        }
        ...
        JsonNode bestTranscriptIndexNode = rootNode.path(
            "best_canonical_transcript"
        );
        ...
        if (!bestTranscriptIndexNode.isMissingNode()) {
            ...
            String variantClassification = bestTranscriptNode
                .path("variant_classification")
                .getTextValue();
            String proteinChange = bestTranscriptNode
                .path("protein_change")
                .getTextValue();
            String geneSymbol = bestTranscriptNode.path("gene").getTextValue();
            int exonAffected = bestTranscriptNode
                .path("exon_affected")
                .getIntValue();

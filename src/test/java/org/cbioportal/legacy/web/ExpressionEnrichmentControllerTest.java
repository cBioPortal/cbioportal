...
public class ExpressionEnrichmentControllerTest {
  ...
  private static final BigDecimal TEST_STANDARD_DEVIATION_IN_UNALTERED_GROUP_1 =
      new BigDecimal(2.6);
  ...
  private static final BigDecimal TEST_STANDARD_DEVIATION_IN_UNALTERED_GROUP_2 =
      new BigDecimal(3.1);
  private static final BigDecimal TEST_P_VALUE_2 = new BigDecimal(2.1);

  @MockitoBean private ExpressionEnrichmentService expressionEnrichmentService;
  ...
  public void fetchGenomicEnrichments() throws Exception {
    ...
    Mockito.when(
            expressionEnrichmentService.getGenomicEnrichments(
                Mockito.anyString(), Mockito.anyMap(), Mockito.any(EnrichmentType.class)))
        .thenReturn(expressionEnrichments);

    mockMvc
        .perform(
            MockMvcRequestBuilders.post("/api/expression-enrichments/fetch")
        ...
        .andExpect(
            MockMvcResultMatchers.jsonPath("$[0].groupsStatistics[0].meanExpression")
                .value(TEST_MEAN_EXPRESSION_IN_ALTERED_GROUP_1))
        .andExpect(
            MockMvcResultMatchers.jsonPath("$[0].groupsStatistics[0].standardDeviation")
                .value(TEST_STANDARD_DEVIATION_IN_ALTERED_GROUP_1))
        .andExpect(
            MockMvcResultMatchers.jsonPath("$[0].groupsStatistics[1].meanExpression")
                .value(TEST_MEAN_EXPRESSION_IN_UNALTERED_GROUP_1))
        .andExpect(
            MockMvcResultMatchers.jsonPath("$[0].groupsStatistics[1].standardDeviation")
        ...
        .andExpect(
            MockMvcResultMatchers.jsonPath("$[1].groupsStatistics[0].meanExpression")
                .value(TEST_MEAN_EXPRESSION_IN_ALTERED_GROUP_2))
        .andExpect(
            MockMvcResultMatchers.jsonPath("$[1].groupsStatistics[0].standardDeviation")
                .value(TEST_STANDARD_DEVIATION_IN_ALTERED_GROUP_2))
        .andExpect(
            MockMvcResultMatchers.jsonPath("$[1].groupsStatistics[1].meanExpression")
                .value(TEST_MEAN_EXPRESSION_IN_UNALTERED_GROUP_2))
        .andExpect(
            MockMvcResultMatchers.jsonPath("$[1].groupsStatistics[1].standardDeviation")
  ...
  public void fetchGenericAssayEnrichments() throws Exception {
    ...
    Mockito.when(
            expressionEnrichmentService.getGenericAssayNumericalEnrichments(
                Mockito.anyString(), Mockito.anyMap(), Mockito.any(EnrichmentType.class)))
        .thenReturn(genericAssayEnrichments);

    mockMvc
        .perform(
            MockMvcRequestBuilders.post("/api/generic-assay-enrichments/fetch")
                .with(csrf())
                .accept(MediaType.APPLICATION_JSON)
                .contentType(MediaType.APPLICATION_JSON)
                .content(

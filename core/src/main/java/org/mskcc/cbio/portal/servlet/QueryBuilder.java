...
public class QueryBuilder extends HttpServlet {
    ...
    public static final String CLINICAL_PARAM_SELECTION =
    ...
    public static final String ATTRIBUTE_URL_BEFORE_FORWARDING =
    ...
    public static final String MRNA_PROFILES_SELECTED =
        "MRNA_PROFILES_SELECTED";
    public static final String COMPUTE_LOG_ODDS_RATIO =
        "COMPUTE_LOG_ODDS_RATIO";
    public static final int MUTATION_DETAIL_LIMIT = 100;
    public static final String MUTATION_DETAIL_LIMIT_REACHED =
    ...
    public static final String SELECTED_PATIENT_SAMPLE_ID_MAP =
        "selected_patient_sample_id_map";
    public static final String DB_VERSION = "db_version";
    public static final String DB_ERROR = "db_error";
    public static final String IS_VIRTUAL_STUDY = "is_virtual_study";
    private static final String DB_CONNECT_ERROR =
        (
    ...
    protected void doPost(
        ...
        HashSet<String> geneticProfileIdSet = getGeneticProfileIds(
            httpServletRequest,
            xdebug
        );
        ...
        try {
            ...
            try {
                dataTypePriority =
                    Integer.parseInt(
                        httpServletRequest.getParameter(DATA_PRIORITY).trim()
                    );
            ...
            String[] exampleStudyQueries = GlobalProperties
                .getExampleStudyQueries()
                .split("\n");
            httpServletRequest.setAttribute(
                "exampleStudyQueries",
                exampleStudyQueries
            ...
            String cancerStudyId = httpServletRequest.getParameter(
                CANCER_STUDY_ID
            );
            String cancerStudyList = httpServletRequest.getParameter(
                CANCER_STUDY_LIST
            );
            ...
            if (
                ...
                cancerStudyList.split(",").length == 1
            ) {
                ...
                if (cancerStudyId != null || cancerStudyList != null) {
                    ...
                    if (cancerStudy != null) {
                        ...
                        if (addProfiles) {
                            ...
                            if (cnaProfile != null) {
                                geneticProfileIdSet.add(
                                    cnaProfile.getStableId()
                                );
                            }
            ...
            if (cancerStudyId != null) {
                ...
                if (cancerStudyId.equals("all") && cancerStudyList != null) {
                    queriedStudies = cancerStudyList;
                }

                Set<String> inputCohortIds = new HashSet<>(
                    Arrays.asList(queriedStudies.split(","))
                );
                            ...
                            .filter(
                                obj -> {
                                    if (inputCohortIds.contains(obj.getId())) {
                                        for (VirtualStudySamples _study : obj
                                            .getData()
                                            .getStudies()) {
                                            try {
                                                if (
                ...
                if (
                    ...
                    studies
                        ...
                        .forEach(
                            obj -> {
                                for (VirtualStudySamples cohortStudyCasesMap : obj
                                    .getData()
                                    .getStudies()) {
                                    if (
                                        studySampleMap.containsKey(
                                            cohortStudyCasesMap.getId()
                                        )
                                    ) {
                                        ...
                                        if (
                                            studySampleMap
                                                ...
                                                .size() !=
                                            0
                                        ) { // original entry does not contain all samples
                                            Set<String> mergedSet = mergeSets(
                                                studySampleMap.get(
                                                    _sharedStudyId
                                                ),
                    ...
                    if (errorsExist) {
                        httpServletRequest.setAttribute(
                            QueryBuilder.USER_ERROR_MESSAGE,
            ...
            String errorMessage = "";
            if (
                dbPortalExpectedSchemaVersion != null &&
                !dbPortalExpectedSchemaVersion.equals(dbActualSchemaVersion)
            ) {
                errorMessage +=
                    "Error could also be related to incompatible DB schema version: DB is at schema version " +
                    dbActualSchemaVersion +
                    " while portal expects schema version " +
                    dbPortalExpectedSchemaVersion +
                    ". ";
            }
     ...
     */
    private HashSet<String> getGeneticProfileIds(
        HttpServletRequest httpServletRequest,
        XDebug xdebug
    ) {
        ...
        while (nameEnumeration.hasMoreElements()) {
            ...
            if (currentName.startsWith(GENETIC_PROFILE_IDS)) {
                String geneticProfileIds[] = httpServletRequest.getParameterValues(
                    currentName
                );
    ...
    private void processData(
        Map<String, Set<String>> inputStudySampleMap,
        String geneList,
        ...
        if (
            inputStudySampleMap.keySet().size() == 1 &&
            inputStudySampleMap.containsKey(singleStudyId)
        ) { // single study
            String _cancerStudyId = inputStudySampleMap
                .keySet()
                .iterator()
                .next();
            if (!sampleSetId.equals("-1")) {
                for (SampleList sampleSet : GetSampleLists.getSampleLists(
                    _cancerStudyId
                )) {
                ...
                List<String> samplesList = new ArrayList<>(
                    Arrays.asList(sampleIdsStr.split(" "))
                );
            ...
            if (
                ...
                sampleIdsStr != null &&
                sampleIdsStr.length() > 0
            ) { //using user customized case list
                studySampleMap = parseCaseIdsTextBoxStr(sampleIdsStr);
            } else {
                ...
                inputStudySampleMap
                    ...
                    .parallelStream()
                    .forEach(
                        (String _cancerStudyId) -> {
                            try {
                                for (SampleList sampleList : GetSampleLists.getSampleLists(
                                    _cancerStudyId
                                )) {
                                    if (
                                        sampleList
                                            .getSampleListCategory()
                                            .equals(sampleListCategory) ||
                                        (
                                            sampleListCategory.equals(
                                                SampleListCategory.ALL_CASES_IN_STUDY
                                            ) &&
                                            sampleList
                                                .getStableId()
                                                .equals(_cancerStudyId + "_all")
                                        )
                                    ) {
                                        ...
                                        if (
                                            ...
                                            sampleIds.retainAll(
                                                inputStudySampleMap.get(
                                                    _cancerStudyId
                                                )
                                            );
                                        }
                                        studySampleMapConcurrent.put(
                                            _cancerStudyId,
                                            sampleIds
                                        );
                                        break;
        ...
        for (String cancerStudyId : inputStudySampleMap.keySet()) {
            CancerStudy selectedCancerStudy = DaoCancerStudy.getCancerStudyByStableId(
                cancerStudyId
            );
            ...
            for (String profileId : geneticProfileIdSet) {
                ...
                ) geneticProfileMap.put(
                    profileId,
                    GeneticProfileUtil.getProfile(profileId, geneticProfileList)
                );
            }
            if (
                inputStudySampleMap.size() > 1 ||
                !inputStudySampleMap.containsKey(singleStudyId)
            ) {
                ...
                for (String pId : defaultGeneticProfileSet.keySet()) {
                    geneticProfileMap.put(
                        pId,
                        defaultGeneticProfileSet.get(pId)
                    );
                }
            }
        }
        ...
        Map<String, Set<String>> cancerTypesMap = DaoClinicalData.getCancerTypeInfoBySamples(
            samples
        );
        if (cancerTypesMap.keySet().size() > 1) {
            showCancerTypesSummary = true;
        } else if (
            cancerTypesMap.keySet().size() == 1 &&
            cancerTypesMap.values().iterator().next().size() > 1
        ) {
            showCancerTypesSummary = true;
        }
        ...
        if (tabIndex != null && tabIndex.equals(QueryBuilder.TAB_VISUALIZE)) {
            HashSet<String> geneticProfileIds = new HashSet<String>(
                geneticProfileMap.keySet()
            );
            ArrayList<GeneticProfile> geneticProfiles = new ArrayList<>(
                geneticProfileMap.values()
            );
            ...
            RequestDispatcher dispatcher = getServletContext()
                .getRequestDispatcher("/WEB-INF/jsp/visualize.jsp");
            dispatcher.forward(request, response);
        } else if (
            tabIndex != null && tabIndex.equals(QueryBuilder.TAB_DOWNLOAD)
        ) {
            ...
            for (GeneticProfile profile : geneticProfileMap.values()) {
                String _sampleIdsStr = StringUtils.join(
                    studySampleMap.get(
                        DaoCancerStudy
                            .getCancerStudyByInternalId(
                                profile.getCancerStudyId()
                            )
                ...
                if (_sampleIdsStr != null && _sampleIdsStr.length() != 0) {
                    GetProfileData remoteCall = new GetProfileData(
                        profile,
                        new ArrayList<>(
                            Arrays.asList(decodedGeneList.split("( )|(\\n)"))
                        ),
                        _sampleIdsStr
                    );
            ...
            ShowData.showDataAtSpecifiedIndex(
                servletContext,
                request,
                response,
                0,
    ...
    private void redirectStudyUnavailable(
        HttpServletRequest request,
        HttpServletResponse response
    )
        throws ServletException, IOException {
    ...
    private Map<String, List<String>> parseCaseIdsTextBoxStr(String _inputStr) {
        ...
        for (String _seg : _segs) {
            ...
            if (_resultMap.containsKey(_studyId)) {
                ...
                List<String> _tmpSampleList = new ArrayList<>(
                    Arrays.asList(_sampleId)
                );
                _resultMap.put(_studyId, _tmpSampleList);
            }
        }
    ...
    private boolean validateForm(
        ...
        throws DaoException {
        boolean errorsExist = false;
        String tabIndex = httpServletRequest.getParameter(
            QueryBuilder.TAB_INDEX
        );
        if (action != null) {
            if (action.equals(ACTION_SUBMIT)) {
                if (studySampleMap.keySet().size() == 0) {
                    httpServletRequest.setAttribute(
                        ...
                        "You are not authorized to view the cancer study."
                    ); //TODO: how to specify the Id(s) here?
                    errorsExist = true;
                } else {
                    UserDetails ud = accessControl.getUserDetails();
                    if (ud != null) {
                        LOG.info(
                            "QueryBuilder.validateForm: Query initiated by user: " +
                            ud.getUsername()
                        );
                    }
                }
                ...
                if (studySampleMap.keySet().size() == 1) {
                    if (geneticProfileIdSet.size() == 0) {
                        if (
                            tabIndex == null ||
                            tabIndex.equals(QueryBuilder.TAB_DOWNLOAD)
                        ) {
                            httpServletRequest.setAttribute(
                                STEP2_ERROR_MSG,
                    ...
                    if (
                        ...
                        if (sampleIds.trim().length() == 0) {
                            ...
                            String sampleSetErrMsg =
                                "Invalid samples(s) for the selected cancer study:";
    ...
    private void forwardToErrorPage(
        HttpServletRequest request,
        HttpServletResponse response,
        String userMessage,
        XDebug xdebug
    ...
    private Set<VirtualStudy> getStudies(Set<String> inputCohortIds)
        throws DaoException {
        ...
        for (String inputCohortId : inputCohortIds) {
            try {
                ...
                // is virtual study
                if (cancerStudy == null) {
                    VirtualStudy virtualStudy = sessionServiceUtil.getVirtualStudyData(
                        inputCohortId
                    );
                    if (
                        virtualStudy != null &&
                        virtualStudy.getData().getStudies().size() > 0
                    ) {
...
enum SampleSet {
    ...
    W_MUT_CNA(
        "w_mut_cna",
        SampleListCategory.ALL_CASES_WITH_MUTATION_AND_CNA_DATA
    ),

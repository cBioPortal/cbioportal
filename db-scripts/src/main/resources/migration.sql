...
FROM `mutation` , `genetic_profile`, `mutation_event` WHERE mutation.`GENETIC_PROFILE_ID` = genetic_profile.`GENETIC_PROFILE_ID` 
...
ALTER TABLE `patient` ADD INDEX `PATIENT_STABLE_ID_INDEX` (`STABLE_ID`);
ALTER TABLE `sample` ADD INDEX `SAMPLE_STABLE_ID_INDEX` (`STABLE_ID`);

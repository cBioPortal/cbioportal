...
public class ReferenceGenomeGeneController {
     ...
     */
    @RequestMapping(
        value = "/reference-genome-genes/{genomeName}",
    ...
    public ResponseEntity<List<ReferenceGenomeGene>> getAllReferenceGenomeGenes(
        @ApiParam(
            required = true,
            value = "Name of Reference Genome hg19"
        ) @PathVariable String genomeName
    ) {
        List<ReferenceGenomeGene> genes = geneMemoizerService.fetchGenes(
            genomeName
        );
        if (genes == null) {
            genes =
    ...
    @RequestMapping(
        value = "/reference-genome-genes/{genomeName}/{geneId}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    ...
    @RequestMapping(
        value = "/reference-genome-genes/{genomeName}/fetch",
    ...
    public ResponseEntity<List<ReferenceGenomeGene>> fetchReferenceGenomeGenes(
        ...
        @ApiParam(required = true, value = "List of Entrez Gene IDs") @Size(
            min = 1,
            max = GENE_MAX_PAGE_SIZE
        ) @RequestBody List<String> geneIds
    ) {
        if (isInteger(geneIds.get(0))) {
            List<Integer> newIds = geneIds
                .stream()
            ...
            return new ResponseEntity<>(
                referenceGenomeGeneService.fetchGenesByHugoGeneSymbolsAndGenomeName(
                    geneIds,
                    genomeName
                ),
                HttpStatus.OK

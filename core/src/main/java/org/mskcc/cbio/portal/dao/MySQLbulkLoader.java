...
public class MySQLbulkLoader {
    ...
    private void openTempFile(String tableName) throws IOException {
        ...
        if (!tempFileHandle.exists()) {
            throw new FileNotFoundException(
                "File does not exist: " + tempFileHandle
            );
        }
        if (!tempFileHandle.isFile()) {
            throw new IllegalArgumentException(
                "Should not be a directory: " + tempFileHandle
            );
        }
        if (!tempFileHandle.canWrite()) {
            throw new IllegalArgumentException(
                "File cannot be written: " + tempFileHandle
            );
        }

        // to improve performance use buffering; FileWriter always assumes default encoding is OK!
        this.tempFileWriter =
            new BufferedWriter(new FileWriter(tempFileHandle, false));
    }
     ...
     * @throws IOException
     */
    private int loadDataFromTempFileIntoDBMS()
        ...
        try {
            ...
            if (nLines != updateCount && !relaxedMode) {
                ...
                if (stmt.getWarnings() != null) {
                    otherDetails =
                        "More error/warning details: " +
                        stmt.getWarnings().getMessage();
                }

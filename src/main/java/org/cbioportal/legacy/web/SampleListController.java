...
public class SampleListController {
  ...
  @PreAuthorize(
      "hasPermission(#sampleListId, 'SampleListId', T(org.cbioportal.legacy.utils.security.AccessLevel).READ)")
  @RequestMapping(
      value = "/sample-lists/{sampleListId}",
      method = RequestMethod.GET,
      produces = MediaType.APPLICATION_JSON_VALUE)
  ...
  @RequestMapping(
      value = "/studies/{studyId}/sample-lists",
      method = RequestMethod.GET,
      produces = MediaType.APPLICATION_JSON_VALUE)
  ...
  public ResponseEntity<List<SampleList>> getAllSampleListsInStudy(
    ...
    if (projection == Projection.META) {
      ...
      responseHeaders.add(
          HeaderKeyConstants.TOTAL_COUNT,
          sampleListService.getMetaSampleListsInStudy(studyId).getTotalCount().toString());
      return new ResponseEntity<>(responseHeaders, HttpStatus.OK);
    } else {
      return new ResponseEntity<>(
  ...
  @PreAuthorize(
      "hasPermission(#sampleListId, 'SampleListId', T(org.cbioportal.legacy.utils.security.AccessLevel).READ)")
  @RequestMapping(
      value = "/sample-lists/{sampleListId}/sample-ids",
      method = RequestMethod.GET,
      produces = MediaType.APPLICATION_JSON_VALUE)
  ...
  public ResponseEntity<List<String>> getAllSampleIdsInSampleList(
    ...
    return new ResponseEntity<>(
        sampleListService.getAllSampleIdsInSampleList(sampleListId), HttpStatus.OK);
  }

  @PreAuthorize(
      "hasPermission(#sampleListIds, 'Collection<SampleListId>', T(org.cbioportal.legacy.utils.security.AccessLevel).READ)")
  @RequestMapping(
      value = "/sample-lists/fetch",

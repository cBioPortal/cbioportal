...
public class OncoPrintSpecification {
    ...
    public OncoPrintSpecification(String[] genes) {
        ...
        constructOncoPrintSpecification(
            genes,
        ...
        for (GeneSet aGeneSet : this.GeneSets) {
            for (GeneWithSpec aGeneWithSpec : aGeneSet.getGenes()) {
                for (GeneticDataTypes theGeneticDataType : GeneticDataTypes.values()) {
                    ResultDataTypeSpec theResultDataTypeSpec = aGeneWithSpec.theOncoPrintGeneDisplaySpec.getResultDataTypeSpec(
                        theGeneticDataType
                    );
                    if (null != theResultDataTypeSpec) {
                        ResultDataTypeSpec copyOfResultDataTypeSpec = ResultDataTypeSpec.newInstance(
                            theResultDataTypeSpec
                        );
                        if (copyOfResultDataTypeSpec.isAcceptAll()) {
                            aParsedFullDataTypeSpec.addSpec(
                                new ConcreteDataTypeSpec(theGeneticDataType)
                            );
                        } else {
                            aParsedFullDataTypeSpec.addSpec(
                                copyOfResultDataTypeSpec.getTheDiscreteDataTypeSetSpec()
                            );
                            aParsedFullDataTypeSpec.addSpec(
                                copyOfResultDataTypeSpec.getCombinedLesserContinuousDataTypeSpec()
                            );
                            aParsedFullDataTypeSpec.addSpec(
                                copyOfResultDataTypeSpec.getCombinedGreaterContinuousDataTypeSpec()
                            );
                        }
                    }
                }

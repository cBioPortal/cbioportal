...
public class TreatmentController {
  ...
  @Autowired private StudyViewFilterUtil filterUtil;
  ...
  @ApiResponse(
      ...
      content =
          @Content(
              array = @ArraySchema(schema = @Schema(implementation = PatientTreatmentRow.class))))
  public ResponseEntity<List<PatientTreatmentRow>> getAllPatientTreatments(
    ...
    List<PatientTreatmentRow> treatments =
        this.getInstance()
            .cachableGetAllPatientTreatments(
                tier, interceptedStudyViewFilter, singleStudyUnfiltered);
  ...
  @ApiResponse(
      ...
      content =
          @Content(
              array = @ArraySchema(schema = @Schema(implementation = SampleTreatmentRow.class))))
  public ResponseEntity<List<SampleTreatmentRow>> getAllSampleTreatments(
    ...
    List<SampleTreatmentRow> treatments =
        this.getInstance()
            .cacheableGetAllSampleTreatments(
                tier, interceptedStudyViewFilter, singleStudyUnfiltered);
    return new ResponseEntity<>(treatments, HttpStatus.OK);
  }
  ...
  @RequestMapping(
      value = "/treatments/display-patient",
      method = RequestMethod.POST,
      produces = MediaType.APPLICATION_JSON_VALUE)
  ...
  public ResponseEntity<Boolean> getContainsTreatmentData(
      ...
      @Parameter(required = true, description = "List of Study IDs")
          ...
          @RequestBody
          Set<String> studyIds) {
    Boolean containsTreatmentData =
        this.getInstance().cacheableGetContainsTreatmentData(studyIds, tier);
  ...
  @PreAuthorize(
      "hasPermission(#studyIds, 'Collection<CancerStudyId>', T(org.cbioportal.utils.security.AccessLevel).READ)")
  @RequestMapping(
      value = "/treatments/display-sample",
      method = RequestMethod.POST,
      produces = MediaType.APPLICATION_JSON_VALUE)
  ...
  public ResponseEntity<Boolean> getContainsSampleTreatmentData(
      ...
      @Parameter(required = true, description = "List of Study IDs")
          ...
          Set<String> studyIds) {
    Boolean containsTreatmentData =
        this.getInstance().cacheableGetContainsSampleTreatmentData(studyIds, tier);
    return new ResponseEntity<>(containsTreatmentData, HttpStatus.OK);
  }

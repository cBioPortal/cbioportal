...
public class ExpressionEnrichmentServiceImplTest extends BaseServiceImplTest {
  ...
  @Spy @InjectMocks private ExpressionEnrichmentUtil expressionEnrichmentUtil;
  ...
  Map<String, List<MolecularProfileCaseIdentifier>> molecularProfilePatientLevelCaseSets =
      new HashMap<>();
  ...
  public void setup() throws MolecularProfileNotFoundException {
    ...
    List<MolecularProfileCaseIdentifier> unalteredPatientLevelSampleIdentifieres =
        new ArrayList<>();
    ...
    molecularProfilePatientLevelCaseSets.put(
        "unaltered samples", unalteredPatientLevelSampleIdentifieres);
    ...
    Mockito.when(
            sampleService.fetchSamples(
                ...
                Arrays.asList(SAMPLE_ID3, SAMPLE_ID4, SAMPLE_ID1, SAMPLE_ID2),
                "ID"))
        .thenReturn(samples);
  }
  ...
  public void getGenomicEnrichments() throws Exception {
    ...
    Mockito.when(
            molecularDataRepository.getGeneMolecularAlterationsIterableFast(MOLECULAR_PROFILE_ID))
    ...
    Assert.assertEquals(
        new BigDecimal("0.6363961030678927"), unalteredGroupStats.getStandardDeviation());
    ...
    Assert.assertEquals(
        new BigDecimal("0.4949747468305834"), unalteredGroupStats.getStandardDeviation());
    ...
    Assert.assertEquals(
        new BigDecimal("2.7577164466275352"), alteredGroupStats.getStandardDeviation());
  ...
  public void getGenericAssayNumericalEnrichments() throws Exception {
    ...
    List<GenericAssayMolecularAlteration> molecularDataList =
        new ArrayList<GenericAssayMolecularAlteration>();
    GenericAssayMolecularAlteration genericAssayMolecularAlteration1 =
    ...
    GenericAssayMolecularAlteration genericAssayMolecularAlteration2 =
        new GenericAssayMolecularAlteration();
    ...
    Mockito.when(
            molecularDataRepository.getGenericAssayMolecularAlterationsIterable(
                MOLECULAR_PROFILE_ID, null, "SUMMARY"))
        .thenReturn(molecularDataList);

    Mockito.when(
            genericAssayService.getGenericAssayMetaByStableIdsAndMolecularIds(
                Arrays.asList(HUGO_GENE_SYMBOL_1, HUGO_GENE_SYMBOL_2),
                Arrays.asList(MOLECULAR_PROFILE_ID, MOLECULAR_PROFILE_ID),
                "SUMMARY"))
        .thenReturn(
            Arrays.asList(
                new GenericAssayMeta(HUGO_GENE_SYMBOL_1),
                new GenericAssayMeta(HUGO_GENE_SYMBOL_2)));

    List<GenericAssayEnrichment> result =
        enrichmentServiceImpl.getGenericAssayNumericalEnrichments(
            MOLECULAR_PROFILE_ID, molecularProfileCaseSets, EnrichmentType.SAMPLE);
  ...
  public void getGenericAssayPatientLevelEnrichments() throws Exception {
    ...
    Mockito.when(
            sampleService.fetchSamples(
                ...
                Arrays.asList(SAMPLE_ID3, SAMPLE_ID4, SAMPLE_ID5, SAMPLE_ID1, SAMPLE_ID2),
                "SUMMARY"))
        .thenReturn(samples);

    List<GenericAssayEnrichment> result =
        enrichmentServiceImpl.getGenericAssayNumericalEnrichments(
            MOLECULAR_PROFILE_ID, molecularProfilePatientLevelCaseSets, EnrichmentType.SAMPLE);

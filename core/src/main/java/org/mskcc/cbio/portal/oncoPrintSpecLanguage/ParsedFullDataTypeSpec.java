...
public class ParsedFullDataTypeSpec {
    ...
    public OncoPrintGeneDisplaySpec cleanUpInput() {
        EnumMap<GeneticDataTypes, TempFullDataTypeSpec> organizedDataTypeSpecs = new EnumMap<GeneticDataTypes, TempFullDataTypeSpec>(
            GeneticDataTypes.class
        );

        for (GeneticDataTypes aGeneticDataType : GeneticDataTypes.values()) {
            organizedDataTypeSpecs.put(
                aGeneticDataType,
                new TempFullDataTypeSpec()
            );
        }
        ...
        for (DataTypeSpec aDataTypeSpec : this.theDataTypeSpecs) {
            organizedDataTypeSpecs
                .get(aDataTypeSpec.theGeneticDataType)
                .addSpec(aDataTypeSpec);
        }
        ...
        for (GeneticDataTypes aGeneticDataType : GeneticDataTypes.values()) {
            if (
                ...
                organizedDataTypeSpecs
                    .get(aGeneticDataType)
                    .cleanUpDataType(
                        theResultFullDataTypeSpec.createResultDataTypeSpec(
                            aGeneticDataType
                        ),
                        aGeneticDataType
                    );

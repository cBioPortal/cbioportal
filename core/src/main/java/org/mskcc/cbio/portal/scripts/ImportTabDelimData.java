...
public class ImportTabDelimData {
    ...
    public void importData(int numLines) throws IOException, DaoException {
        ...
        try {
            ...
            int sampleStartIndex = getStartIndex(
                parts,
                hugoSymbolIndex,
                entrezGeneIdIndex,
                rppaGeneRefIndex,
            ...
            if (isRppaProfile) {
                if (rppaGeneRefIndex == -1) {
                    throw new RuntimeException(
                        "Error: the following column should be present for RPPA data: Composite.Element.Ref"
                    );
                }
            } else if (isGsvaProfile) {
                if (genesetIdIndex == -1) {
                    throw new RuntimeException(
                        "Error: the following column should be present for gene set score data: geneset_id"
                    );
                }
            } else if (isGenericAssayProfile) {
                if (genericAssayIdIndex == -1) {
                    throw new RuntimeException(
                        "Error: the following column should be present for this type of data: ENTITY_STABLE_ID"
                    );
                }
            } else if (hugoSymbolIndex == -1 && entrezGeneIdIndex == -1) {
                throw new RuntimeException(
                    "Error: at least one of the following columns should be present: Hugo_Symbol or Entrez_Gene_Id"
                );
            }
            ...
            ProgressMonitor.setCurrentMessage(
                " --> total number of samples: " + sampleIds.length
            );
            ...
            if (nrUnknownSamplesAdded > 0) {
                ProgressMonitor.logWarning(
                    "WARNING: Number of samples added on the fly because they were missing in clinical data:  " +
                    nrUnknownSamplesAdded
                );
            }
            if (samplesSkipped > 0) {
                ProgressMonitor.setCurrentMessage(
            ...
            if (isDiscretizedCnaProfile) {
                existingCnaEvents =
                    new HashMap<CnaEvent.Event, CnaEvent.Event>();
                ...
                if (isGsvaProfile) {
                    recordAdded =
                        parseGenesetLine(
                            ...
                            sampleStartIndex,
                            genesetIdIndex,
                            filteredSampleIndices,
                            daoGeneticAlteration
                        );
                } else if (isGenericAssayProfile) {
                    recordAdded =
                        parseGenericAssayLine(
                            line,
                            lenParts,
                            sampleStartIndex,
                            genericAssayIdIndex,
            ...
            if (isRppaProfile) {
                ProgressMonitor.setCurrentMessage(
                    " --> total number of extra records added because of multiple genes in one line:  " +
                    nrExtraRecords
                );
            }
            if (entriesSkipped > 0) {
            ...
            if (numRecordsToAdd == 0) {
                throw new DaoException(
                    "Something has gone wrong!  I did not save any records" +
                    " to the database!"
                );
            }
        } finally {
    ...
    private boolean parseLine(
        ...
        int hugoSymbolIndex,
        int entrezGeneIdIndex,
        int rppaGeneRefIndex,
        boolean isRppaProfile,
        boolean isDiscretizedCnaProfile,
        DaoGeneOptimized daoGene,
        List<Integer> filteredSampleIndices,
        ...
        if (!line.startsWith("#") && line.trim().length() > 0) {
            ...
            if (parts.length > nrColumns) {
                if (line.split("\t").length > nrColumns) {
                    ProgressMonitor.logWarning(
                        "Ignoring line with more fields (" +
                        parts.length +
                        ") than specified in the headers(" +
                        nrColumns +
                        "): \n" +
                        parts[0]
                    );
            ...
            String values[] = (String[]) ArrayUtils.subarray(
                parts,
                sampleStartIndex,
            ...
            if (isRppaProfile && geneSymbol == null) {
                ProgressMonitor.logWarning(
                    "Ignoring line with no Composite.Element.REF value"
                );
                return false;
            }
            //get entrez
            ...
            if (entrez != null) {
                if (entrez.isEmpty()) {
                    ...
                    ProgressMonitor.logWarning(
                        "Ignoring line with invalid Entrez_Id " + entrez
                    );
                    return false;
                }
            }
            ...
            if (geneSymbol == null && entrez == null) {
                ProgressMonitor.logWarning(
                    "Ignoring line with no Hugo_Symbol or Entrez_Id value"
                );
                return false;
            } else {
                if (
                    geneSymbol != null &&
                    (geneSymbol.contains("///") || geneSymbol.contains("---"))
                ) {
                    ...
                    if (geneSymbol != null) {
                        genesMatchingAnAlias =
                            daoGene.getGenesForAlias(geneSymbol);
                    }

                    Set<CanonicalGene> microRNAGenes = new HashSet<>();
                    Set<CanonicalGene> nonMicroRNAGenes = new HashSet<>();
                    Iterator<CanonicalGene> geneIterator = Stream
                        .concat(genes.stream(), genesMatchingAnAlias.stream())
                        .iterator();
                    ...
                    if (!microRNAGenes.isEmpty()) {
                        ...
                        for (CanonicalGene gene : microRNAGenes) {
                            boolean result = storeGeneticAlterations(
                                values,
                                daoGeneticAlteration,
                                gene,
                                geneSymbol
                        ...
                        if (!recordStored) {
                            if (nonMicroRNAGenes.isEmpty()) {
                                ...
                                ProgressMonitor.logWarning(
                                    "Gene symbol " +
                                    geneSymbol +
                                    " found to be ambiguous (a mixture of microRNA and other types). Record will be skipped for this gene."
                                );
                            }
                            return false;
                        }
                        ...
                        if (genes.size() == 1) {
                            ...
                            if (isDiscretizedCnaProfile) {
                                ...
                                for (int i = 0; i < values.length; i++) {
                                    ...
                                    if (
                                        values[i].equals(
                                                CNA_VALUE_PARTIAL_DELETION
                                            )
                                    ) {
                                        values[i] =
                                            CNA_VALUE_HOMOZYGOUS_DELETION;
                                    }
                                    if (
                                        values[i].equals(
                                                CNA_VALUE_AMPLIFICATION
                                            ) ||
                            ...
                            if (recordStored) {
                                for (CnaEvent cnaEvent : cnaEventsToAdd) {
                                    if (
                                        existingCnaEvents.containsKey(
                                            cnaEvent.getEvent()
                                        )
                                    ) {
                                        cnaEvent.setEventId(
                                            existingCnaEvents
                                                .get(cnaEvent.getEvent())
                                                .getEventId()
                                        );
                                        DaoCnaEvent.addCaseCnaEvent(
                                            cnaEvent,
                                            false
                                        );
                                    } else {
     ...
     * @throws DaoException
     */
    private boolean parseGenesetLine(
        String line,
        int nrColumns,
        int sampleStartIndex,
        int genesetIdIndex,
        ...
        if (!line.startsWith("#") && line.trim().length() > 0) {
            ...
            Geneset geneset = DaoGeneset.getGenesetByExternalId(
                parts[genesetIdIndex]
            );
            if (geneset != null) {
                storedRecord =
                    storeGeneticEntityGeneticAlterations(
                        values,
                        daoGeneticAlteration,
                        geneset.getGeneticEntityId(),
                        EntityType.GENESET,
                ...
                ProgressMonitor.logWarning(
                    "Geneset " +
                    parts[genesetIdIndex] +
                    " not found in DB. Record will be skipped."
                );
            }
        }
        return storedRecord;
    ...
    private boolean parseGenericAssayLine(
        String line,
        int nrColumns,
        int sampleStartIndex,
        int genericAssayIdIndex,
        List<Integer> filteredSampleIndices,
        DaoGeneticAlteration daoGeneticAlteration
    )
        throws DaoException {
        ...
        if (!line.startsWith("#") && line.trim().length() > 0) {
            ...
            GenericAssayMeta genericAssayMeta = DaoGenericAssay.getGenericAssayMetaByStableId(
                parts[genericAssayIdIndex]
            );
            ...
            if (genericAssayMeta == null) {
                ...
                GeneticEntity geneticEntity = DaoGeneticEntity.getGeneticEntityByStableId(
    ...
    private boolean storeGeneticAlterations(
        String[] values,
        DaoGeneticAlteration daoGeneticAlteration,
        CanonicalGene gene,
        String geneSymbol
        ...
        try {
            if (!importSetOfGenes.contains(gene.getEntrezGeneId())) {
                daoGeneticAlteration.addGeneticAlterations(
                    geneticProfileId,
                ...
                if (
                    geneSymbol != null &&
                    !geneSymbol.equalsIgnoreCase(
                        gene.getHugoGeneSymbolAllCaps()
                    )
                ) geneSymbolMessage =
                    " (given as alias in your file as: " + geneSymbol + ")";
                ProgressMonitor.logWarning(
                    "Gene " +
                    gene.getHugoGeneSymbolAllCaps() +
                    " (" +
                    ...
                    ")" +
                    geneSymbolMessage +
                    " found to be duplicated in your file. Duplicated row will be ignored!"
                );
                return false;
            }
        } catch (Exception e) {
            throw new RuntimeException(
                "Aborted: Error found for row starting with " +
                geneSymbol +
                ": " +
                e.getMessage()
            );
     ...
     */
    private boolean storeGeneticEntityGeneticAlterations(
        String[] values,
        DaoGeneticAlteration daoGeneticAlteration,
        Integer geneticEntityId,
        EntityType geneticEntityType,
        String geneticEntityName
    ) {
        try {
            if (importedGeneticEntitySet.add(geneticEntityId)) {
                daoGeneticAlteration.addGeneticAlterationsForGeneticEntity(
                ...
                ProgressMonitor.logWarning(
                    "Data for genetic entity " +
                    geneticEntityName +
                    " [" +
                    geneticEntityType +
                    "] already imported from file. Record will be skipped."
                );
                return false;
            }
        } catch (Exception ex) {
     ...
     * @throws DaoException
     */
    private List<CanonicalGene> parseRPPAGenes(String antibodyWithGene)
        throws DaoException {
        DaoGeneOptimized daoGene = DaoGeneOptimized.getInstance();
        ...
        if (parts.length < 2) {
            ProgressMonitor.logWarning(
                "Could not parse Composite.Element.Ref value " +
                antibodyWithGene +
        ...
        if (!arrayIdSet.add(arrayId)) {
            ProgressMonitor.logWarning(
                ...
                " in [" +
                antibodyWithGene +
                "] found to be duplicated. Record will be skipped."
            );
            return null;
        }
        ...
        for (String symbol : symbols) {
            if (symbol.equalsIgnoreCase("NA")) {
                ...
                ProgressMonitor.logWarning(
                    "Gene " +
                    symbol +
                    " will be interpreted as 'Not Available' in this case. Record will be skipped for this gene."
                );
            } else {
        ...
        for (String symbol : symbolsNotFound) {
            ProgressMonitor.logWarning(
                "Gene " +
                symbol +
                " not found in DB. Record will be skipped for this gene."
            );
        }
    ...
    private List<CanonicalGene> importPhosphoGene(
        ...
        for (CanonicalGene gene : genes) {
            ...
            if (phosphoGene == null) {
                ProgressMonitor.logWarning(
                    "Phosphoprotein " +
                    phosphoSymbol +
                    " not yet known in DB. Adding it to `gene` table with 3 aliases in `gene_alias` table."
                );
    ...
    private int getStartIndex(String[] headers, int... featureColIds) {
        ...
        if (
            ...
            genericEntityProperties.trim().length() != 0
        ) {
        ...
        for (int i = 0; i < headers.length; i++) {
            ...
            if (
                featureColNames
                    .stream()
                    .noneMatch(e -> e.equalsIgnoreCase(h)) &&
                i > lastFeatureCol
            ) {
        ...
        if (startIndex == -1) throw new RuntimeException(
            "Could not find a sample column in the file"
        );

        return startIndex;
    }

    private String[] filterOutNormalValues(
        List<Integer> filteredSampleIndices,
        String[] values
    ) {

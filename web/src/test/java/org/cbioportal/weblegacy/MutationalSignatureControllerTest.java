...
@ContextConfiguration(
    classes = {
        MutationalSignatureControllerConfig.class,
...
public class MutationalSignatureControllerTest {
    ...
    public void setup() {
        ...
        signature1 =
            new MutationalSignature(
                CANONICAL_SNP_TYPES,
                "sample1",
                new int[] { 40, 0, 0, 0, 0, 1 }
            );
        signature2 =
            new MutationalSignature(
                CANONICAL_SNP_TYPES,
                "sample2",
                new int[] { 1, 2, 99, 6, 4, 1 }
            );
    }
    ...
    public void mutationalSignaturesBySampleIdTest() throws Exception {
        ...
        Mockito
            .when(
                mutationalSignatureServiceMock.getMutationalSignaturesBySampleIds(
        ...
        this.mockMvc.perform(
            ...
            .andExpect(
                MockMvcResultMatchers.jsonPath("$[0].sample").value("sample1")
            )
            .andExpect(
                MockMvcResultMatchers.jsonPath("$[0].counts[0]").value(40)
            )
            .andExpect(
                MockMvcResultMatchers.jsonPath("$[0].counts[1]").value(0)
            )
            .andExpect(
                MockMvcResultMatchers.jsonPath("$[0].counts[2]").value(0)
            )
            .andExpect(
                MockMvcResultMatchers.jsonPath("$[0].counts[3]").value(0)
            )
            .andExpect(
                MockMvcResultMatchers.jsonPath("$[0].counts[4]").value(0)
            )
            .andExpect(
                MockMvcResultMatchers.jsonPath("$[0].counts[5]").value(1)
            )
            .andExpect(
    ...
    public void mutationalSignaturesByStudyTest() throws Exception {
        ...
        Mockito
            .when(
                mutationalSignatureServiceMock.getMutationalSignatures(
                    org.mockito.Matchers.anyString()
                )
            )
            .thenReturn(mockResponse);
        ...
        this.mockMvc.perform(
            ...
            .andExpect(
                MockMvcResultMatchers.jsonPath("$[1].counts[0]").value(1)
            )
            .andExpect(
                MockMvcResultMatchers.jsonPath("$[1].counts[1]").value(2)
            )
            .andExpect(
                MockMvcResultMatchers.jsonPath("$[1].counts[2]").value(99)
            )
            .andExpect(
                MockMvcResultMatchers.jsonPath("$[1].counts[3]").value(6)
            )
            .andExpect(
                MockMvcResultMatchers.jsonPath("$[1].counts[4]").value(4)
            )
            .andExpect(
                MockMvcResultMatchers.jsonPath("$[1].counts[5]").value(1)
            )
            .andExpect(
                MockMvcResultMatchers.jsonPath(
                    "$[1].mutationTypes",

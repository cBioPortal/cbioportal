...
@Import({MyBatisLegacyConfig.class, GenesetHierarchyMyBatisRepository.class})
@DataJpaTest
@DirtiesContext
...
public class GenesetHierarchyMyBatisRepositoryTest {
  ...
  @Autowired private GenesetHierarchyMyBatisRepository genesetHierarchyMyBatisRepository;

  /**
   * Test data is as follows: Root node -> Sub node A -> Parent node 1 -> MORF_ATRX " " " ->
   * HINATA_NFKB_MATRIX " " -> Parent node 2 -> HINATA_NFKB_MATRIX Root node -> Sub node B -> x
   * (dead branch)
   */
  @Test
  ...
  public void getGenesetHierarchyParents() {
    ...
    result =
        genesetHierarchyMyBatisRepository.getGenesetHierarchyParents(Arrays.asList("MORF_ATRX"));
  ...
  public void getGenesetHierarchyGenesets() {
    ...
    List<Geneset> result =
        genesetHierarchyMyBatisRepository.getGenesetHierarchyGenesets(nodes.get(0).getNodeId());
    ...
    result =
        genesetHierarchyMyBatisRepository.getGenesetHierarchyGenesets(nodes.get(1).getNodeId());

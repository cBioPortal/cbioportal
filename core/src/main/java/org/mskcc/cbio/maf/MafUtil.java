...
public class MafUtil {
    private static final Pattern validNucleotidesPattern = Pattern.compile(
        "^([ATGC]*)$"
    );
    ...
    public static final String VARIANT_CLASSIFICATION =
    ...
    public static final String MATCHED_NORM_SAMPLE_BARCODE =
        "Matched_Norm_Sample_Barcode";
    public static final String MATCH_NORM_SEQ_ALLELE1 =
        "Match_Norm_Seq_Allele1";
    public static final String MATCH_NORM_SEQ_ALLELE2 =
        "Match_Norm_Seq_Allele2";
    public static final String TUMOR_VALIDATION_ALLELE1 =
        "Tumor_Validation_Allele1";
    public static final String TUMOR_VALIDATION_ALLELE2 =
        "Tumor_Validation_Allele2";
    public static final String MATCH_NORM_VALIDATION_ALLELE1 =
        "Match_Norm_Validation_Allele1";
    public static final String MATCH_NORM_VALIDATION_ALLELE2 =
    ...
    public static final String ONCOTATOR_VARIANT_CLASSIFICATION =
        "ONCOTATOR_VARIANT_CLASSIFICATION";
    public static final String ONCOTATOR_REFSEQ_MRNA_ID =
        "ONCOTATOR_REFSEQ_MRNA_ID";
    public static final String ONCOTATOR_UNIPROT_ENTRY_NAME =
        "ONCOTATOR_UNIPROT_ENTRY_NAME";
    public static final String ONCOTATOR_UNIPROT_ACCESSION =
        "ONCOTATOR_UNIPROT_ACCESSION";
    public static final String ONCOTATOR_CODON_CHANGE =
        "ONCOTATOR_CODON_CHANGE";
    public static final String ONCOTATOR_PROTEIN_POS_START =
        "ONCOTATOR_PROTEIN_POS_START";
    public static final String ONCOTATOR_PROTEIN_POS_END =
    ...
    public static final String DRIVER_FILTER_ANNOTATION =
        "cbp_driver_annotation";
    public static final String DRIVER_TIERS_FILTER = "cbp_driver_tiers";
    public static final String DRIVER_TIERS_FILTER_ANNOTATION =
    ...
    public MafUtil(String headerLine, Set<String> namespaces) {
        ...
        for (int i = 0; i < parts.length; i++) {
            ...
            if (header.equalsIgnoreCase(CHROMOSOME)) {
                ...
                header.equalsIgnoreCase(ONCOTATOR_VARIANT_CLASSIFICATION)
            ) {
                ...
                header.equalsIgnoreCase(DRIVER_TIERS_FILTER_ANNOTATION)
            ) {
                ...
                for (String ns : namespaces) {
                    if (
                        header
                            .toLowerCase()
                            .startsWith(ns + NAMESPACE_DELIMITER)
                    ) {
                        String nsKey = header
                            .toLowerCase()
                            .replace(ns + NAMESPACE_DELIMITER, "");
                        Map<String, Integer> nsKeyIndexMap =
                            this.namespaceIndexMap.getOrDefault(
                                    ns,
    ...
    public MafRecord parseRecord(String line) {
        ...
        // standard MAF cols
        record.setCenter(
            TabDelimitedFileUtil.getPartString(centerIndex, parts)
        );
        record.setChr(TabDelimitedFileUtil.getPartString(chrIndex, parts));
        record.setStartPosition(
            TabDelimitedFileUtil.getPartLong(startPositionIndex, parts)
        );
        record.setEndPosition(
            TabDelimitedFileUtil.getPartLong(endPositionIndex, parts)
        );
        // store the literal value of the gene ID column for later parsing
        record.setGivenEntrezGeneId(
        ...
        record.setNcbiBuild(
            TabDelimitedFileUtil.getPartString(ncbiIndex, parts)
        );
        record.setReferenceAllele(
            TabDelimitedFileUtil.getPartString(referenceAlleleIndex, parts)
        );
        record.setStrand(
            TabDelimitedFileUtil.getPartString(strandIndex, parts)
        );
        record.setDbSNP_RS(
            TabDelimitedFileUtil.getPartString(dbSNPIndex, parts)
        );
        record.setTumorSampleID(
            TabDelimitedFileUtil.getPartString(tumorSampleIndex, parts)
        );
        record.setTumorSeqAllele1(
            TabDelimitedFileUtil.getPartString(tumorSeqAllele1Index, parts)
        );
        record.setTumorSeqAllele2(
            TabDelimitedFileUtil.getPartString(tumorSeqAllele2Index, parts)
        );
        record.setVariantClassification(
            TabDelimitedFileUtil.getPartString(
                variantClassificationIndex,
                parts
            )
        );
        record.setVariantType(
            TabDelimitedFileUtil.getPartString(variantTypeIndex, parts)
        );
        record.setMutationStatus(
            TabDelimitedFileUtil.getPartString(mutationStatusIndex, parts)
        );
        record.setValidationStatus(
            TabDelimitedFileUtil.getPartString(validationStatusIndex, parts)
        );
        record.setSequencer(
            TabDelimitedFileUtil.getPartString(sequencerIndex, parts)
        );
        record.setDbSnpValStatus(
            TabDelimitedFileUtil.getPartString(dbSnpValStatusIndex, parts)
        );
        record.setMatchedNormSampleBarcode(
            TabDelimitedFileUtil.getPartString(
                matchedNormSampleBarcodeIndex,
                parts
            )
        );
        record.setMatchNormSeqAllele1(
            TabDelimitedFileUtil.getPartString(matchNormSeqAllele1Index, parts)
        );
        record.setMatchNormSeqAllele2(
            TabDelimitedFileUtil.getPartString(matchNormSeqAllele2Index, parts)
        );
        record.setTumorValidationAllele1(
            TabDelimitedFileUtil.getPartString(
                tumorValidationAllele1Index,
                parts
            )
        );
        record.setTumorValidationAllele2(
            TabDelimitedFileUtil.getPartString(
                tumorValidationAllele2Index,
                parts
            )
        );
        record.setMatchNormValidationAllele1(
            TabDelimitedFileUtil.getPartString(
                matchNormValidationAllele1Index,
                parts
            )
        );
        record.setMatchNormValidationAllele2(
            TabDelimitedFileUtil.getPartString(
                matchNormValidationAllele2Index,
                parts
            )
        );
        record.setVerificationStatus(
            TabDelimitedFileUtil.getPartString(verificationStatusIndex, parts)
        );
        record.setSequencingPhase(
            TabDelimitedFileUtil.getPartString(sequencingPhaseIndex, parts)
        );
        record.setSequenceSource(
            TabDelimitedFileUtil.getPartString(sequenceSourceIndex, parts)
        );
        record.setValidationMethod(
            TabDelimitedFileUtil.getPartString(validationMethodIndex, parts)
        );
        record.setScore(TabDelimitedFileUtil.getPartString(scoreIndex, parts));
        record.setBamFile(
            TabDelimitedFileUtil.getPartString(bamFileIndex, parts)
        );

        record.setAminoAcidChange(
            TabDelimitedFileUtil
                .getPartString(aminoAcidChangeIndex, parts)
                .trim()
        );

        // allele frequency (count) columns
        record.setTumorAltCount(
            TabDelimitedFileUtil.getPartInt(tumorAltCountIndex, parts)
        );
        record.setTumorRefCount(
            TabDelimitedFileUtil.getPartInt(tumorRefCountIndex, parts)
        );
        record.setNormalAltCount(
            TabDelimitedFileUtil.getPartInt(normalAltCountIndex, parts)
        );
        record.setNormalRefCount(
            TabDelimitedFileUtil.getPartInt(normalRefCountIndex, parts)
        );
        ...
        record.setTumorDepth(
            TabDelimitedFileUtil.getPartInt(tumorDepthIndex, parts)
        );
        record.setTumorVaf(
            TabDelimitedFileUtil.getPartPercentage(tumorVafIndex, parts)
        );
        record.setNormalDepth(
            TabDelimitedFileUtil.getPartInt(normalDepthIndex, parts)
        );
        record.setNormalVaf(
            TabDelimitedFileUtil.getPartPercentage(normalVafIndex, parts)
        );

        // custom annotator columns
        record.setProteinChange(
            TabDelimitedFileUtil
                .getPartString(getColumnIndex(PROTEIN_CHANGE), parts)
                .trim()
        );
        record.setProteinPosition(
            TabDelimitedFileUtil.getPartString(
                getColumnIndex(PROTEIN_POSITION),
                parts
            )
        );
        record.setCodons(
            TabDelimitedFileUtil.getPartString(getColumnIndex(CODONS), parts)
        );
        record.setSwissprot(
            TabDelimitedFileUtil.getPartString(getColumnIndex(SWISSPROT), parts)
        );
        record.setRefSeq(
            TabDelimitedFileUtil.getPartString(getColumnIndex(REFSEQ), parts)
        );

        // Mutation Assessor columns
        record.setMaFuncImpact(
            TabDelimitedFileUtil.getPartString(maFImpactIndex, parts)
        );
        ...
        record.setMaLinkVar(
            TabDelimitedFileUtil.getPartString(maLinkVarIndex, parts)
        );
        record.setMaLinkMsa(
            TabDelimitedFileUtil.getPartString(maLinkMsaIndex, parts)
        );
        record.setMaLinkPdb(
            TabDelimitedFileUtil.getPartString(maLinkPdbIndex, parts)
        );

        // Oncotator columns
        record.setOncotatorDbSnpRs(
            TabDelimitedFileUtil.getPartString(oncoDbSnpRsIndex, parts)
        );
        record.setOncotatorVariantClassification(
            TabDelimitedFileUtil.getPartString(
                oncoVariantClassificationIndex,
                parts
            )
        );
        record.setOncotatorRefseqMrnaId(
            TabDelimitedFileUtil.getPartString(oncoRefseqMrnaIdIndex, parts)
        );
        record.setOncotatorUniprotName(
            TabDelimitedFileUtil.getPartString(oncoUniprotNameIndex, parts)
        );
        record.setOncotatorUniprotAccession(
            TabDelimitedFileUtil.getPartString(oncoUniprotAccessionIndex, parts)
        );
        record.setOncotatorCodonChange(
            TabDelimitedFileUtil.getPartString(oncoCodonChangeIndex, parts)
        );
        record.setOncotatorProteinPosStart(
            TabDelimitedFileUtil.getPartInt(oncoProteinPosStartIndex, parts)
        );
        record.setOncotatorProteinPosEnd(
            TabDelimitedFileUtil.getPartInt(oncoProteinPosEndIndex, parts)
        );

        // custom filtering of passenger and driver mutations columns
        record.setDriverFilter(
            TabDelimitedFileUtil.getPartStringAllowEmptyAndNA(
                driverIndex,
                parts
            )
        );
        record.setDriverFilterAnn(
            TabDelimitedFileUtil.getPartStringAllowEmpty(driverAnnIndex, parts)
        );
        record.setDriverTiersFilter(
            TabDelimitedFileUtil.getPartStringAllowEmptyAndNA(
                driverTiersIndex,
                parts
            )
        );
        record.setDriverTiersFilterAnn(
            TabDelimitedFileUtil.getPartStringAllowEmpty(
                driverTiersAnnIndex,
                parts
            )
        ...
        if (!namespaceIndexMap.isEmpty()) {
            for (Map.Entry<String, Map<String, Integer>> nsKeyIndexMap : namespaceIndexMap.entrySet()) {
                ...
                for (Map.Entry<String, Integer> nsKeyIndexPairs : nsKeyIndexMap
                    .getValue()
                    .entrySet()) {
                    String keyName = nsKeyIndexPairs.getKey();
                    Integer keyIndex = nsKeyIndexPairs.getValue();
                    namespaceKeyValueMappings.put(
                        keyName,
                        TabDelimitedFileUtil.getPartStringAllowEmptyAndNA(
                            keyIndex,
                            parts
                ...
                recordNamespaceAnnotationJsonMap.put(
                    namespace,
                    namespaceKeyValueMappings
                );
            }
     ...
     */
    public static String resolveTumorSeqAllele(
        ...
        if (
            (
                Strings.isNullOrEmpty(tumorSeqAllele1) ||
                tumorSeqAllele1.equalsIgnoreCase("NA")
            ) &&
            (
                Strings.isNullOrEmpty(tumorSeqAllele2) ||
                tumorSeqAllele2.equalsIgnoreCase("NA")
            )
        ) {
        ...
        if (
            Strings.isNullOrEmpty(tumorSeqAllele1) ||
            tumorSeqAllele1.equals("NA") ||
            tumorSeqAllele1.equals(referenceAllele)
        ) {
            return tumorSeqAllele2;
        } else if (
            Strings.isNullOrEmpty(tumorSeqAllele2) ||
            tumorSeqAllele2.equals("NA") ||
            tumorSeqAllele2.equals(referenceAllele)
        ) {
            return tumorSeqAllele1;
        } else if (
            variantContainsAmbiguousTumorSeqAllele(
                referenceAllele,
                tumorSeqAllele1,
                tumorSeqAllele2
            )
        ) {
            return tumorSeqAllele2.equals("-")
                ? tumorSeqAllele1
                : tumorSeqAllele2;
        } else {
            return tumorSeqAllele1;
     ...
     * @author angelicaochoa
     */
    public static boolean variantContainsAmbiguousTumorSeqAllele(
        String referenceAllele,
        String tumorSeqAllele1,
        String tumorSeqAllele2
    ) {
        ...
        return (
            (tumorSeqAllele1.equals("-") || tumorSeqAllele2.equals("-")) &&
            (
                validNucleotidesPattern
                    .matcher(tumorSeqAllele1.toUpperCase())
                    .matches() ||
                validNucleotidesPattern
                    .matcher(tumorSeqAllele2.toUpperCase())
                    .matches()
            )
        );
    }

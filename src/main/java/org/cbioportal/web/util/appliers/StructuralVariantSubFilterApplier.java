...
public class StructuralVariantSubFilterApplier implements StudyViewSubFilterApplier {
  ...
  public List<SampleIdentifier> filter(List<SampleIdentifier> toFilter, StudyViewFilter filters) {
    ...
    List<String> includedStudyIds =
        toFilter.stream().map(SampleIdentifier::getStudyId).distinct().collect(Collectors.toList());

    List<MolecularProfile> molecularProfiles =
        molecularProfileService.getMolecularProfilesInStudies(includedStudyIds, "SUMMARY");
    ...
    for (StudyViewStructuralVariantFilter structuralVariantFilter : structVarFilters) {
      ...
      List<MolecularProfile> filteredMolecularProfiles =
          structuralVariantFilter.getMolecularProfileIds().stream()
      ...
      for (List<StructuralVariantFilterQuery> structVarQueries :
          structuralVariantFilter.getStructVarQueries()) {

        // Remove samples in remainingSampleIdentifiers that belong to a study that is not
        // accessible to the current user
        // (do not appear in the molecularProfileMap argument).
        final List<SampleIdentifier> filteredSampleIdentifiers =
            remainingSampleIdentifiers.stream()
                .filter(i -> mapByStudyId.containsKey(i.getStudyId()))
                .collect(Collectors.toList());
        ...
        final List<StructuralVariantFilterQuery> entrezIdEnhancedSvQueries =
            studyViewFilterUtil.resolveEntrezGeneIds(structVarQueries);
        remainingSampleIdentifiers =
            structuralVariantService
                .fetchStructuralVariantsByStructVarQueries(
                    molecularProfileIds, sampleIds, entrezIdEnhancedSvQueries)
                .stream()
                .map(
                    m -> {
  ...
  private static List<StudyViewStructuralVariantFilter> getStructVarFilters(
      StudyViewFilter filters) {
    final List<StudyViewStructuralVariantFilter> structuralVariantFilters =
        filters.getStructuralVariantFilters();
    if (structuralVariantFilters == null || structuralVariantFilters.isEmpty()) {
      return new ArrayList<>();
    }
    final List<StudyViewStructuralVariantFilter> structVarFilters =
        structuralVariantFilters.stream()
            .filter(
                structuralVariantFilter -> !structuralVariantFilter.getStructVarQueries().isEmpty())
            .collect(Collectors.toList());

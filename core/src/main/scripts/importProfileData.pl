...
if( -e $loadTypeFile ){
	...
	system ("$JAVA_HOME/bin/java -Xmx1524M -cp $cp -DPORTAL_HOME='$portalHome' org.mskcc.cbio.cgds.scripts.ImportProfileData $args --loadMode $loadType");
	...
	my $cmd = "$JAVA_HOME/bin/java -Xmx1524M -cp $cp -DPORTAL_HOME='$portalHome' org.mskcc.cbio.cgds.scripts.ImportProfileData $args --loadMode bulkLoad";
	print "-DPORTAL_HOME='$portalHome' org.mskcc.cbio.cgds.scripts.ImportProfileData $args --loadMode bulkLoad\n";
	system( $cmd );
}

...
public class GetCoExpressionJSON extends HttpServlet {
    ...
    protected void doPost(
        ...
        boolean isFullResult = Boolean.parseBoolean(
            httpServletRequest.getParameter("is_full_result")
        );
        ...
        if (!isFullResult) {
            GeneticProfile final_gp = DaoGeneticProfile.getGeneticProfileByStableId(
                profileId
            );
            if (final_gp != null) {
                try {
                    Map<Long, double[]> map = CoExpUtil.getExpressionMap(
                        final_gp.getGeneticProfileId(),
                        caseSetId,
                        caseIdsKey
                    );
                    ...
                    for (int i = 0; i < mapSize; i++) {
                        ...
                        if (
                            compared_gene_exp != null && query_gene_exp != null
                        ) {
                            //Filter out cases with empty value on either side
                            int min_length = query_gene_exp.length <
                            ...
                            for (int k = 0; k < min_length; k++) {
                                if (
                                    !Double.isNaN(query_gene_exp[k]) &&
                                    !Double.isNaN(compared_gene_exp[k])
                                ) {
                                    new_query_gene_exp_arrlist.add(
                                        query_gene_exp[k]
                                    );
                                    new_compared_gene_exp_arrlist.add(
                                        compared_gene_exp[k]
                                    );
                                }
                            }
                            Double[] _new_query_gene_exp = new_query_gene_exp_arrlist.toArray(
                                new Double[0]
                            );
                            Double[] _new_compared_gene_exp = new_compared_gene_exp_arrlist.toArray(
                                new Double[0]
                            );
                            ...
                            for (
                                int m = 0;
                                m < _new_query_gene_exp.length;
                                m++
                            ) {
                                new_query_gene_exp[m] =
                                    _new_query_gene_exp[m].doubleValue();
                                new_compared_gene_exp[m] =
                                    _new_compared_gene_exp[m].doubleValue();
                            }

                            if (
                                new_query_gene_exp.length != 0 &&
                                new_compared_gene_exp.length != 0
                            ) {
                                ...
                                if (
                                    ...
                                    if (
                                        (
                                            spearman >= coExpScoreThreshold ||
                                            spearman <=
                                            (-1) *
                                            coExpScoreThreshold
                                        ) &&
                                        (
                                            (spearman > 0 && pearson > 0) ||
                                            (spearman < 0 && pearson < 0)
                                        )
                                    ) {
                                        CanonicalGene comparedGene = daoGeneOptimized.getGene(
                                            compared_gene_id
                                        );
            ...
            if (final_gp != null) {
                try {
                        ...
                        if (
                            ...
                            if (
                                ...
                                fullResutlStr.append(
                                    comparedGene.getHugoGeneSymbolAllCaps() +
                                    "\t" +
                                    (double) Math.round(pearson * 100) /
                                    100 +
                                    "\t" +
                                    (double) Math.round(spearman * 100) /
                                    100 +
                                    "\n"
                                );
                            }
                    ...
                    String fileName =
                        ...
                        geneSymbol +
                        "_" +
                        final_gp.getProfileName().replaceAll("\\s+", "_") +
                        "_" +
                        cancerStudyIdentifier.replaceAll("\\s+", "_") +
                        ".txt";

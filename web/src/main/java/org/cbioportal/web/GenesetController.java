...
public class GenesetController {
    ...
    public ResponseEntity<List<Geneset>> getAllGenesets(
        ...
        @ApiParam("Page size of the result list") @Max(Integer.MAX_VALUE) @Min(
    ...
    public ResponseEntity<Geneset> getGeneset(
        @ApiParam(
            required = true,
            value = "Gene set ID e.g. GNF2_ZAP70"
        ) @PathVariable String genesetId
    )
        throws GenesetNotFoundException {
        return new ResponseEntity<>(
            genesetService.getGeneset(genesetId),
            HttpStatus.OK
        );
    }

    @RequestMapping(
        value = "/genesets/fetch",
    ...
    public ResponseEntity<List<Geneset>> fetchGenesets(
        @ApiParam(required = true, value = "List of Gene set IDs") @Size(
            min = 1,
    ...
    public ResponseEntity<String> getGenesetVersion() {
        return new ResponseEntity<>(
            genesetService.getGenesetVersion(),
            HttpStatus.OK
        );
    }
}

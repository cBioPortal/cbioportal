...
def fetch_oncokb_mutation_annotations(mutation_data):
    request_url = "https://demo.oncokb.org/api/v1/annotate/mutations/byProteinChange"
    request_headers = {'Content-Type': 'application/json', 'Accept': 'application/json'}
    request_payload = create_mutation_request_payload(mutation_data)
...
def create_mutation_request_payload(mutation_data):
    elements = {}
...
def fetch_oncokb_copy_number_annotations(copy_number_data):
    request_payload = create_copy_number_request_payload(copy_number_data)
    #print(request_payload)
    return libImportOncokb.fetch_oncokb_annotations(request_payload, 10)
...
def create_copy_number_request_payload(copy_number_data):
    elements = {}
    for copy_number in copy_number_data:
        elements[copy_number["id"]] = '{"copyNameAlterationType":"'+ copy_number["alteration"]+'", "gene":{"entrezGeneId":'+str(copy_number["entrezGeneId"])+ \
            '}, "id":"'+copy_number["id"]+'", "tumorType":null}'
    
    # normalize for alteration id since same alteration is represented in multiple samples
    payload = list(elements.values()) #'[' + ', '.join(elements.values()) + ']'
    return payload
...
def main_import(study_id, properties_filename):    
    ...
    #mutation_result = fetch_oncokb_mutation_annotations(mutation_study_data)
    #print(mutation_result)
    
    # 4. Call oncokb to get annotations for the cna data retrieved
    cna_result = fetch_oncokb_copy_number_annotations(cna_study_data)
    print(cna_result)

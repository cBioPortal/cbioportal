...
public class SampleListServiceImpl implements SampleListService {
    ...
    public List<SampleList> getAllSampleLists(
        ...
        List<SampleList> sampleListsFromRepo = sampleListRepository.getAllSampleLists(
            projection,
            pageSize,
            pageNumber,
            sortBy,
        ...
        List<SampleList> sampleLists = (AUTHENTICATE.equals("false"))
            ? sampleListsFromRepo
    ...
    public SampleList getSampleList(String sampleListId)
        ...
        SampleList sampleList = sampleListRepository.getSampleList(
            sampleListId
        );
        if (sampleList == null) {
    ...
    public List<SampleList> getAllSampleListsInStudy(
        ...
        List<SampleList> sampleLists = sampleListRepository.getAllSampleListsInStudy(
            studyId,
            projection,
            pageSize,
            pageNumber,
    ...
    @Override
    public List<SampleList> fetchSampleLists(
        List<String> sampleListIds,
        String projection
    ) {
        List<SampleList> sampleLists = sampleListRepository.getSampleLists(
            sampleListIds,
            projection
        );
        ...
        sampleLists.forEach(
            s ->
                s.setSampleIds(
                    sampleListToSampleIds
                        .stream()
                        .filter(p -> p.getSampleListId().equals(s.getListId()))
                        .map(SampleListToSampleId::getSampleId)
                        .collect(Collectors.toList())
                )
        );

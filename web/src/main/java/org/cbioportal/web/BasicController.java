...
@ConditionalOnProperty(name = "authenticate", havingValue = "saml_plus_basic")
public class BasicController
{
	...
	private boolean checkIfSampleExists(String sampleID)
	{
		...
		if (!checkIfSampleExistsInStudy(studyID, sampleID)) {
			if (studyID.equals(ARCHER)) {
				...
				checkIfPatientExists(IMPACT, sampleID);
			} else {
				return false;
			} 
		}
	...
	{
		// tbd - create tech stack to get study id given sample id
		Matcher matcher = dmpSampleIDPattern.matcher(sampleID);
		if (matcher.find()) {

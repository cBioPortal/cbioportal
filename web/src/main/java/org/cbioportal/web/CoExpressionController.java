...
public class CoExpressionController {
    ...
    @PreAuthorize(
        "hasPermission(#molecularProfileIdA, 'MolecularProfileId', 'read') and hasPermission(#molecularProfileIdB, 'MolecularProfileId', 'read')"
    )
    @RequestMapping(
    ...
    public ResponseEntity<List<CoExpression>> fetchCoExpressions(
        @ApiParam(
            required = true,
            value = "Molecular Profile ID from the Genetic Entity referenced in the co-expression filter e.g. acc_tcga_rna_seq_v2_mrna"
        ) @RequestParam String molecularProfileIdA,
        @ApiParam(
            required = true,
            value = "Molecular Profile ID (can be the same as molecularProfileIdA) e.g. acc_tcga_rna_seq_v2_mrna"
        ) @RequestParam String molecularProfileIdB,
        @ApiParam(
            required = true,
            value = "List of Sample IDs/Sample List ID and Entrez Gene ID/Gene set ID"
        ) @Valid @RequestBody CoExpressionFilter coExpressionFilter,
        @ApiParam("Threshold") @RequestParam(
            defaultValue = "0.3"
        ) Double threshold
    )
        throws Exception {

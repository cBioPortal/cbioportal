 ...
 */
@Service
public final class SearchCancerStudiesMetadataUseCase {

    private final CancerStudyRepository studyRepository;


    /**
     * Constructs a new {@link SearchCancerStudiesMetadataUseCase} with the specified repository.
     *
     * @param studyRepository the repository used to access cancer study metadata.
     */
    public SearchCancerStudiesMetadataUseCase(CancerStudyRepository studyRepository) {
    ...
    public List<CancerStudyMetadata> execute(ProjectionType projectionType, SortAndSearchCriteria sortAndSearchCriteria) {
        return switch (projectionType) {
            case DETAILED -> studyRepository.getCancerStudyMetadata(sortAndSearchCriteria);

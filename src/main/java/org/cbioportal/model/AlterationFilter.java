...
public class AlterationFilter extends BaseAlterationFilter implements Serializable {
  ...
  @JsonIgnore private Select<MutationEventType> mutationTypeSelect;
  @JsonIgnore private Select<CNA> cnaTypeSelect;

  public AlterationFilter() {}

  public AlterationFilter(
      Select<MutationEventType> mutationTypesMap,
  ...
  public void setCopyNumberAlterationEventTypes(Map<CNA, Boolean> copyNumberAlterationEventTypes) {
    if (copyNumberAlterationEventTypes == null) {
      throw new IllegalArgumentException(
          "null value is not allowed for copyNumberAlterationEventTypes");
    }
  ...
  public Select<MutationEventType> getMutationTypeSelect() {
    if (mutationTypeSelect != null) return mutationTypeSelect;

    if (mutationEventTypes == null
        || mutationEventTypes.getOrDefault(MutationEventType.any, false)
        || allOptionsSelected(
            mutationEventTypes, Arrays.asList(MutationEventType.any.toString()))) {
      return Select.all();
    }
    ...
    if (mutationEventTypes.getOrDefault(MutationEventType.other, false)) {
      List<MutationEventType> unSelected =
          mutationEventTypes.entrySet().stream()
              .filter(e -> !e.getValue())
              .map(Entry::getKey)
      ...
      List<MutationEventType> selected =
          mutationEventTypes.entrySet().stream()
  ...
  public Select<CNA> getCNAEventTypeSelect() {
    if (cnaTypeSelect != null) return cnaTypeSelect;
    if (allOptionsSelected(copyNumberAlterationEventTypes, null)) {
      return Select.all();
    }
    return Select.byValues(
        copyNumberAlterationEventTypes.entrySet().stream()
            .filter(Entry::getValue)
            .map(Entry::getKey));
  }
  ...
  private boolean allOptionsSelected(Map<?, Boolean> options, List<String> excludeKeys) {
    return options.entrySet().stream()
        .allMatch(
            e -> {
              return excludeKeys == null || !excludeKeys.contains(e.getKey().toString())
                  ? e.getValue()
                  : true;
            });
  }

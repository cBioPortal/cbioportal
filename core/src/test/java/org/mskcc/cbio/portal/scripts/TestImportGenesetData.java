...
public class TestImportGenesetData {
    ...
    public void testImportGenesetData() throws Exception {
        ...
        File file = new File(
            "src/test/resources/genesets/unit-test1_genesets.gmt"
        );
        ...
        file =
            new File(
                "src/test/resources/genesets/unit-test1_supp-genesets.txt"
            );
        ...
        assertEquals(
            "http://www.broadinstitute.org/gsea/msigdb/cards/GCNP_SHH_UP_EARLY.V1_UP",
            geneset.getRefLink()
        );
        ...
        assertEquals(
            "Genes up-regulated in RK3E cells (kidney epithelium) over-expressing GLI1 [GeneID=2735].",
        ...
        updateInfo = true;
        skippedGenes =
            ImportGenesetData.importData(file, updateInfo, newVersion);

        // Open supplementary file
        file =
            new File(
                "src/test/resources/genesets/unit-test2_supp-genesets.txt"
            );
        ImportGenesetData.importSuppGenesetData(file);

        geneset = DaoGeneset.getGenesetByExternalId("UNITTEST_GENESET2");
        assertEquals(
            "A made up description is suited for this a fake gene.",
            geneset.getDescription()
        );
        geneset = DaoGeneset.getGenesetByExternalId("UNITTEST_GENESET1");
        assertEquals(
            "Thought of new nice name for this geneset",
            geneset.getName()
        );
        geneset = DaoGeneset.getGenesetByExternalId("UNITTEST_GENESET1");

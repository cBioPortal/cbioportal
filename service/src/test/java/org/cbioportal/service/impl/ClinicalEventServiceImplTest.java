...
public class ClinicalEventServiceImplTest extends BaseServiceImplTest {
    ...
    public void getAllClinicalEventsOfPatientInStudy() throws Exception {
        ...
        Mockito
            .when(
                clinicalEventRepository.getDataOfClinicalEvents(
                    Arrays.asList(CLINICAL_EVENT_ID)
                )
            )
            .thenReturn(expectedClinicalEventDataList);

        List<ClinicalEvent> result = clinicalEventService.getAllClinicalEventsOfPatientInStudy(
            STUDY_ID,
            PATIENT_ID_1,
            PROJECTION,
            PAGE_SIZE,
        ...
        Assert.assertEquals(
            clinicalEventData,
            result.get(0).getAttributes().get(0)
        );
    }

    @Test(expected = PatientNotFoundException.class)
    public void getAllClinicalEventsOfPatientInStudyPatientNotFound()
        throws Exception {
        Mockito
    ...
    public void getMetaPatientClinicalEvents() throws Exception {
        ...
        Mockito
            .when(
                clinicalEventRepository.getMetaPatientClinicalEvents(
                    STUDY_ID,
                    PATIENT_ID_1
                )
            )
            .thenReturn(expectedBaseMeta);
        BaseMeta result = clinicalEventService.getMetaPatientClinicalEvents(
            STUDY_ID,
            PATIENT_ID_1
        );
    ...
    public void getAllClinicalEventsInStudy() throws Exception {
        ...
        List<ClinicalEvent> result = clinicalEventService.getAllClinicalEventsInStudy(
            STUDY_ID,
            PROJECTION,
            PAGE_SIZE,
            PAGE_NUMBER,

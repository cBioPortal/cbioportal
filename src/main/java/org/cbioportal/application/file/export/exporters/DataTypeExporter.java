 ...
 * @param <D> - a data type
 */
public abstract class DataTypeExporter<
        M extends GeneticDatatypeMetadata,
        D extends Iterator<SequencedMap<String, String>> & Closeable>
    implements Exporter {
  ...
  public boolean exportData(FileWriterFactory fileWriterFactory, ExportDetails exportDetails) {
    Optional<M> metadataOptional =
        getMetadata(exportDetails.getStudyId(), exportDetails.getSampleIds());
    if (metadataOptional.isEmpty()) {
      LOG.debug(
          "No metadata found for study {} by {} exporter. Skipping export of this datatype.",
    ...
    if (!metadata.getCancerStudyIdentifier().equals(exportDetails.getStudyId())) {
      throw new IllegalStateException(
          "Metadata study ID ("
              ...
              + ") does not match the provided study ID ("

...
public class MrnaPercentileControllerTest {
    ...
    private static final String TEST_SAMPLE_STABLE_ID =
    ...
    public void fetchMrnaPercentile() throws Exception {
        ...
        Mockito
            .when(
                mrnaPercentileService.fetchMrnaPercentile(
                    Mockito.anyString(),
                    Mockito.anyString(),
                    Mockito.anyListOf(Integer.class)
                )
            )
            .thenReturn(mrnaPercentileList);
        ...
        mockMvc
            .perform(
                MockMvcRequestBuilders
                    .post(
                        "/molecular-profiles/test_molecular_profile_id/mrna-percentile/fetch"
                    )
            ...
            .andExpect(
                MockMvcResultMatchers
                    .jsonPath("$[0].sampleId")
                    .value(TEST_SAMPLE_STABLE_ID)
            )
            .andExpect(
                MockMvcResultMatchers
                    .jsonPath("$[0].zScore")
                    .value(TEST_Z_SCORE_1)
            )
            .andExpect(
                MockMvcResultMatchers
                    .jsonPath("$[0].percentile")
                    .value(TEST_PERCENTILE_1)
            )
            .andExpect(
                MockMvcResultMatchers
            ...
            .andExpect(
                MockMvcResultMatchers
                    .jsonPath("$[1].zScore")
                    .value(TEST_Z_SCORE_2)
            )
            .andExpect(
                MockMvcResultMatchers
                    .jsonPath("$[1].percentile")
                    .value(TEST_PERCENTILE_2)
            );
    }
}

...
public class DiscreteCopyNumberController {
    ...
    public ResponseEntity<List<DiscreteCopyNumberData>> getDiscreteCopyNumbersInMolecularProfile(
        @ApiParam(
            required = true,
            value = "Molecular Profile ID e.g. acc_tcga_gistic"
    ...
    @RequestMapping(
        value = "/molecular-profiles/{molecularProfileId}/discrete-copy-number-counts/fetch",
        method = RequestMethod.POST,
        consumes = MediaType.APPLICATION_JSON_VALUE,
        produces = MediaType.APPLICATION_JSON_VALUE
    )
    ...
    public ResponseEntity<List<CopyNumberCount>> fetchCopyNumberCounts(
        ...
        @ApiParam(
            required = true,
            value = "List of copy number count identifiers"
        ) @Size(
            min = 1,
            max = COPY_NUMBER_COUNT_MAX_PAGE_SIZE

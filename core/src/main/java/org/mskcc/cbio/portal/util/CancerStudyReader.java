...
public class CancerStudyReader {
    ...
    public static CancerStudy loadCancerStudy(File file)
        throws IOException, DaoException {
        return loadCancerStudy(file, true, true);
    }

    public static CancerStudy loadCancerStudy(
        File file,
        boolean strict,
        boolean addStudyToDb
    )
        throws IOException, DaoException {
        ...
        if (
            strict &&
            null ==
            DaoTypeOfCancer.getTypeOfCancerById(cancerStudy.getTypeOfCancerId())
        ) {
            throw new IllegalArgumentException(
                cancerStudy.getTypeOfCancerId() +
                " is not a supported cancer type."
            );
        }
    ...
    private static Boolean checkSpecies(String studyId, String genomeName) {
        ...
        try {
            CancerStudy oldCancerStudy = DaoCancerStudy.getCancerStudyByStableId(
                studyId
            );
    ...
    private static CancerStudy getCancerStudy(TrimmedProperties properties) {
        ...
        String typeOfCancer = properties
            .getProperty("type_of_cancer")
            .toLowerCase();
        if (typeOfCancer == null) {
            throw new IllegalArgumentException(
                "type of cancer is not specified."
            );
        }
        ...
        if (!checkSpecies(cancerStudyIdentifier, referenceGenome)) {
            throw new IllegalArgumentException(
                "Species not match with old study"
            );
        }
    ...
    private static boolean publicStudy(TrimmedProperties properties) {
        ...
        if (studyAccess != null) {
            ...
            throw new IllegalArgumentException(
                "study_access must be either 'public' or 'private', but is " +
                studyAccess
            );
        }

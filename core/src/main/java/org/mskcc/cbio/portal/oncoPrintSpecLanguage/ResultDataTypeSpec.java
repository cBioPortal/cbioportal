...
public class ResultDataTypeSpec {
     ...
     * method.
     */
    public static ResultDataTypeSpec newInstance(
        ResultDataTypeSpec aResultDataTypeSpec
    ) {
        if (null == aResultDataTypeSpec) {
            return null;
        }
        ResultDataTypeSpec theNewResultDataTypeSpec = new ResultDataTypeSpec(
            aResultDataTypeSpec.theGeneticDataType
        );
        theNewResultDataTypeSpec.setAcceptAll(
            aResultDataTypeSpec.isAcceptAll()
        );
        theNewResultDataTypeSpec.setTheDiscreteDataTypeSetSpec(
            DiscreteDataTypeSetSpec.newInstance(
                aResultDataTypeSpec.getTheDiscreteDataTypeSetSpec()
            )
        );

        theNewResultDataTypeSpec.setCombinedGreaterContinuousDataTypeSpec(
            ContinuousDataTypeSpec.newInstance(
                aResultDataTypeSpec.getCombinedGreaterContinuousDataTypeSpec()
            )
        );

        theNewResultDataTypeSpec.setCombinedLesserContinuousDataTypeSpec(
            ContinuousDataTypeSpec.newInstance(
                aResultDataTypeSpec.getCombinedLesserContinuousDataTypeSpec()
            )
        );
    ...
    public void setCombinedGreaterContinuousDataTypeSpec(
        ...
        this.combinedGreaterContinuousDataTypeSpec =
    ...
    public void setCombinedLesserContinuousDataTypeSpec(
        ...
        this.combinedLesserContinuousDataTypeSpec =
        ...
        switch (this.theGeneticDataType.getTheDataTypeCategory()) {
            case Continuous:
                ...
                if (combinedLesserContinuousDataTypeSpec != null) {
                    sb
                        .append(combinedLesserContinuousDataTypeSpec.toString())
                        .append(" ");
                }
                if (combinedGreaterContinuousDataTypeSpec != null) {
                    sb
                        .append(
                            combinedGreaterContinuousDataTypeSpec.toString()
                        )
                        .append(" ");
                }
    ...
    public boolean equals(Object aThat) {
        ...
        return (
            ...
            EqualsUtil.areEqual(
                this.theDiscreteDataTypeSetSpec,
                that.theDiscreteDataTypeSetSpec
            ) &&
            EqualsUtil.areEqual(
                this.combinedGreaterContinuousDataTypeSpec,
                that.combinedGreaterContinuousDataTypeSpec
            ) &&
            EqualsUtil.areEqual(
                this.combinedLesserContinuousDataTypeSpec,
                that.combinedLesserContinuousDataTypeSpec
            )
        );
    }
}

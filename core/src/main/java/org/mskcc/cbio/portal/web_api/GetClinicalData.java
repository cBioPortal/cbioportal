...
public class GetClinicalData {
     ...
     */
    public static List<Patient> getClinicalData(
        int cancerStudyId,
        HashSet<String> setOfCaseIds
    )
        throws DaoException {
     ...
     * @param attrId
     */
    public static JSONObject getJsonDatum(
        int cancerStudyId,
        String sampleId,
        String attrId
    )
        ...
        List<ClinicalData> data = DaoClinicalData.getSampleAndPatientData(
            cancerStudyId,
            Collections.singletonList(sampleId),
            attr
        );
        if (data.isEmpty()) {
    ...
    public static String getTxtDatum(
        int cancerStudyId,
        String sampleId,
        String attrId
    )
        ...
        return (
            "" + c.getStableId() + "\t" + c.getAttrId() + "\t" + c.getAttrVal()
        );
    }
    ...
    public static JSONObject generateJson(
        ...
        for (ClinicalAttribute attr : DaoClinicalAttributeMeta.getDatum(
            attrIds,
            cancer_study_id
        )) {
     ...
     * @throws DaoException
     */
    public static JSONObject getJSON(int cancerStudyId, List<String> sampleIds)
        throws DaoException {
        List<ClinicalData> clinicals = DaoClinicalData.getSampleAndPatientData(
            cancerStudyId,
            sampleIds
        );
     ...
     * @return
     */
    public static String getTxt(int cancerStudyId, List<String> sampleIds)
        throws DaoException {
        List<ClinicalData> allClinicals = DaoClinicalData.getSampleAndPatientData(
            cancerStudyId,
            sampleIds
        );
        ...
        for (ClinicalData c : allClinicals) {
            ...
            Map<String, ClinicalData> got = sampleId2Clinical.get(
                c.getStableId()
            );

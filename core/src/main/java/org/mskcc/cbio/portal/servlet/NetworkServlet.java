...
public class NetworkServlet extends HttpServlet {
    ...
    private static final String NODE_ATTR_PERCENT_CNA_AMPLIFIED =
        "PERCENT_CNA_AMPLIFIED";
    private static final String NODE_ATTR_PERCENT_CNA_GAINED =
        "PERCENT_CNA_GAINED";
    private static final String NODE_ATTR_PERCENT_CNA_HOM_DEL =
        "PERCENT_CNA_HOMOZYGOUSLY_DELETED";
    private static final String NODE_ATTR_PERCENT_CNA_HET_LOSS =
        "PERCENT_CNA_HEMIZYGOUSLY_DELETED";
    private static final String NODE_ATTR_PERCENT_MRNA_WAY_UP =
        "PERCENT_MRNA_WAY_UP";
    private static final String NODE_ATTR_PERCENT_MRNA_WAY_DOWN =
        "PERCENT_MRNA_WAY_DOWN";
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse res)
        ...
        try {
            ...
            if ("getdatasources".equalsIgnoreCase(cmd)) {
                ...
                List<String> dataSources = DaoInteraction
                    .getInstance()
                    .getDataSources();
            ...
            writeMsg(
                "Error loading network. Please report this to " +
    ...
    public void processGetNetworkRequest(
        ...
        try {
            if (cancerStudyId != null) {
                cancerStudy =
                    DaoCancerStudy.getCancerStudyByStableId(cancerStudyId);
                if (cancerStudy == null) {
                    return;
                }
            } else {
            ...
            Set<String> queryGenes = new HashSet<String>(
                Arrays.asList(geneListStr.toUpperCase().split("[, ]+"))
            );
            int nMiRNA = filterNodes(queryGenes);
            if (nMiRNA > 0) {
                messages.append(
                    "MicroRNAs were excluded from the network query. "
                );
            }
            ...
            try {
                netSize =
                    NetworkIO.NetworkSize.valueOf(strNetSize.toUpperCase());
            } catch (Exception e) {
                netSize = NetworkIO.NetworkSize.LARGE;
            }

            String dsSrc = req.getParameter("source");
            List<String> dataSources = dsSrc == null
                ? null
            ...
            if ("cpath2".equalsIgnoreCase(netSrc)) {
                ...
                if (logXDebug) {
                    xdebug.logMsg(
                        "GetPathwayCommonsNetwork",
                        "<a href=\"" +
                        NetworkIO.getCPath2URL(queryGenes) +
                        "\" target=\"_blank\">cPath2 URL</a>"
                    );
                }
            } else {
                network =
                    NetworkIO.readNetworkFromCGDS(
                        queryGenes,
                        netSize,
                        dataSources,
                        true
            ...
            xdebug.logMsg(
                this,
                "Successfully retrieved networks from " +
                netSrc +
                ": took " +
            ...
            if (network.countNodes() != 0 && cancerStudyId != null) {
                ...
                List<Integer> internalSampleIds = InternalIdUtil.getInternalNonNormalSampleIds(
                    ...
                    new ArrayList<String>(targetSampleIds)
                );

                //  Get User Selected Genetic Profiles
                Set<GeneticProfile> geneticProfileSet = getGeneticProfileSet(
                    req,
                    cancerStudyId
                );

                // getzScoreThreshold
                double zScoreThreshold = Double.parseDouble(
                    req.getParameter(QueryBuilder.Z_SCORE_THRESHOLD)
                );
                ...
                xdebug.logMsg(
                    this,
                    "Retrived data from CGDS. Took " +
                    xdebug.getTimeElapsed() +
                    "ms"
                );

                if (mapQueryGeneAlterationCaseNumber != null) {
                    addAttributesForQueryGenes(
                        queryNodes,
                        internalSampleIds.size(),
                ...
                if (nLinker != null && nLinker.matches("[0-9]+")) {
                    ...
                    xdebug.startTimer();
                    pruneNetworkByAlteration(
                        network,
                        diffusion,
                        Integer.parseInt(nLinker),
                        querySize
                    );
                    ...
                    if (nBefore != nAfter) {
                        ...
                        messages.append(
                            " most frequently altered neighbor genes "
                        );
                    ...
                    xdebug.logMsg(
                        this,
                        "Prune network. Took " + xdebug.getTimeElapsed() + "ms"
                    );
                }

                String encodedQueryAlteration = encodeQueryAlteration(
                    mapQueryGeneAlterationCaseNumber
                );
                ...
                if (logXDebug) {
                    xdebug.logMsg(
                        this,
                        "<a href=\"" +
                        getNetworkServletUrl(
                            req,
                            false,
                            false,
                            false,
                            encodedQueryAlteration
                        ) +
                ...
                messages.append(
                    " (<a href=\"http://chianti.ucsd.edu/cyto_web/plugins/displayplugininfo.php?"
                );
                messages.append(
                    "name=GraphMLReader\" target=\"_blank\">GraphMLReader plugin</a>"
                );
            ...
            if (download != null && download.equalsIgnoreCase("on")) {
                ...
                res.addHeader(
                    "content-disposition",
                    "attachment; filename=cbioportal." +
                    (sif ? "sif" : "graphml")
                );
                messages.append(
                    "In order to open this file in Cytoscape, please install GraphMLReader plugin."
                );
            } else {
            ...
            NetworkIO.NodeLabelHandler nodeLabelHandler = new NetworkIO.NodeLabelHandler() {
                ...
                public String getLabel(Node node) {
                    if (
                        node.getType().equals(NodeType.DRUG)
            ...
            if (sif) {
                ...
                graph =
                    NetworkIO.writeNetwork2GraphML(network, nodeLabelHandler);
            }
            ...
            if (
                (msgoff == null || !msgoff.equals("t")) && messages.length() > 0
            ) {
    ...
    private int filterNodes(Set<String> queryGenes) {
        ...
        try {
            ...
            for (Iterator<String> it = queryGenes.iterator(); it.hasNext();) {
                ...
                if (
                    gene == null || gene.isMicroRNA() || gene.isPhosphoProtein()
                ) {
                    it.remove();
                    n++;
                }
     ...
     */
    private void pruneNetworkByAlteration(
        Network network,
        double diffusion,
        int nKeep,
        int nQuery
     ...
     * @return
     */
    private List<Node> getNodesToRemove(
        final Network network,
        final double diffusion,
        final int n
    ) {
        final Map<Node, Double> mapDiffusion = getMapDiffusedTotalAlteredPercentage(
            network,
            diffusion
        );

        // keep track of the top nKeep
        PriorityQueue<Node> topAlteredNodes = new PriorityQueue<Node>(
            n,
            new Comparator<Node>() {

                public int compare(Node n1, Node n2) {
                    int ret = mapDiffusion
                        .get(n1)
                        .compareTo(mapDiffusion.get(n2));
                    if (diffusion != 0 && ret == 0) { // if the same diffused perc, use own perc
                        ret =
                            Double.compare(
                                getTotalAlteredPercentage(n1),
        ...
        for (Node node : network.getNodes()) {
            ...
            if (topAlteredNodes.size() < n) {
                ...
                if (n == 0) {
                    ...
                    if (
                        mapDiffusion.get(node) >
                        mapDiffusion.get(topAlteredNodes.peek())
                    ) {
    ...
    private Map<Node, Double> getMapDiffusedTotalAlteredPercentage(
        Network network,
        double diffusion
    ) {
        ...
        for (Node node : network.getNodes()) {
            map.put(
                node,
                getDiffusedTotalAlteredPercentage(network, node, diffusion)
            );
        }
        return map;
    }

    private double getDiffusedTotalAlteredPercentage(
        Network network,
        Node node,
        double diffusion
    ) {
    ...
    private double getTotalAlteredPercentage(Node node) {
        Double alterPerc = (Double) node.getAttribute(
            NODE_ATTR_PERCENT_ALTERED
        );
    ...
    private Set<String> getSampleIds(
        ...
        if (strSampleIds == null || strSampleIds.length() == 0) {
            ...
            if (strSampleIds == null || strSampleIds.length() == 0) {
                ...
                ArrayList<SampleList> sampleSets = GetSampleLists.getSampleLists(
                    cancerStudyId
                );
    ...
    private Set<GeneticProfile> getGeneticProfileSet(
        HttpServletRequest req,
        String cancerStudyId
    )
        throws ServletException, DaoException {
        ...
        for (String geneticProfileIdsStr : req.getParameterValues(
            QueryBuilder.GENETIC_PROFILE_IDS
        )) {
            for (String profileId : geneticProfileIdsStr.split(" ")) {
                GeneticProfile profile = GeneticProfileUtil.getProfile(
                    profileId,
                    profileList
                );
                if (null != profile) {
    ...
    private void addAttributesForQueryGenes(
        Set<Node> nodes,
        int nCases,
        ...
        for (Node node : nodes) {
            String symbol = NetworkUtils.getSymbol(node);
            Map<String, Integer> mapAltrationCaseNumber = mapQueryGeneAlterationCaseNumber.get(
                symbol
            );
            for (Map.Entry<String, Integer> entry : mapAltrationCaseNumber.entrySet()) {
                node.setAttribute(
                    mapHeatMapKeyToAttrName.get(entry.getKey()),
                    1.0 * entry.getValue() / nCases
                );
            }
        }
    }

    private Map<String, Map<String, Integer>> getMapQueryGeneAlterationCaseNumber(
        HttpServletRequest req
    ) {
        ...
        if (heatMap != null) {
            Map<String, Map<String, Integer>> mapQueryGeneAlterationCaseNumber = new HashMap<String, Map<String, Integer>>();
            ...
            for (int i = 1; i < heatMapLines.length; i++) {
                ...
                for (int j = 1; j < strs.length; j++) {
                    Map<String, Integer> map = mapQueryGeneAlterationCaseNumber.get(
                        genes[j]
                    );
                    if (!strs[j].isEmpty()) {
    ...
    static {
        ...
        mapHeatMapKeyToAttrName.put(
            "Any",
            NetworkServlet.NODE_ATTR_PERCENT_ALTERED
        );
        mapHeatMapKeyToAttrName.put(
            "AMP",
            NetworkServlet.NODE_ATTR_PERCENT_CNA_AMPLIFIED
        );
        mapHeatMapKeyToAttrName.put(
            "GAIN",
            NetworkServlet.NODE_ATTR_PERCENT_CNA_GAINED
        );
        mapHeatMapKeyToAttrName.put(
            "HETLOSS",
            NetworkServlet.NODE_ATTR_PERCENT_CNA_HET_LOSS
        );
        mapHeatMapKeyToAttrName.put(
            "HOMDEL",
            NetworkServlet.NODE_ATTR_PERCENT_CNA_HOM_DEL
        );
        mapHeatMapKeyToAttrName.put(
            "DOWN",
            NetworkServlet.NODE_ATTR_PERCENT_MRNA_WAY_DOWN
        );
        mapHeatMapKeyToAttrName.put(
            "UP",
            NetworkServlet.NODE_ATTR_PERCENT_MRNA_WAY_UP
        );
        mapHeatMapKeyToAttrName.put(
            "MUT",
            NetworkServlet.NODE_ATTR_PERCENT_MUTATED
        );
    }

    private void addCGDSDataAsNodeAttribute(
        Node node,
        long entrezGeneId,
        Set<GeneticProfile> profiles,
        ...
        for (GeneticProfile profile : profiles) {
            if (
                ...
                Set<String> samples = getMutatedSamples(
                ...
                node.setAttribute(
                    NODE_ATTR_PERCENT_MUTATED,
                    1.0 * samples.size() / internalSampleIds.size()
                );
            } else if (
                profile.getGeneticAlterationType() ==
                GeneticAlterationType.COPY_NUMBER_ALTERATION
            ) {
                Map<String, Set<String>> cnaSamples = getCNASamples(
                    profile.getGeneticProfileId(),
                    internalSampleIds,
                    entrezGeneId
                );
                ...
                if (!samples.isEmpty()) {
                    alteredSamples.addAll(samples);
                    node.setAttribute(
                        NODE_ATTR_PERCENT_CNA_AMPLIFIED,
                        1.0 * samples.size() / internalSampleIds.size()
                    );
                }
                ...
                Set<String>[] samples = getMRnaAlteredSamples(
                    profile.getGeneticProfileId(),
                    internalSampleIds,
                    entrezGeneId,
                    zScoreThreshold
                ...
                node.setAttribute(
                    NODE_ATTR_PERCENT_MRNA_WAY_UP,
                    1.0 * samples[0].size() / internalSampleIds.size()
                );
                node.setAttribute(
                    NODE_ATTR_PERCENT_MRNA_WAY_DOWN,
                    1.0 * samples[1].size() / internalSampleIds.size()
                );
            }
        }
        ...
        node.setAttribute(
            NODE_ATTR_PERCENT_ALTERED,
            1.0 * alteredSamples.size() / internalSampleIds.size()
        );
    }
     ...
     */
    private Set<String> getMutatedSamples(
        ...
        List<ExtendedMutation> mutationList = mutationModelConverter.convert(
            mutationRepositoryLegacy.getMutations(
                internalSampleIds,
                (int) entrezGeneId,
                geneticProfileId
            )
     ...
     * @throws DaoException
     */
    private Map<String, Set<String>> getCNASamples(
        int geneticProfileId,
        List<Integer> internalSampleIds,
        long entrezGeneId
    )
        ...
        Map<Integer, String> sampleMap = DaoGeneticAlteration
            .getInstance()
     ...
     * @throws DaoException
     */
    private Set<String>[] getMRnaAlteredSamples(
        int geneticProfileId,
        List<Integer> internalSampleIds,
        long entrezGeneId,
        double zScoreThreshold
    ...
    private Map<String, Map<String, Integer>> decodeQueryAlteration(
        String strQueryAlteration
    ) {
    ...
    private String encodeQueryAlteration(
        Map<String, Map<String, Integer>> queryAlteration
    ) {
        ...
        for (Map.Entry<String, Map<String, Integer>> entry1 : queryAlteration.entrySet()) {
            ...
            sb.append(':');
            for (Map.Entry<String, Integer> entry2 : entry1
                .getValue()
                .entrySet()) {
    ...
    private String getNetworkServletUrl(
        HttpServletRequest req,
        boolean complete,
        boolean download,
        boolean sif,
        ...
        String geneListStr = req.getParameter(QueryBuilder.GENE_LIST);
        String geneticProfileIdsStr = req.getParameter(
            QueryBuilder.GENETIC_PROFILE_IDS
        );
        String cancerStudyId = req.getParameter(QueryBuilder.CANCER_STUDY_ID);
        String caseSetId = req.getParameter(QueryBuilder.CASE_SET_ID);
        String zscoreThreshold = req.getParameter(
    ...
    private void writeXDebug(XDebug xdebug, HttpServletResponse res)
        ...
        for (Object msg : xdebug.getDebugMessages()) {
            writer.write(
                ((org.mskcc.cbio.portal.util.XDebugMessage) msg).getMessage()
            );
            writer.write("\n");
        }

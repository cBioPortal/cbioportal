...
public class MolecularDataServiceImpl implements MolecularDataService {
    ...
    public List<GeneMolecularData> fetchMolecularData(
        ...
        for (Sample sample : samples) {
            Integer indexOfSampleId = internalSampleIdsMap.get(
            ...
            if (indexOfSampleId != null) {
                for (GeneMolecularAlteration molecularAlteration : molecularAlterations) {
                    ...
                    molecularData.setEntrezGeneId(
                        molecularAlteration.getEntrezGeneId()
                    );
                    molecularData.setValue(
                        molecularAlteration.getSplitValues()[indexOfSampleId]
                    );
    ...
    @Override
    public Integer getNumberOfSamplesInMolecularProfile(
        String molecularProfileId
    ) {
        String commaSeparatedSampleIdsOfMolecularProfile = molecularDataRepository.getCommaSeparatedSampleIdsOfMolecularProfile(
            molecularProfileId
    ...
    public List<GeneMolecularData> getMolecularDataInMultipleMolecularProfiles(
        ...
        List<GeneMolecularData> molecularDataList = new ArrayList<>();
        List<String> distinctMolecularProfileIds = molecularProfileIds
            .stream()
            .distinct()
            .sorted()
            .collect(Collectors.toList());

        List<String> commaSeparatedSampleIdsOfMolecularProfiles = molecularDataRepository.getCommaSeparatedSampleIdsOfMolecularProfiles(
        ...
        for (int i = 0; i < distinctMolecularProfileIds.size(); i++) {
            List<Integer> internalSampleIds = Arrays
                .stream(
                    commaSeparatedSampleIdsOfMolecularProfiles.get(i).split(",")
                )
                .mapToInt(Integer::parseInt)
                .boxed()
                .collect(Collectors.toList());
            ...
            internalSampleIdsMap.put(
                distinctMolecularProfileIds.get(i),
                molecularProfileSampleMap
            );
        ...
        Map<String, MolecularProfile> molecularProfileMapById = distinctMolecularProfiles
            .stream()
            .collect(
                Collectors.toMap(
        ...
        Map<String, List<MolecularProfile>> molecularProfileMapByStudyId = distinctMolecularProfiles
            .stream()
            .collect(
        ...
        if (sampleIds == null) {
            samples =
                sampleService.getSamplesByInternalIds(allInternalSampleIds);
            for (String molecularProfileId : distinctMolecularProfileIds) {
                internalSampleIdsMap
                    ...
                    .forEach(
                        s ->
                            molecularProfiles.add(
                                molecularProfileMapById.get(molecularProfileId)
                            )
                    );
            }
        } else {
        ...
        List<GeneMolecularAlteration> molecularAlterations = molecularDataRepository.getGeneMolecularAlterationsInMultipleMolecularProfiles(
            distinctMolecularProfileIds,
            entrezGeneIds,
            projection
        );
        Map<String, List<GeneMolecularAlteration>> molecularAlterationsMap = molecularAlterations
            .stream()
            .collect(
                Collectors.groupingBy(
        ...
        for (Sample sample : samples) {
            for (MolecularProfile molecularProfile : molecularProfileMapByStudyId.get(
                sample.getCancerStudyIdentifier()
            )) {
                String molecularProfileId = molecularProfile.getStableId();
                Integer indexOfSampleId = internalSampleIdsMap
                    .get(molecularProfileId)
                    .get(sample.getInternalId());
                if (
                    indexOfSampleId != null &&
                    molecularAlterationsMap.containsKey(molecularProfileId)
                ) {
                    for (GeneMolecularAlteration molecularAlteration : molecularAlterationsMap.get(
                        molecularProfileId
                    )) {
                        GeneMolecularData molecularData = new GeneMolecularData();
                        molecularData.setMolecularProfileId(molecularProfileId);
    ...
    @Override
    @PreAuthorize(
        "hasPermission(#molecularProfileIds, 'Collection<MolecularProfileId>', 'read')"
    )
    ...
    private void validateMolecularProfile(String molecularProfileId)
        ...
        if (
            ...
            molecularProfile
                .getMolecularAlterationType()
                .equals(MolecularAlterationType.FUSION)
        ) {

...
public class SampleServiceImpl implements SampleService {
    ...
    public List<Sample> getAllSamples(
        ...
        List<Sample> samples = sampleRepository.getAllSamples(
            keyword,
            studyIds,
            projection,
            pageSize,
    ...
    public List<Sample> getAllSamplesInStudy(
        ...
        List<Sample> samples = sampleRepository.getAllSamplesInStudy(
            studyId,
            projection,
            pageSize,
            pageNumber,
    ...
    public List<Sample> getAllSamplesOfPatientInStudy(
        ...
        List<Sample> samples = sampleRepository.getAllSamplesOfPatientInStudy(
            studyId,
            patientId,
            projection,
            pageSize,
    ...
    public BaseMeta getMetaSamplesOfPatientInStudy(
        ...
        return sampleRepository.getMetaSamplesOfPatientInStudy(
            studyId,
            patientId
        );
    }
    ...
    public List<Sample> getAllSamplesOfPatientsInStudy(
        ...
        List<Sample> samples = sampleRepository.getAllSamplesOfPatientsInStudy(
            studyId,
            patientIds,
            projection
        );
    ...
    public List<Sample> getSamplesOfPatientsInMultipleStudies(
        ...
        List<Sample> samples = sampleRepository.getSamplesOfPatientsInMultipleStudies(
            studyIds,
            patientIds,
            projection
        );
    ...
    public List<Sample> fetchSamples(
        ...
        List<Sample> samples = sampleRepository.fetchSamples(
            studyIds,
            sampleIds,
            projection
        );
    ...
    private void processSamples(List<Sample> samples, String projection) {
        if (projection.equals("DETAILED")) {
            ...
            List<String> distinctStudyIds = samples
                .stream()
                .map(Sample::getCancerStudyIdentifier)
                .distinct()
                .collect(Collectors.toList());
            List<MolecularProfile> molecularProfiles = molecularProfileRepository.getMolecularProfilesInStudies(
                distinctStudyIds,
                projection
            );
            List<String> studiesProfiledWithSVs = molecularProfiles
                .stream()
                .filter(
                    p ->
                        p
            ...
            for (String studyId : distinctStudyIds) {
                ...
                structuralVariantSampleIdsMap.put(
                    studyId,
                    new HashSet<String>(
                        sampleListRepository.getAllSampleIdsInSampleList(
                            studyId + STRUCTURAL_VARIANT
                        )
                    )
                );
            }
            ...
            samples.forEach(
                sample -> {
                    ...
                    sample.setCopyNumberSegmentPresent(
                        samplesWithCopyNumberSegMap.contains(
                            sample.getInternalId()
                        )
                    );
                    if (
                        studiesProfiledWithSVs.contains(
                            sample.getCancerStudyIdentifier()
                        )
                    ) {
                        if (
                            !structuralVariantSampleIdsMap
                                .get(sample.getCancerStudyIdentifier())
                                .isEmpty()
                        ) {
                            sample.setProfiledForFusions(
                                structuralVariantSampleIdsMap
                                    .get(sample.getCancerStudyIdentifier())
                                    .contains(sample.getStableId())
                            );

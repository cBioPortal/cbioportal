...
public class ClickhouseMutationRepository implements MutationRepository {
  ...
  public List<Mutation> getMutationsInMultipleMolecularProfiles(
    ...
    Integer offset =
        PaginationCalculator.offset(
            mutationSearchCriteria.pageSize(), mutationSearchCriteria.pageNumber());
    List<ProfileSamplePair> profileSamplesPairs =
        groupAndFilterProfileAndSample(molecularProfileIds, sampleIds);
  ...
  public MutationMeta getMetaMutationsInMultipleMolecularProfiles(
    ...
    return mapper.getMetaMutationsInMultipleMolecularProfiles(
        profileSamplesPairs, entrezGeneIds, false);
  }
   ...
   */
  private List<ProfileSamplePair> groupAndFilterProfileAndSample(
    ...
    return groupedCases.entrySet().stream()
        .map(entry -> new ProfileSamplePair(entry.getKey(), new ArrayList<>(entry.getValue())))
        .toList();
  }
}

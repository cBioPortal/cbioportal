...
/* THIS FILE HAS BEEN MODIFIED FROM THE ORIGAL
 *
 * The modifications are Copyright (C) 2022 Memorial Sloan Kettering Cancer Center
 * and are free software: you can redistribute them and/or modify
 * them under the terms of the GNU Affero General Public License as published
 * by the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * The modificaations are distributed in the hope that they will be useful,
 ...
 *
 * Modifications:
 * - 2022_06_30 : bugfix to correct multiple instances of header
 *                in output of function runQuery().
 *
 */
  ...
  public <T> List<T> runQuery(final String query,
    ...
    StringBuilder sb = new StringBuilder();
      try (final FlightStream flightStream = getStream(flightInfo, bearerToken, headerCallOption)) {
        ...
        // MODIFIED HERE TO ELIMINATE DUPLICATED HEADER LINES IN OUTPUT
        boolean headerAdded = false;
        while (flightStream.next()) {
            VectorSchemaRoot vectorSchemaRoot = flightStream.getRoot();
            String tsvString = vectorSchemaRoot.contentToTSVString();
            if (headerAdded) {
                // strip header from output
                int firstNewlinePos = tsvString.indexOf('\n');
                if (firstNewlinePos != -1 && firstNewlinePos + 1 < tsvString.length()) {
                    // there is something beyond the header
                    sb.append(tsvString.substring(firstNewlinePos + 1));
                }
            } else {
                sb.append(tsvString);
                headerAdded = true;

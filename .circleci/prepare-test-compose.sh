...
python3 - "$COMPOSE_DIR/config/init.sh" "$COMPOSE_DIR/data/init.sh" "$TEST_DIR/scripts/docker-compose.sh" "$COMPOSE_DIR" <<'PY'
from pathlib import Path
import sys

config_init, data_init, compose_helper, compose_dir = map(Path, sys.argv[1:])
...
if ! git -C "$COMPOSE_DIR" diff --quiet -- config/init.sh data/init.sh; then
  git -C "$COMPOSE_DIR" config user.email "ci@cbioportal.org"
  git -C "$COMPOSE_DIR" config user.name "cBioPortal CI"
  git -C "$COMPOSE_DIR" add config/init.sh data/init.sh
  git -C "$COMPOSE_DIR" commit -m "ci: use explicit backend image for test fixtures" >/dev/null
fi

...
public class TestPanCancerStudySummaryImport {
    ...
    public void testPanCancerImport() throws Exception {
        ...
        addCancerStudy(
            "src/test/resources/panCancerStudySummary/meta_study.txt"
        );

        //ImportClinicalData portal-study/data_clinical.txt multi_cancer_study
        addClinicalData(
            "src/test/resources/panCancerStudySummary/meta_clinical.txt",
        ...
        addProfileData(
            "src/test/resources/panCancerStudySummary/meta_CNA.txt",
            "src/test/resources/panCancerStudySummary/data_CNA.txt"
        );
        ...
        addProfileData(
            "src/test/resources/panCancerStudySummary/meta_mutations_extended.txt",
            "src/test/resources/panCancerStudySummary/data_mutations_extended.txt"
        );

        // ImportSampleList portal-study/case_lists/cases_all.txt
        // we're passing a single file, not the directory
        addSampleLists(
            "src/test/resources/panCancerStudySummary/cases_all.txt"
        );
    ...
    private void addCancerStudy(String fileName) throws Exception {
        ImportCancerStudy runner = new ImportCancerStudy(
            new String[] { fileName }
        );
        runner.run();
    }

    private void addProfileData(String descriptorFileName, String dataFileName)
        throws Exception {
        try {
            ImportProfileData runner = new ImportProfileData(
                new String[] {
                    "--meta",
                    descriptorFileName,
                    "--loadMode",
    ...
    private void addSampleLists(String fileName) throws Exception {
        ImportSampleList runner = new ImportSampleList(
            new String[] { fileName }
        );
        runner.run();
    }

    private void addClinicalData(
        String descriptorFileName,
        String dataFileName
    )
        throws Exception {
        ImportClinicalData runner = new ImportClinicalData(
            new String[] {
                "--meta",
                descriptorFileName,
                "--loadMode",
    ...
    public void runChecksForGetCancerTypeInfo() throws Exception {
        ...
        List<Sample> samples = DaoSample.getSamplesByCancerStudy(
            study.getInternalId()
        );
        ...
        Map<String, Set<String>> cancerTypeInfoMap = DaoClinicalData.getCancerTypeInfoBySamples(
            sampleList
        );
        assertEquals(cancerTypeInfoMap.keySet().size(), 2);
        assertEquals(
            cancerTypeInfoMap.containsKey(ClinicalAttribute.CANCER_TYPE),
            true
        );
        assertEquals(
            cancerTypeInfoMap.containsKey(
                ClinicalAttribute.CANCER_TYPE_DETAILED
            ),
            true
        );
        assertEquals(
            cancerTypeInfoMap.get(ClinicalAttribute.CANCER_TYPE).size(),
            2
        );
        assertEquals(
            cancerTypeInfoMap
                .get(ClinicalAttribute.CANCER_TYPE_DETAILED)
                .size(),
            5
        );
    }

...
public class GenesetCorrelationServiceImplTest extends BaseServiceImplTest {
  ...
  @InjectMocks private GenesetCorrelationServiceImpl genesetCorrelationService;
  ...
  public void setUp() throws Exception {
    ...
    geneList.add(gene);
    Mockito.when(genesetService.getGenesByGenesetId(GENESET_ID1)).thenReturn(geneList);
    ...
    Mockito.when(
            geneticDataService.fetchMolecularData(
                MOLECULAR_PROFILE_ID,
                Arrays.asList(SAMPLE_ID1, SAMPLE_ID2),
                Arrays.asList(1),
                "SUMMARY"))
  ...
  private GeneMolecularData getSimpleFlatGeneDataItem(
      String sampleStableId, int entrezGeneId, String value) {
  ...
  public void fetchCorrelatedGenes() throws Exception {
    ...
    List<GenesetCorrelation> result =
        genesetCorrelationService.fetchCorrelatedGenes(
            GENESET_ID1, MOLECULAR_PROFILE_ID, Arrays.asList(SAMPLE_ID1, SAMPLE_ID2), 0.3);

    // what we expect: gene 1 has good correlation with geneset 1, while gene 2 is anti-correlated.
    // So
    // we expect only gene 1 to return, with correlation close to 1.0 (it is artificially high in
    // this example)
    ...
    result =
        genesetCorrelationService.fetchCorrelatedGenes(
            GENESET_ID1, MOLECULAR_PROFILE_ID, Arrays.asList(SAMPLE_ID1, SAMPLE_ID2), -1.0);

    // now we expect both genes to return, since correlation threshold is at -1.0 (just a dummy

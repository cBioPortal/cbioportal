...
Some IDPs (e.g. Azure Active Directory) cache user data for more than 2 hours causing cbioportal to complain that the authentication statement is too old to be used. You can fix this problem by setting `forceAuthN` to true. Below is an examle how you can do this with the properties. You can choose any binding type you like. `bindings:HTTP-Redirect` is given just as an example.

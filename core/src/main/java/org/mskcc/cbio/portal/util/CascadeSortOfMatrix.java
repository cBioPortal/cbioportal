...
Finally, the first and only extent of rows 0 and 1 is sorted in row 2:
  ...
  8  8  7  8  8
    ...
    */
    ...
    public Object[][] sort(Object[][] arrayData) {
        ...
        for (int rowIndex = 1; rowIndex < arrayData.length; rowIndex++) {
            ...
            while (true) {
                ...
                while (
                    (nextExtentStart < width) &&
                    !cumulativeChanges[nextExtentStart]
                ) {
                    nextExtentStart++;
                }

                sortExtent(arrayData, rowIndex, extentStart, nextExtentStart);
                recordChangeInThisRowOfData(
                    arrayData,
                    rowIndex,
                    extentStart,
                    nextExtentStart,
     ...
     */
    private void recordChangeInThisRowOfData(
        Object[][] data,
        int row,
        int extentStart,
        int nextExtentStart,
        boolean[] cumulativeChanges
    ) {
    ...
    */
    ...
    public Object[][] sortExtent(
        ...
        for (int i = 0; i < len; i++) {
            extentCopy[i] =
                new ElementAndIndex(data[rowIndex][i + extentStart], i);
        }

        //System.out.println( "extentCopy is:\n" + pToString(extentCopy ));
        SortExtentComparator mySortExtentComparator = new SortExtentComparator(
            theObjectComparator
        );
        ...
        for (int row = 0; row < data.length; row++) {
            ...
            for (int offset = 0; offset < len; offset++) {
                data[row][extentStart + offset] =
                    buffer[extentCopy[offset].index];
            }
        }
     ...
     */
    // NOT USED
    public static Object[][] copyToArrayOfArray(
        ArrayList<ArrayList<Object>> arrayListData
    ) {
        Object[][] data = new Object[arrayListData.size()][arrayListData
            .get(0)
            .size()];
        int rowNum = 0;
     ...
     */
    // NOT USED
    public static ArrayList<ArrayList<Object>> copyToArrayListOfArrayList(
        Object[][] arrayData
    ) {

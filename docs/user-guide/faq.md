...
[ASCAT (Allele-Specific Copy number Analysis of Tumors)](https://www.pnas.org/doi/full/10.1073/pnas.1009843107) is a computational tool used in cancer genomics. It is designed to analyze allele-specific copy number variations (CNVs) in tumor DNA. CNVs refer to changes in the number of copies of a particular gene or genomic region, which can contribute to cancer development and progression.

ASCAT helps to distinguish between somatic and germline variations and provides a more accurate depiction of the tumor's genomic landscape. This information is crucial for understanding the genetic basis of cancer, identifying potential therapeutic targets, and personalizing treatment strategies.

The analysis involves the following steps:
1. **Data Collection**: Tumor and normal tissue samples are collected and sequenced.
2. **Preprocessing**: Raw sequencing data is processed to identify CNVs and allelic ratios.
3. **Analysis**: ASCAT uses the processed data to determine the allele-specific copy numbers, considering tumor purity and ploidy.
4. **Interpretation**: The results are interpreted to understand the genomic alterations in the tumor, which can inform clinical decision-making.

Copy number data from the GDC analysis pipelines is provided in ASCAT format. ASCAT data is not supported directly by cBioPortal, instead it is first converted to GISTIC as described below.
...
The final conversion threshold (6 &le; TCN) is somewhat flexible and can vary between different studies depending on the data used. We chose 6 and applied it to all GDC studies after seeing that it resulted in the most consistency between TCGA-GDC and PanCancer Atlas.

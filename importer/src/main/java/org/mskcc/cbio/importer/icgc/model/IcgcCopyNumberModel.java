...
public class IcgcCopyNumberModel {
    ...
    /*
    represents a POJO for an ICGC copy number somatic mutation record
    field names selected to facilitate mapping by TSV -> object mapping utility
    n.b. don't add new fields or change field names
     */
    ...
    public Optional<Tuple3 <String,String,String> > resolveCnvTuple() {
        /*
        ICGC copy number data does not specify a strand. Check both + & -
        strands for a known gene
         */
        String geneName ="";
        geneName = geneMapper.findGeneNameByGenomicPosition(this.getChromosome(),
                this.getChromosome_start(), "1");
        if (Strings.isNullOrEmpty(geneName)){
            geneName = geneMapper.findGeneNameByGenomicPosition(this.getChromosome(),
                    this.getChromosome_start(), "-1");
        }
        ...
        return  Optional.of(new Tuple3<String,String,String>(geneName, this.getIcgc_sample_id(),
                CnvTransformer.resolveCopyNumberVariation(this.getCopy_number())));
    }

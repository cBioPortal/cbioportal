...
public class IGVLinking {
    ...
    public static String[] getIGVArgsForSegViewing(
        ...
        CopyNumberSegmentFile cnsf = DaoCopyNumberSegmentFile.getCopyNumberSegmentFile(
            cancerStudy.getInternalId()
        );

        return new String[] {
            GlobalProperties.getSegfileUrl() + cnsf.filename,
            encodedGeneList,
            cnsf.referenceGenomeId.toString(),
            cnsf.filename
        };
    }

    // returns null if exception has been thrown during processing
    public static String[] getIGVArgsForBAMViewing(
        String cancerStudyStableId,
        String caseId,
        String locus
    ) {
        ...
        return new String[] {
            tumorBAMFileURL,
            encodedLocus,
            CopyNumberSegmentFile.ReferenceGenomeId.hg19.toString(),
            trackName
        };
    }

    public static boolean validBAMViewingArgs(
        String cancerStudy,
        String caseId,
        String locus
    ) {
    ...
    private static boolean encryptionBinLocated() {
        return new File(
            GlobalProperties.getProperty(GlobalProperties.OPENSSL_BINARY)
        )
        .exists();
    }
    ...
    private static String getBAMFileURL(String caseId) {
        ...
        return (token == null)
            ? null
            : GlobalProperties
                .getProperty(GlobalProperties.BROAD_BAM_URL)
    ...
    private static String getToken(String caseId) {
        ...
        try {
            messageToEncrypt =
                getMessageToEncrypt(caseId, IGVLinking.getCurrentTime());
    ...
    private static File getMessageToEncrypt(String caseId, String timestamp)
        throws Exception {
        File token = FileUtils.getFile(
    ...
    private static File getEncryptedMessage(File messageToEncrypt)
        throws Exception {
        File encryptedMessage = null;
        ...
        try {
            encryptedMessage =
                FileUtils.getFile(
                    FileUtils.getTempDirectoryPath(),
                    "broad-bam-encrypted.txt"
                );
            IGVLinking.execute(
                IGVLinking.getEncryptCommand(messageToEncrypt, encryptedMessage)
            );
        } catch (Exception e) {
            FileUtils.deleteQuietly(encryptedMessage);
            throw e;
    ...
    private static String getEncryptCommand(
        ...
        return (
            ...
            " rsautl -encrypt" +
            " -inkey " +
            GlobalProperties.getProperty(GlobalProperties.ENCRYPTION_KEY) +
            " -keyform PEM -pubin" +
            " -in " +
            messageToEncrypt.getCanonicalPath() +
            " -out " +
            encryptedMessage.getCanonicalPath()
        );
    }

    private static File getSignedMessage(File encryptedMessage)
        throws Exception {
        File signedMessage = null;
        ...
        try {
            signedMessage =
                FileUtils.getFile(
                    FileUtils.getTempDirectoryPath(),
                    "broad-bam-signed-encrypted.txt"
                );
            IGVLinking.execute(
                IGVLinking.getSignCommand(encryptedMessage, signedMessage)
            );
        } catch (Exception e) {
            FileUtils.deleteQuietly(signedMessage);
            throw e;
    ...
    private static String getSignCommand(
        ...
        return (
            ...
            " rsautl -sign" +
            " -inkey " +
            GlobalProperties.getProperty(GlobalProperties.SIGNATURE_KEY) +
            " -keyform PEM" +
            " -in " +
            encryptedMessage.getCanonicalPath() +
            " -out " +
            signedMessage.getCanonicalPath()
        );
    }
    ...
    private static File getBase64Message(File signedMessage) throws Exception {
        ...
        try {
            base64Message =
                FileUtils.getFile(
                    FileUtils.getTempDirectoryPath(),
                    "broad-bam-base64-signed-encrypted.txt"
                );
            IGVLinking.execute(
                IGVLinking.getBase64Command(signedMessage, base64Message)
            );
        } catch (Exception e) {
            FileUtils.deleteQuietly(base64Message);
            throw e;
    ...
    private static String getBase64Command(
        File signedMessage,
        File base64Message
    )
        throws Exception {
        return (
            ...
            signedMessage.getCanonicalPath() +
            " -out " +
            base64Message.getCanonicalPath()
        );
    }
    ...
    private static boolean knownCaseId(String caseId) {
        ...
        try {
            if (client.executeMethod(method) == HttpStatus.SC_OK) {
                return processSampleIdCheckResult(
    ...
    private static String getBAMCheckingURL(String caseId) {
        ...
        String url = GlobalProperties.getProperty(
            GlobalProperties.BROAD_BAM_CHECKING_URL
        );
        return (url != null && !url.isEmpty() && encodedCaseId != null)
            ? url.replace(SAMPLE_REGEX, encodedCaseId)
            : null;
    }
    ...
    private static String getNormalSampleId(String tumorSampleId) {
        ...
        String normalId = tumorSampleId.replace(
            TUMOR_BAM_SIGNATURE,
        ...
        if (tumorSampleIdPrefix != null) {
            String encodedTumorSampleId = getEncoded(
                tumorSampleIdPrefix + NORMAL_BAM_SIGNATURE
            );
            if (
                encodedTumorSampleId != null &&
    ...
    private static String getTumorSampleIdPrefix(String tumorSampleId) {
        ...
        return (tumorSignatureIndex > 0)
            ? tumorSampleId.substring(0, tumorSignatureIndex)
            : null;
    }
}

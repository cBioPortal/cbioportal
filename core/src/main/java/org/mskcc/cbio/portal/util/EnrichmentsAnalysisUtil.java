 ...
 * @date Mar 16, 2015
 */
public class EnrichmentsAnalysisUtil
    implements DaoGeneticAlteration.AlterationProcesser {
    ...
    private final String COL_NAME_PCT_ALTERED =
        "percentage of alteration in altered group";
    private final String COL_NAME_PCT_UNALTERED =
        "percentage of alteration in unaltered group";
    ...
    private final String COL_NAME_MEAN_ALTERED =
        "mean of alteration in altered group";
    private final String COL_NAME_MEAN_UNALTERED =
        "mean of alteration in unaltered group";
    private final String COL_NAME_STDEV_ALTERED =
        "standard deviation of alteration in altered group";
    private final String COL_NAME_STDEV_UNALTERED =
        "standard deviation of alteration in unaltered group";
    ...
    public ObjectNode processMutHm(
        long entrezGeneId,
        ...
        String cytoband = getCytoband(
            ((CanonicalGene) gene).getGeneticEntityId(),
        ...
        if (!(Arrays.asList(queriedGenes)).contains(geneName)) { //remove queried genes from result
            ...
            _datum.put(
                ...
                Integer.toString(countAltered(mapSampleValue, "altered")) +
                "////" +
                Double.toString(calcPct(mapSampleValue, "altered"))
            );
            _datum.put(
                COL_NAME_PCT_UNALTERED,
                Integer.toString(countAltered(mapSampleValue, "unaltered")) +
                "////" +
                Double.toString(calcPct(mapSampleValue, "unaltered"))
            );
            _datum.put(
                COL_NAME_RATIO,
                calcRatio(
                    calcPct(mapSampleValue, "altered"),
                    calcPct(mapSampleValue, "unaltered")
                )
            );
            _datum.put(COL_NAME_DIRECTION, "place holder"); //calculation is done by the front-end
            _datum.put(
                COL_NAME_P_VALUE,
                calcPval(mapSampleValue, geneticProfileStableId)
            );
            if (
                !(
                    calcPct(mapSampleValue, "altered") == 0.0 &&
        ...
        if (
            ...
            if (!(Arrays.asList(queriedGenes)).contains(geneName)) { //remove queried genes from result
                ...
                _datum.put(
                    COL_NAME_PCT_UNALTERED,
                    Integer.toString(
                        countAltered(mapSampleValue, "unaltered")
                    ) +
                    "////" +
                    Double.toString(calcPct(mapSampleValue, "unaltered"))
                );
            ...
            geneticAlterationType == GeneticAlterationType.MRNA_EXPRESSION
        ) {
            _datum.put(COL_NAME_GENE, geneName);
            _datum.put(COL_NAME_CYTOBAND, cytoband);
            _datum.put(
                COL_NAME_MEAN_ALTERED,
                calcMean(mapSampleValue, "altered", geneticProfileStableId)
            );
            _datum.put(
                COL_NAME_MEAN_UNALTERED,
                calcMean(mapSampleValue, "unaltered", geneticProfileStableId)
            );
            _datum.put(
                COL_NAME_STDEV_ALTERED,
                calcSTDev(mapSampleValue, "altered", geneticProfileStableId)
            );
            _datum.put(
                COL_NAME_STDEV_UNALTERED,
                calcSTDev(mapSampleValue, "unaltered", geneticProfileStableId)
            );
            _datum.put(
                COL_NAME_P_VALUE,
                calcPval(mapSampleValue, geneticProfileStableId)
            ...
            geneticAlterationType == GeneticAlterationType.PROTEIN_LEVEL
        ) {
            _datum.put(COL_NAME_GENE, geneName);
            _datum.put(COL_NAME_CYTOBAND, cytoband);
            _datum.put(
    ...
    ) { // group type: altered or unaltered
        switch (groupType) {
            case "altered":
        ...
        if (
            ...
            switch (groupType) {
                case "altered":
                    for (Integer alteredSampleId : alteredSampleIds) {
                        if (
                            singleGeneCaseValueMap.containsKey(alteredSampleId)
                        ) {
                            try {
                                if (
                                    Double.parseDouble(
                ...
                case "unaltered":
                    for (Integer unalteredSampleId : unalteredSampleIds) {
                        if (
                            singleGeneCaseValueMap.containsKey(
                                unalteredSampleId
                            )
                        ) {
                            try {
                ...
                if (profileStableId.indexOf("rna_seq") != -1) {
                    try {
                        alteredArray[_index_altered] =
                            Math.log(
                                Double.parseDouble(
                                    singleGeneCaseValueMap.get(alteredSampleId)
                                )
                            ) /
                            Math.log(2);
                    } catch (NumberFormatException e) {
            ...
            if (singleGeneCaseValueMap.containsKey(unalteredSampleId)) {
                if (profileStableId.indexOf("rna_seq") != -1) {
                    ...
                    try {
                        unalteredArray[_index_unaltered] =
                            Double.parseDouble(
                                singleGeneCaseValueMap.get(unalteredSampleId)
                            );
                    } catch (NumberFormatException e) {
                        e.getStackTrace();
                        //TODO: add proper handling of this situation
        ...
        for (Integer unalteredSampleId : unalteredSampleIds) {
            if (singleGeneCaseValueMap.containsKey(unalteredSampleId)) {
                if (
                    ...
                    String value = singleGeneCaseValueMap.get(
                        unalteredSampleId
                    );
                    if (value.equals("Non")) {
                        a += 1;
    ...
    private String getCytoband(
        int geneticEntityId,
        String geneticProfileStableId
    ) {
        try {
            int cancerStudyId = DaoGeneticProfile
                .getGeneticProfileByStableId(geneticProfileStableId)
                .getCancerStudyId();
            String genomeName = null;
            try {
                genomeName =
                    DaoCancerStudy
                        .getCancerStudyByInternalId(cancerStudyId)
                        .getReferenceGenome();
            } catch (NullPointerException ne) {
                genomeName = GlobalProperties.getReferenceGenomeName();
            }
            int genomeId = DaoReferenceGenome
                .getReferenceGenomeByGenomeName(genomeName)
                .getReferenceGenomeId();
            return DaoReferenceGenomeGene
                .getInstance()
                .getGene(geneticEntityId, genomeId)
                .getCytoband();

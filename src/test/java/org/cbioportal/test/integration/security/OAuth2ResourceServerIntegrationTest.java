...
@ContextConfiguration(
    initializers = {MyMysqlInitializer.class, MyOAuth2ResourceServerKeycloakInitializer.class})
@DirtiesContext
public class OAuth2ResourceServerIntegrationTest extends ContainerConfig {

  public static final String CBIO_URL_FROM_BROWSER =
      String.format("http://localhost:%d", CBIO_PORT);

  private static final String tokenUriPath = "/realms/cbio/protocol/openid-connect/token";

  @Test
  public void testAccessForbiddenForAnonymousUser() throws IOException {
    HttpHelper.HttpResponse response =
        HttpHelper.sendGetRequest(CBIO_URL_FROM_BROWSER + "/api/studies", null, null);
  ...
  public void testAccessForbiddenForFakeBearerToken() throws IOException {
    MockServerClient mockServerClient =
        new MockServerClient(
            mockServerContainer.getHost(), mockServerContainer.getFirstMappedPort());
            ...
            HttpRequest.request()
                ...
                .withBody(
                    StringBody.subString(
                        "refresh_token=" + URLEncoder.encode(encodedOfflineToken, "UTF-8"))))
        .respond(HttpResponse.response().withStatusCode(401));
    HttpHelper.HttpResponse response =
        HttpHelper.sendGetRequest(
            CBIO_URL_FROM_BROWSER + "/api/studies", encodedOfflineToken, null);
    assertEquals(401, response.code);
  }
    ...
    mockServerClient
        ...
        .respond(
            HttpResponse.response()
                .withBody(
                    "{\"access_token\": \"" + encodeWithoutSigning(accessTokenClaims) + "\"}"));
    HttpHelper.HttpResponse response =
        HttpHelper.sendGetRequest(
            CBIO_URL_FROM_BROWSER + "/api/studies", encodedOfflineToken, null);

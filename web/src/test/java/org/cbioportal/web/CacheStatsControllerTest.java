...
public class CacheStatsControllerTest {
    ...
    @Bean
    public static CacheStatisticsService cacheStatisticsService()
        throws CacheNotFoundException {
        CacheStatisticsService cacheStatisticsServiceMock = Mockito.mock(
            CacheStatisticsService.class
        );
        Mockito
            .when(
                cacheStatisticsServiceMock.getKeyCountsPerClass(
                    Mockito.anyString()
                )
            )
            .thenAnswer(
        ...
        Mockito
            .when(
                cacheStatisticsServiceMock.getKeysInCache(Mockito.anyString())
            )
            .thenAnswer(
                new Answer<List<String>>() {
        ...
        mockMvc
            .perform(
                MockMvcRequestBuilders
                    .get("/" + VALID_CACHE_ALIAS + "/keysInCache")
    ...
    public void testGetKeysInInvalidCache() throws Exception {
        mockMvc
            .perform(
                MockMvcRequestBuilders
                    .get("/" + INVALID_CACHE_ALIAS + "/keysInCache")
                    .accept(MediaType.APPLICATION_JSON)
            )
    ...
    public void testGetKeyCountsPerClass() throws Exception {
        mockMvc
            .perform(
                MockMvcRequestBuilders
                    .get("/" + VALID_CACHE_ALIAS + "/keyCountsPerClass")
                    .accept(MediaType.APPLICATION_JSON)
            )
            .andExpect(MockMvcResultMatchers.status().isOk())
            .andExpect(
    ...
    public void testGetKeyCountsPerClassInvalidCache() throws Exception {
        mockMvc
            .perform(
                MockMvcRequestBuilders
                    .get("/" + INVALID_CACHE_ALIAS + "/keyCountsPerClass")
                    .accept(MediaType.APPLICATION_JSON)
            )
            .andExpect(MockMvcResultMatchers.status().isNotFound());
    }

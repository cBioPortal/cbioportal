...
public class ClinicalDataEnrichmentController {
    ...
    public ResponseEntity<List<ClinicalDataEnrichment>> fetchClinicalEnrichments(
        @ApiParam(
            required = true,
            value = "List of altered and unaltered Sample/Patient IDs"
        ...
        @ApiIgnore @Valid @RequestAttribute( // prevent reference to this attribute in the swagger-ui interface. this attribute is needed for the @PreAuthorize tag above.
            required = false,
            value = "interceptedGroupFilter"
        ) GroupFilter interceptedGroupFilter
    ) {
        List<String> studyIds = interceptedGroupFilter
            .getGroups()
            .stream()
            .flatMap(
                group ->
        ...
        List<String> sampleIds = interceptedGroupFilter
            .getGroups()
            .stream()
            .flatMap(
                group ->
        ...
        List<List<Sample>> groupedSamples = interceptedGroupFilter
            .getGroups()
            .stream()
            .map(
                group ->
    ...
    private List<ClinicalDataEnrichment> fetchClinicalDataEnrichemnts(
        ...
        if (!groupedSamples.isEmpty()) {
            Set<String> studyIds = groupedSamples
                .stream()
                .flatMap(
                    samples ->
            ...
            List<ClinicalAttribute> clinicalAttributes = clinicalAttributeService.fetchClinicalAttributes(
                new ArrayList<String>(studyIds),
                "SUMMARY"
            );

            // remove all duplicate attributes
            Map<String, ClinicalAttribute> clinicalAttributesByUniqId = clinicalAttributes
                .stream()
                .collect(
                    Collectors.toMap(
                        c -> c.getAttrId() + c.getPatientAttribute(),
                        c -> c,
                        (e1, e2) -> e2
                    )
                );
            ...
            clinicalAttributes =
                new ArrayList<>(clinicalAttributesByUniqId.values());

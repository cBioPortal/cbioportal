...
public class GenesetHierarchyController {
    ...
    public ResponseEntity<List<GenesetHierarchyInfo>> fetchGenesetHierarchyInfo(
        @ApiParam(
            required = true,
            value = "Genetic Profile ID e.g. gbm_tcga_gsva_scores. The final hierarchy " +
        ...
        @ApiParam(
            required = false,
            value = "Identifier of pre-defined sample list with samples to query, e.g. brca_tcga_all"
        ) @RequestParam(required = false) String sampleListId,
        @ApiParam(
            required = false,
            value = "Fill this one if you want to specify a subset of samples:" +

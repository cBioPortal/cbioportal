...
public class GenePrefsParser {
    ...
    public static HashMap<String, GeneCounterPrefs> getGenePrefs(
        String userGeneListStr
    ) {
        ...
        for (String gene : genes) {
            geneCounterPrefMap.put(
                parseGeneSymbol(gene),
                parseGeneWithMicroEncoding(gene)
            );
        }
    ...
    private static void validateGeneWithMicroEncoding(
        String geneWithMicroEncoding
    ) {
        if (geneWithMicroEncoding.contains(":")) {
            String parts[] = geneWithMicroEncoding.split(":");
            if (parts.length != 2) {
                throw new IllegalArgumentException(
                    "Illegal String:  " + geneWithMicroEncoding
                );
            }

    ...
    public void calculate(
        ...
        if (!genePanelDataMap.isEmpty()) {
            genePanels =
                genePanelService.fetchGenePanels(
                    new ArrayList<>(genePanelDataMap.keySet()),
                    "DETAILED"
                );
        }
        ...
        Set<String> profiledCases = profiled
            ...
            .map(
                x ->
                    x.getStudyId() +
                    (countByPatients ? x.getPatientId() : x.getSampleId())
            )
        ...
        for (AlterationCountByGene alterationCountByGene : alterationCounts) {
            ...
            alterationCountByGene.setMatchingGenePanelIds(
                allPanels
                    .stream()
                    .map(panel -> panel.getStableId())
                    .collect(Collectors.toSet())
            );
            alterationCountByGene.setNumberOfProfiledCases(
                profiledCasesForGene.size()
            );
        }
    }
}

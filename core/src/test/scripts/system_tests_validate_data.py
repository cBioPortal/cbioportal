...
class ValidateDataSystemTester(unittest.TestCase):
    ...
    def assertFileGenerated(self, tmp_file_name, expected_file_name):
        ...
        original_maxDiff = self.maxDiff
        try:
            self.assertTrue(os.path.exists(tmp_file_name))
            with open(tmp_file_name, 'rU') as out_file, \
                    open(expected_file_name, 'rU') as ref_file:
                # generate a diff between the expected and the generated file,
                # even if the diff is longer than a few lines
                self.maxDiff = None
                self.assertMultiLineEqual(ref_file.read(), out_file.read())
            ...
            # restore the maximum length of diffs to be generated by assertions
            self.maxDiff = original_maxDiff

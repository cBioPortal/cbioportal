...
public class EnrichmentsJSON extends HttpServlet {
    ...
    protected void doPost(
        ...
        try {
            ...
            String _alteredCaseList = httpServletRequest.getParameter(
                "altered_case_id_list"
            );
            String[] alteredCaseList = _alteredCaseList.split("\\s+");
            String _unalteredCaseList = httpServletRequest.getParameter(
                "unaltered_case_id_list"
            );
            String[] unalteredCaseList = _unalteredCaseList.split("\\s+");
            String profileId = httpServletRequest.getParameter("profile_id");
            String[] queriedGenes = httpServletRequest
                .getParameter("gene_list")
                .split("\\s+");
            ...
            alteredSampleIds.retainAll(
                DaoSampleProfile.getAllSampleIdsInProfile(gpId)
            );
            ...
            if (profileType == GeneticAlterationType.MUTATION_EXTENDED) { //get only genes that has mutations -- performance concern
                Set<CanonicalGene> profileGeneSet = DaoMutation.getGenesInProfile(
                    gpId
                );
            ...
            final List synced_result = Collections.synchronizedList(
                new ArrayList()
            );
            synced_result.clear();
            final EnrichmentsAnalysisUtil dataProxy = new EnrichmentsAnalysisUtil(
                gpStableId,
                profileType,
                copyNumType,
                alteredSampleIds,
            ...
            if (profileType == GeneticAlterationType.MUTATION_EXTENDED) {
                final List<Integer> sampleIds = new ArrayList<>(
                    alteredSampleIds
                );
                ...
                final Map mutHm = mutationModelConverter.convertSampleIdAndEntrezGeneIdToMap(
                    mutationRepositoryLegacy.getSimplifiedMutations(
                        sampleIds,
                        intEntrezGeneIds,
                        gpId
                    )
                ...
                final List<Set<Long>> gene_short_lists = splitGenes(
                    bin,
                    nThread,
                    entrezGeneIds
                );
                        ...
                        new Thread(
                            new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    Set<Long> gene_short_list = gene_short_lists.get(
                                        result_index.get()
                                    );
                                    for (Long entrezGeneId : gene_short_list) {
                                        try {
                                            synced_result.add(
                                                dataProxy.processMutHm(
                                                    entrezGeneId,
                                                    (ArrayList) sampleIds,
                                                    mutHm
                                                )
                ...
                int genes_length = DaoGeneticAlteration.getGenesCountInProfile(
                    gpId
                );
                            ...
                            new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    try {
                                        ...
                                        synced_result.addAll(
                                            DaoGeneticAlteration.getProcessedAlterationData(
                                                gpId,
                                                offSet,
                                                dataProxy
                                            )
            ...
            BenjaminiHochbergFDR bhFDR = new BenjaminiHochbergFDR(
                originalPvalues
            );
            bhFDR.calculate();
    ...
    class pValueComparator implements Comparator {
        ...
        public int compare(Object o1, Object o2) {
            ...
            if (
                obj1.get("p-Value").asDouble() > obj2.get("p-Value").asDouble()
            ) return 1; else if (
                obj1.get("p-Value").asDouble() == obj2.get("p-Value").asDouble()
    ...
    private List<Set<Long>> splitGenes(
        int bin,
        int nThread,
        ...
        for (int i = 0; i < nThread; i++) {
            ...
            if ((i + 1) * bin >= all_genes.size()) {
                gene_short_list =
                    new ArrayList<>(
                        all_genes.subList(i * bin, all_genes.size() - 1)
                    );
                gene_short_list.add(all_genes.get(all_genes.size() - 1));
            } else {
                gene_short_list =
                    new ArrayList<>(all_genes.subList(i * bin, (i + 1) * bin));
            }

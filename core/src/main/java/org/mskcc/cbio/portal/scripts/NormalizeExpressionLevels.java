...
public class NormalizeExpressionLevels {
    ...
    private static HashMap<Long, ArrayList<String[]>> geneCopyNumberStatus =
        null;
    ...
    private static final Logger LOG = LoggerFactory.getLogger(
        NormalizeExpressionLevels.class
    );
    ...
    public static void driver(String[] args) throws RuntimeException {
        ...
        if (args.length != 4 && args.length != 5) {
            fatalError(
                "incorrect number of arguments. Arguments should be '<copy_number_file> <expression_file> <output_file> <normal_sample_suffix> [<min_number_of_diploids>]'."
            );
        }
        ...
        if (args.length == 5) {
            try {
                ...
                fatalError(
                    "incorrect arguments. 'min_number_of_diploids', was entered as " +
                    args[4] +
     ...
     * @return
     */
    private static CanonicalGene resolveCanonicalGene(
        String[] values,
        Integer entrezGeneIdIndex,
        Integer hugoGeneSymbolIndex
    ) {
        ...
        if (entrezGeneIdIndex != -1) {
            try {
                gene =
                    daoGeneOptimized.getGene(
                        Long.parseLong(values[entrezGeneIdIndex])
                    );
            } catch (NumberFormatException e) {}
        }
        if (gene == null && hugoGeneSymbolIndex != -1) {
            gene =
                daoGeneOptimized.getNonAmbiguousGene(
                    values[hugoGeneSymbolIndex]
                );
        }
        ...
        if (gene == null) {
            ...
            message +=
                ...
                (
                    entrezGeneIdIndex != -1
                        ? values[entrezGeneIdIndex]
                        : "unavailable,  "
                );
            message +=
                "hugo symbol= " +
                (
                    hugoGeneSymbolIndex != -1
                        ? values[hugoGeneSymbolIndex]
                        : "unavailable"
                );
            warning(message);
    ...
    private static void computeZScoreXP(String file) {
        ...
        try {
            ...
            fatalError(
                "cannot open <output_file> '" + zScoresFile + "' for writing."
            );
        }
        try {
        ...
        try {
            ...
            int entrezGeneIdIndex = findColumnIndex(
                values,
                ENTREZ_GENE_ID_COLUMN
            );
            int hugoGeneSymbolIndex = findColumnIndex(
                values,
                HUGO_SYMBOL_COLUMN
            );
            ...
            if (NO_POSITION == firstSamplePosition) {
                fatalError(
                    "no sample id in <expression_file>, '" + file + "'."
                );
            }
            ...
            while ((line = in.readLine()) != null) {
                ...
                CanonicalGene gene = resolveCanonicalGene(
                    values,
                    entrezGeneIdIndex,
                    hugoGeneSymbolIndex
                );
                ...
                if (
                    geneCopyNumberStatus == null ||
                    geneCopyNumberStatus.containsKey(gene.getEntrezGeneId())
                ) {
                    genesFound++;
                    ...
                    ArrayList<String[]> cnStatus = geneCopyNumberStatus == null
                        ? null
                        : geneCopyNumberStatus.get(gene.getEntrezGeneId());
                    double[] zscores = getZscore(
                        tumorSampleExpressions,
                        cnStatus
                    );
                    ...
                    if (zscores != null) {
                        ...
                        for (int k = 0; k < zscores.length; k++) {
                            ...
                            if (zscores[k] != Double.NaN) {
                                // limit precision
                                outputLine.add(
                                    String.format(Locale.US, "%.4f", zscores[k])
                                );
                            } else {
            ...
            if (0 == genesFound) {
                fatalError(
                    "none of the genes in the expression file '" +
                    file +
                    "' were in the copy number file."
                );
            }
            if (0 == rowsWithSomeDiploidCases) {
                fatalError(
                    "no genes with at least " +
                    MIN_NUM_DIPLOIDS +
                    " diploid cases found. Check copy number file."
                );
            }
        } catch (IOException e) {
    ...
    private static int getFirstDataColumn(String[] values) {
        ...
        for (int i = 0; i < values.length; i++) {
            if (
                !values[i].equalsIgnoreCase("GENE SYMBOL") &&
     ...
     */
    private static double[] getZscore(
        ArrayList<String[]> xp,
        ArrayList<String[]> cn
    ) {
        double[] z = null;
        ...
        for (int i = 0; i < xp.size(); i++) {
            ...
            if (cn == null || diploidSamples.contains(xp.get(i)[0])) {
                ...
                if (
                    xp.get(i)[1].compareTo("NA") != 0 &&
                    xp.get(i)[1].compareTo("NaN") != 0 &&
    ...
    public static double[] getZ(
        ...
        if (0.0d == std) {
            ...
            fatalError(
                "cannot normalize relative to distribution with standard deviation of 0.0."
            );
        }
        for (int i = 0; i < xp.size(); i++) {
            if (
     ...
     */
    public static HashMap<Long, ArrayList<String[]>> readCopyNumberFile(
        String file
    ) {
        ...
        try {
            ...
            fatalError(
                "cannot open copy number file '" + file + "' for reading."
            );
        }
        ...
        try {
            ...
            if (entrezGeneIdIndex == -1 && hugoGeneSymbolIndex == -1) {
                fatalError(
                    "Copy number file header must contain at least one of the following: " +
                    ENTREZ_GENE_ID_COLUMN +
                    ", " +
                    HUGO_SYMBOL_COLUMN
                );
            ...
            if (NO_POSITION == firstSamplePosition) {
                fatalError(
                    "no sample id contains in <CopyNumberFile>, '" + file + "'."
                );
            }
            ...
            for (int i = firstSamplePosition; i < SAMPLES; i++) {
                ...
                if (tempSamplesNames.contains(samples[i])) {
                    fatalError(
                        "multiple columns with same truncated sample id of " +
                        samples[i] +
                        " in <CopyNumberFile>, '" +
                        file +
                        "'."
            ...
            LOG.info(
                file + ":  " + (SAMPLES - firstSamplePosition) + " SAMPLES"
            );
    ...
    */
    public static boolean isTCGANormal(String name) {
        String suffix = "";
        int dash = 0;
        search:for (int i = 0; i < name.length(); i++) {
            if (name.charAt(i) == '-') {
                dash++;
            }
            if (dash == 3) {

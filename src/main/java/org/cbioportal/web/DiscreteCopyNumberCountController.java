...
public class DiscreteCopyNumberCountController {
  ...
  @ApiResponse(
      ...
      description = "OK",
      content =
          @Content(array = @ArraySchema(schema = @Schema(implementation = CopyNumberCount.class))))
  public ResponseEntity<List<CopyNumberCount>> fetchCopyNumberCounts(
      ...
      @Parameter(required = true, description = "List of copy number count identifiers")
          ...
          @RequestBody
          List<CopyNumberCountIdentifier> copyNumberCountIdentifiers)
    ...
    return new ResponseEntity<>(
        discreteCopyNumberService.fetchCopyNumberCounts(
            molecularProfileId, entrezGeneIds, alterations),
        HttpStatus.OK);
  }
}

...
public final class MutationKeywordUtils {
    ...
    public static String guessOncotatorMutationKeyword(
        String aminoAcidChange,
        String mutationType
    ) {
        if (
            mutationType.equals("Nonsense_Mutation") ||
        ...
        if (mutationType.equals("In_Frame_Ins")) {
            if (
                aminoAcidChange.matches("[0-9]+_[0-9]+[A-Z]*((ins)|(>)).*\\*.*")
            ) {
                // insertion of *
                return "truncating";
            }
            ...
            if (
                aminoAcidChange.matches("1_1M>?[A-LN-Z][A-Z]*") ||
                aminoAcidChange.matches("1_2ins[A-LN-Z][A-Z]*")
            ) { // insertion in the start position
                return "truncating";
            }

            Pattern p = Pattern.compile(
                "([0-9]+)_([0-9]+)[A-Z]*((ins)|(>))[A-Z]+"
            );
            Matcher m = p.matcher(aminoAcidChange);
            if (m.find()) {
        ...
        if (mutationType.equals("In_Frame_Del")) {
            if (
                aminoAcidChange.matches("M[A-Z]*1del") ||
            ...
            if (
                aminoAcidChange.matches("[A-Z]*\\*[0-9]+del") ||
            ...
            if (m.matches()) {
                return (
                    ...
                    m.group(2) +
                    "-" +
                    (Integer.parseInt(m.group(2)) + m.group(1).length() - 1) +
                    " deletion"
                );
            }
    ...
    public static String guessCosmicKeyword(String aminoAcidChange) {
        ...
        if (m.matches()) {
            int n1 =
                Integer.parseInt(m.group(2)) - Integer.parseInt(m.group(1)) + 1;

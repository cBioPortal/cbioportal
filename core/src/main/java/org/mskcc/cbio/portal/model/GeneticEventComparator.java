...
public class GeneticEventComparator implements Comparator<Object> {
    ...
    private HashMap<mutations, Integer> mutationsSortOrderHash = new HashMap<mutations, Integer>(); // load this from mutationsSortOrder
    ...
    public GeneticEventComparator(
        ArrayList<EnumSet<CNA>> CNAsortOrder,
    ...
    private void initSortOrder() {
        ...
        for (mutations aMutations : mutations.values()) {
            if (!mutationsSortOrderHash.containsKey(aMutations)) {
                throw new IllegalArgumentException(
                    "mutations sets missing: " + aMutations
                );
            }
        }
    }
    ...
    public int compare(Object o1, Object o2) {
        if (
            !(o1 instanceof GeneticEventImpl && o2 instanceof GeneticEventImpl)
        ) {
        ...
        // CNA
        if (
            cnaSortOrderHash.get(ge1.getCnaValue()).intValue() <
            cnaSortOrderHash.get(ge2.getCnaValue()).intValue()
        ) {
            return -1;
        }
        if (
        ...
        // MRNA
        if (
            mrnaSortOrderHash.get(ge1.getMrnaValue()).intValue() <
            mrnaSortOrderHash.get(ge2.getMrnaValue()).intValue()
        ) {
            return -1;
        }
        if (
        ...
        // RPPA
        if (
            rppaSortOrderHash.get(ge1.getRPPAValue()).intValue() <
            rppaSortOrderHash.get(ge2.getRPPAValue()).intValue()
        ) {
            return -1;
        }
        if (
        ...
        // mutations
        if (
            mutationsSortOrderHash.get(ge1.getMutationValue()).intValue() <
            mutationsSortOrderHash.get(ge2.getMutationValue()).intValue()
        ) {
            return -1;
        }
        if (

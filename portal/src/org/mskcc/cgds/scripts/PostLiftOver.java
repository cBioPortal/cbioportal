...
public class PostLiftOver {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (sizeMismatch(originialFile, mappedFile, unmappedFile)) {
                System.out.println(
                    "Input sizes do not match! Error while lifting over?"
                );
            } else {
                updatePositions(
                    originialFile,
    ...
    public static void updatePositions(
        ...
        throws IOException {
        BufferedReader sourceIn = new BufferedReader(new FileReader(inputFile));
        ...
        BufferedReader auxIn = new BufferedReader(new FileReader(auxFile));
        ...
        while ((sourceLine = sourceIn.readLine()) != null) {
            ...
            if (modifiedRow != null && sourceRow == modifiedRow) {
                modified = true;
            ...
            if (unmappedLine != null && isUnmapped(record, unmappedLine)) {
                System.out.println(
                    "[warning] cannot lift over (line:" +
                    sourceRow +
                    "): " +
                    "[gene:" +
            ...
            // record is lifted over successfully, update the positions
                ...
                if (!mappedParts[0].equals("chr" + sourceChr)) {
                    ...
                    System.out.println(
                        "[warning] chromosome numbers mismatch (gene:" +
                        record.getHugoGeneSymbol() +
                        ")"
                    );
                    System.out.println(
                    ...
                    System.out.println(
                        "mapped(line:" + mappedRow + "): " + mappedLine
                    );
                ...
                for (int i = 0; i < sourceParts.length; i++) {
                    ...
                    } else if (chrChanged && util.getChrIndex() == i) {
     ...
     * @throws IOException	if an IO error occurs
     */
    private static boolean sizeMismatch(
        String inputFile,
        String mappedFile,
        String unmappedFile
    )
     ...
     */
    private static boolean isUnmapped(MafRecord record, String unmappedLine) {
     ...
     * @throws IOException	if an IO error occurs
     */
    private static String getUnmappedLine(BufferedReader unmappedIn)
        throws IOException {
        String unmappedLine = unmappedIn.readLine();

        while (unmappedLine != null && unmappedLine.trim().startsWith("#")) {
     ...
     * @throws IOException	if an IO error occurs
     */
    private static Integer getNextModRow(BufferedReader auxIn)
        throws IOException {

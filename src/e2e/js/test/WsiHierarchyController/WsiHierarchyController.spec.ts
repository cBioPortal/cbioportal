...
const config = {
    ...
    authSecret:
        ...
        "local-development-wsi-secret-change-me-32chars",
    ...
    return hierarchy.sampleGroups.flatMap((sample) =>
        sample.parts.flatMap((part) =>
            part.blocks.flatMap((block) =>
                block.slides.map((slide) => slide.imageId),
            ),
        ),
    );
}
...
function makeToken(
    ...
    const payload = base64url({
        ...
        image_id: imageId,
        wsi_auth_version: 2,
        tile_source_sha256: crypto
            .createHash("sha256")
            .update(source)
            .digest("hex"),
        thumbnail_source_sha256: crypto
            .createHash("sha256")
            .update(thumbnail)
            .digest("hex"),
        thumbnail_width: 256,
...
async function statusOf(request: Promise<any>): Promise<number> {
    try {
        ...
        expect(
            error.response,
            "request did not return an HTTP response",
...
describe("Authenticated WsiHierarchyController and tile contract", () => {
    ...
    const unmatchedTileSlideId =
        config.unmatchedTileSlideId || unmatchedSlide.imageId;
    ...
    frontendIt(
        "matches the authenticated backend hierarchy through the frontend proxy",
        async function () {
            const requestOptions = await authenticatedRequestOptions();
            ...
            const [backendResponse, frontendResponse] = await Promise.all([
                ...
                axios.get<PatientHierarchy>(
                    frontendHierarchyUrl,
    ...
    tileIt(
        "binds tiles and thumbnails to the exact source in the capability",
        async function () {
            ...
            expect(
                (
                    await axios.get(tileUrl, {
                        ...sourceRequest(token, source),
    ...
    tileIt(
        "rejects missing, invalid, expired, over-maximum, and wrong-audience tokens",
        async function () {
            const source = "file:///app/testdata/CMU-1-Small-Region.svs";
            ...
            expect(
                await statusOf(
                    axios.get(pathToTest, { headers: sourceHeaders }),
                ),
            ).to.equal(401);
            expect(
                await statusOf(
                    axios.get(pathToTest, sourceRequest("not-a-jwt", source)),
                ),
            ).to.equal(401);
            expect(
                await statusOf(
    ...
    tileIt("accepts a replacement token after refresh", async function () {
        ...
        expect(
            (
                await axios.get(
                    `${config.tileServerUrl}/tiles/zxy/0/0/0`,
                    sourceRequest(second.data.accessToken, source),
                )
            ).status,
        ).to.equal(200);
    });
    ...
    hierarchyTileConsistencyIt(
        "reports tile-serving capability consistently with live behavior",
        async function () {
            const slides = fixture.hierarchy.sampleGroups
                .flatMap((sample) => sample.parts)
                .flatMap((part) => part.blocks)
                .flatMap((block) => block.slides);
            const servableSlides = slides.filter(
                (slide) => slide.canServeTiles === true,
            );
            const nonServableSlides = slides.filter(
                (slide) => slide.canServeTiles === false,
            );
            expect(
                servableSlides.map((slide) => slide.imageId),
            ).to.have.members([blockSlide.imageId, partSlide.imageId]);
            expect(
                nonServableSlides.map((slide) => slide.imageId),
            ).to.deep.equal([unmatchedSlide.imageId]);
            const requestOptions = await authenticatedRequestOptions();

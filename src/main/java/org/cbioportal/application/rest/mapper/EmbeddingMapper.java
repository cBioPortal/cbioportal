...
/**
 * MapStruct mapper that converts the domain-level {@link Embedding} (assembled from raw repository
 * rows) into the {@link EmbeddingDTO} returned by the REST API. Several target fields are pulled up
 * from the nested {@code embeddingDefinition}, since the DTO is intentionally flat.
 */
...
public interface EmbeddingMapper {
  ...
  /**
   * @param embedding the domain object combining study/sample counts, the embedding definition, and
   *     per-point data

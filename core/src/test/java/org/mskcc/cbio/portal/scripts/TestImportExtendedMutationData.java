...
public class TestImportExtendedMutationData {
    ...
    public void setUp() throws DaoException {
        ...
        geneticProfile.setGeneticAlterationType(
        ...
        geneticProfileId =
            DaoGeneticProfile
                .getGeneticProfileByStableId("test")
                .getGeneticProfileId();
        ...
        geneticProfile2.setProfileName(
            "test duplicate mutation events profile"
        );
        geneticProfile2.setStableId("test_dup_mut_events");
        geneticProfile2.setGeneticAlterationType(
            GeneticAlterationType.MUTATION_EXTENDED
        );
        geneticProfile2.setDatatype("test");
        DaoGeneticProfile.addGeneticProfile(geneticProfile2);
        geneticProfileIdDupEvents =
            DaoGeneticProfile
                .getGeneticProfileByStableId("test_dup_mut_events")
                .getGeneticProfileId();
    ...
    @Test
    public void testImportExtendedMutationDataExtended()
        ...
        int sampleId = DaoSample
            .getSampleByCancerStudyAndSampleId(studyId, "TCGA-AA-3664-01")
            .getInternalId();
     ...
     */
    @Test
    public void testImportExtendedMutationDataSwissprotName()
        throws IOException, DaoException {
        loadStudyContext1();
        MySQLbulkLoader.bulkLoadOn();

        File file = new File(
            "src/test/resources/data_mutations_swissprotname.maf"
        );
        ImportExtendedMutationData parser = new ImportExtendedMutationData(
            file,
            geneticProfileId,
        ...
        int sampleId = DaoSample
            .getSampleByCancerStudyAndSampleId(studyId, "TCGA-A2-A0CR-01")
            .getInternalId();
        ExtendedMutation m = DaoMutation
            .getMutations(geneticProfileId, sampleId, 64581)
            .get(0);
     ...
     */
    @Test
    public void testImportExtendedMutationDataSwissprotAccession()
        throws IOException, DaoException {
        loadStudyContext1();
        MySQLbulkLoader.bulkLoadOn();

        File file = new File(
            "src/test/resources/data_mutations_swissprotaccession.maf"
        );
        ImportExtendedMutationData parser = new ImportExtendedMutationData(
            file,
            geneticProfileId,
    ...
    private void checkSwissprotLoaded() throws DaoException {
        ...
        sampleId =
            DaoSample
                .getSampleByCancerStudyAndSampleId(studyId, "TCGA-A2-A0T5-01")
                .getInternalId();
        validateMutationAminoAcid(geneticProfileId, sampleId, 3339, "X4137?");
        m = DaoMutation.getMutations(geneticProfileId, sampleId, 3339).get(0);
        assertEquals("P98160", m.getOncotatorUniprotAccession());
        validateMutationAminoAcid(geneticProfileId, sampleId, 54407, "T32P");
        m = DaoMutation.getMutations(geneticProfileId, sampleId, 54407).get(0);
     ...
     */
    @Test
    public void testImportExtendedMutationDataOncotated()
        throws IOException, DaoException {
     ...
     */
    @Test
    public void testImportExtendedMutationDataExtendedCustomFiltering()
        throws IOException, DaoException {
        MySQLbulkLoader.bulkLoadOn();

        File file = new File("src/test/resources/data_mutations_extended.txt");
        Set<String> customFiltering = new HashSet<String>(
            Arrays.asList("Missense_Mutation", "Nonsense_Mutation")
        );
        ImportExtendedMutationData parser = new ImportExtendedMutationData(
            file,
            geneticProfileId,
     ...
     */
    @Test
    public void testImportExtendedMutationDataExtendedNoFiltering()
        throws IOException, DaoException {
        MySQLbulkLoader.bulkLoadOn();
    ...
    private void validateMutationAminoAcid(
        int geneticProfileId,
        Integer sampleId,
        long entrezGeneId,
        String expectedAminoAcidChange
    )
        throws DaoException {
        ...
        assertEquals(
            expectedAminoAcidChange,
            mutationList.get(0).getProteinChange()
        );
    }
    ...
    private void rejectSilentLOHIntronWildtype() throws DaoException {
        ...
        assertEquals(
            0,
            DaoMutation.getMutations(geneticProfileId, sampleId, 114548).size()
        ); // silent
        assertEquals(
            0,
            DaoMutation.getMutations(geneticProfileId, sampleId, 343035).size()
        ); // LOH
        assertEquals(
            0,
            DaoMutation.getMutations(geneticProfileId, sampleId, 80114).size()
        ); // Wildtype
        assertEquals(
            0,
            DaoMutation.getMutations(geneticProfileId, sampleId, 219736).size()
        ); // Wildtype
        assertEquals(
            0,
            DaoMutation.getMutations(geneticProfileId, sampleId, 6609).size()
        ); // Intron
    }
    ...
    private void checkGermlineMutations() throws DaoException {
        ...
        assertEquals(
            1,
            DaoMutation.getMutations(geneticProfileId, sampleId, 64581).size()
        );
        ...
        assertEquals(
            1,
            DaoMutation.getMutations(geneticProfileId, sampleId, 50839).size()
        );
    ...
    private void checkMutationCounts() throws DaoException {
        ...
        CancerStudy study = DaoCancerStudy.getCancerStudyByStableId(
            studyStableId
        );
        ...
        List<ClinicalData> clinicalData = DaoClinicalData.getSampleData(
            study.getInternalId(),
            new ArrayList<String>(Arrays.asList("TCGA-AA-3664-01")),
            clinicalAttribute
        );
    ...
    private void rejectMissenseAndNonsenseMutations() throws DaoException {
        ...
        assertEquals(
            0,
            DaoMutation.getMutations(geneticProfileId, sampleId, 63967).size()
        ); // Missense
        assertEquals(
            0,
            DaoMutation.getMutations(geneticProfileId, sampleId, 79699).size()
        ); // Missense
        assertEquals(
            0,
            DaoMutation.getMutations(geneticProfileId, sampleId, 204219).size()
        ); // Missense
        assertEquals(
            0,
            DaoMutation.getMutations(geneticProfileId, sampleId, 51259).size()
        ); // Nonsense
        assertEquals(
            0,
            DaoMutation.getMutations(geneticProfileId, sampleId, 84902).size()
        ); // Nonsense
    }
     ...
     */
    @Test
    public void testImportExtendedMutationDataExtendedWithSharedMutationEvent()
        throws IOException, DaoException {
        // import maf
        MySQLbulkLoader.bulkLoadOn();
        File file = new File(
            "src/test/resources/data_mutations_extended_duplicate_events.txt"
        );
        ImportExtendedMutationData parser = new ImportExtendedMutationData(
            file,
            geneticProfileIdDupEvents,

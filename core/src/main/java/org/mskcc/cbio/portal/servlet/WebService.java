...
public class WebService extends HttpServlet {
    ...
    public static final String SUPPRESS_MONDRIAN_HEADER =
        "suppress_mondrian_header";
    ...
    public void processClient(
        ...
        try {
            ...
            if (
                cmd.equals("getCancerStudies") || cmd.equals("getCancerTypes")
            ) {
            ...
            if (
                null ==
                WebserviceParserUtils.getCancerStudyId(httpServletRequest) &&
                null ==
                httpServletRequest.getParameter(WebService.CASE_SET_ID) &&
                null ==
                ...
                outputError(
                    writer,
                    "No cancer study (cancer_study_id), or genetic profile (genetic_profile_id) " +
            ...
            HashSet<String> cancerStudyIDs = WebserviceParserUtils.getCancerStudyIDs(
                httpServletRequest
            );
            ...
            for (String cancerStudyID : cancerStudyIDs) {
                if (
                    ...
                    outputError(
                        writer,
                        "The cancer study identified by the request (" +
                        cancerStudyID +
                ...
                if (
                    accessControl
                        .isAccessibleCancerStudy(cancerStudyID)
                        .size() !=
                    1
                ) {
                    outputError(
                    ...
                    if (ud != null) {
                        logger.info(
                            "WebService.processClient: Query initiated by user: " +
                            ud.getUsername()
                        );
                    }
                }
    ...
    private void getProteinArrayInfo(
        HttpServletRequest httpServletRequest,
        PrintWriter writer
    )
        throws DaoException, ProtocolException {
        ...
        writer.print(
            GetProteinArrayData.getProteinArrayInfo(
                cancerStudyId,
                targetGeneList,
                type
            )
    ...
    private void getProteinArrayData(
        HttpServletRequest httpServletRequest,
        PrintWriter writer
    )
        throws DaoException, ProtocolException {
        ...
        if (arrayId == null || arrayId.length() == 0) {
            cancerStudyId =
                WebserviceParserUtils
                    .getCancerStudyIDs(httpServletRequest)
                    .iterator()
                    .next();
        }
        List<String> targetSampleIds = null;
        if (
            null != httpServletRequest.getParameter(CASE_LIST) ||
            null != httpServletRequest.getParameter(CASE_SET_ID) ||
            null != httpServletRequest.getParameter(CASE_IDS_KEY)
        ) targetSampleIds =
            WebserviceParserUtils.getSampleIds(httpServletRequest);

        String arrayInfo = httpServletRequest.getParameter("array_info");
        boolean includeArrayInfo =
            arrayInfo != null && arrayInfo.equalsIgnoreCase("1");
        writer.print(
            GetProteinArrayData.getProteinArrayData(
                cancerStudyId,
    ...
    private void getTypesOfCancer(PrintWriter writer)
        throws DaoException, ProtocolException {
        String out = GetTypesOfCancer.getTypesOfCancer();
        writer.print(out);
    }

    private void getCancerStudies(
        HttpServletRequest httpServletRequest,
        PrintWriter writer
    )
        throws DaoException, ProtocolException {
    ...
    private void getGeneticProfiles(
        HttpServletRequest httpServletRequest,
        PrintWriter writer
    )
        throws DaoException {
        String cancerStudyStableId = WebserviceParserUtils.getCancerStudyId(
            httpServletRequest
        );
        if (cancerStudyStableId == null) {
            outputMissingParameterError(writer, CANCER_STUDY_ID);
        } else {
            String out = GetGeneticProfiles.getGeneticProfilesAsTable(
                cancerStudyStableId
            );
            writer.print(out);
        }
    ...
    private void getSampleLists(
        ...
        if (cancerStudyStableId == null) {
            ...
            String out = GetSampleLists.getSampleListsAsTable(
                cancerStudyStableId
            );
            writer.print(out);
        }
    ...
    private void getProfileData(HttpServletRequest request, PrintWriter writer)
        ...
        ArrayList<String> geneticProfileIdList = WebserviceParserUtils.getGeneticProfileId(
            request
        );
        ...
        Boolean suppressMondrianHeader = Boolean.parseBoolean(
            request.getParameter(SUPPRESS_MONDRIAN_HEADER)
        );
        GetProfileData getProfileData = new GetProfileData(
            geneticProfileIdList,
            targetGeneList,
            sampleList,
            suppressMondrianHeader
        );
    ...
    private void getClinicalData(
        ...
        if (cancerStudyId == null) {
            Set<String> cancerStudyIds = WebserviceParserUtils.getCancerStudyIDs(
                request
            );
            if (cancerStudyIds.size() != 1) {
                writer.print(
                    "The cmd only support one and only one cancer study."
                );
                return;
            }
            cancerStudyId = cancerStudyIds.iterator().next();
        }
        int internalCancerStudyId = DaoCancerStudy
        ...
        if (format == null || "txt".equals(format)) { // default to txt if format parameter is not specified
            if (attrId == null) {
                writer.print(
                    GetClinicalData.getTxt(internalCancerStudyId, sampleIds)
                );
            } else {
                if (sampleIds.size() != 1) {
            ...
            if (attrId == null) {
                JSONObject.writeJSONString(
                    GetClinicalData.getJSON(internalCancerStudyId, sampleIds),
                    writer
                );
            } else {
            ...
            throw new ProtocolException(
                "please specify the format, i.e. format=txt OR format=json"
            );
        }
    }

    private void getSampleAndPatientMappingTable(
        HttpServletRequest request,
        PrintWriter writer
    )
        throws DaoException, ProtocolException, IOException {
    ...
    private void getMutSig(HttpServletRequest request, PrintWriter writer)
        ...
        if (qValueThreshold == null && geneList == null) {
            ...
            StringBuffer output = GetMutSig.getMutSig(
                cancerID,
                qValueThreshold,
                true
            );
            ...
            writer.print(
                "Invalid command. Please input a valid Q-Value Threshold, or Gene List."
            );
        }
    }

    private void getMutationData(
        HttpServletRequest request,
        PrintWriter writer
    )
        ...
        for (String geneticProfileId : geneticProfileIdList) {
            ArrayList<String> targetGeneList = getGeneList(request);
            String out = GetMutationData.getProfileData(
                geneticProfileId,
                targetGeneList,
                sampleList
            );
    ...
    private void validateRequestForProfileOrMutationData(
        HttpServletRequest request
    )
        ...
        if (geneticProfileIdStr == null || geneticProfileIdStr.length() == 0) {
            throw new ProtocolException(
                "Missing Parameter:  " + GENETIC_PROFILE_ID
            );
        }
    ...
    private void outputMissingParameterError(
        PrintWriter writer,
        String missingParameter
    ) {
        outputError(
            writer,
            "you must specify a " + missingParameter + " parameter."
        );
    }
    ...
    private void verifyDbConnection() {
        if (DaoCancerStudy.getCount() == 0) {
            System.err.println(
                "****  Fatal Error in CGDS.  Could not connect to " + "database"
            );
        }
    }

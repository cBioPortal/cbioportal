...
public class SignificantlyMutatedGeneMyBatisRepositoryTest {
    ...
    public void getSignificantlyMutatedGenesIdProjection() throws Exception {
        List<MutSig> result = significantlyMutatedGeneMyBatisRepository.getSignificantlyMutatedGenes(
            "study_tcga_pub",
            "ID",
            null,
            null,
    ...
    public void getSignificantlyMutatedGenesSummaryProjection()
        ...
        Assert.assertEquals(
            "study_tcga_pub",
            mutSig.getCancerStudyIdentifier()
        );
    ...
    public void getSignificantlyMutatedGenesSummaryProjectionPValueSort()
        ...
        Assert.assertEquals(2, result.size());
        Assert.assertEquals(
            new BigDecimal("0.000000012"),
            result.get(0).getpValue()
        );
        Assert.assertEquals(
            new BigDecimal("0.00000315"),
            result.get(1).getpValue()
        );
    }

    @Test
    public void getMetaSignificantlyMutatedGenes() throws Exception {
        BaseMeta result = significantlyMutatedGeneMyBatisRepository.getMetaSignificantlyMutatedGenes(
            "study_tcga_pub"
        );

...
public class MutationSpectrumControllerTest {
  ...
  @MockitoBean private MutationSpectrumService mutationSpectrumService;
  ...
  public void fetchMutationSpectrums() throws Exception {
    ...
    Mockito.when(
            mutationSpectrumService.fetchMutationSpectrums(
                TEST_MOLECULAR_PROFILE_ID, Arrays.asList(TEST_SAMPLE_ID_1, TEST_SAMPLE_ID_2)))
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.post(
                    "/api/molecular-profiles/test_molecular_profile_id/mutation-spectrums/fetch")
                .with(csrf())

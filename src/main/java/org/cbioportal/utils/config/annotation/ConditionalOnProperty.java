...
public @interface ConditionalOnProperty {
  ...
  /**
   * havingValue string [ ]. Value or values compared with application property (can be string or
   * string[]).
   */
  String[] havingValue() default {};

  /**
   * isNot boolean. When true a bean will be created when the application property is not equal to
   * any of the values passed by havingValues.
   */
  boolean isNot() default false;

  /**
   * matchIfMissing boolean. Default behavior when application property is not defined. Default is
   * false, which means the bean is not created when property is undefined.
   */

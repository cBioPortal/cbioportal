...
public class UuidDataAccessTokenServiceImpl implements DataAccessTokenService {
    ...
    private static final Log log = LogFactory.getLog(
        UuidDataAccessTokenServiceImpl.class
    );
    ...
    public DataAccessToken createDataAccessToken(
        ...
        if (getNumberOfTokensForUsername(username) >= maxNumberOfAccessTokens) {
            if (allowRevocationOfOtherTokens) {
                ...
                throw new MaxNumberTokensExceededException(
                    "User has reached max number of tokens allowed (" +
    ...
    public List<DataAccessToken> getAllDataAccessTokens(String username) {
        List<DataAccessToken> allDataAccessTokens = dataAccessTokenRepository.getAllDataAccessTokensForUsername(
            username
        );
    ...
    public DataAccessToken getDataAccessToken(String username) {
        ...
        DataAccessToken newestDataAccessToken = allDataAccessTokens.get(
            allDataAccessTokens.size() - 1
        );
    ...
    public DataAccessToken getDataAccessTokenInfo(String token) {
        DataAccessToken dataAccessToken = dataAccessTokenRepository.getDataAccessToken(
            token
        );
        if (dataAccessToken == null) {
            throw new TokenNotFoundException(
                "Specified token " + token + " does not exist"
            );
        }
        return dataAccessToken;
    }
    ...
    public void revokeAllDataAccessTokens(String username) {
        dataAccessTokenRepository.removeAllDataAccessTokensForUsername(
            username
        );
    }
    ...
    public Boolean isValid(String dataAccessToken) {
        ...
        try {
            storedDataAccessToken =
                dataAccessTokenRepository.getDataAccessToken(dataAccessToken);
        } catch (Exception e) {
            log.error(
                "Error retrieving data access token, " +
                dataAccessToken +
                " from token store"
            );
        ...
        if (
            storedDataAccessToken == null ||
            storedDataAccessToken.getExpiration().before(currentDate)
        ) {
            return Boolean.FALSE;
        }
        return Boolean.TRUE;
    ...
    private void revokeOldestDataAccessTokenForUsername(String username) {
        ...
        dataAccessTokenRepository.removeDataAccessToken(
            oldestDataAccessToken.getToken()
        );
    }
}

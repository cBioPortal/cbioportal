...
public class NamespaceDataController {
    ...
    @RequestMapping(value = "/namespace-data/fetch", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE,
                    produces = MediaType.APPLICATION_JSON_VALUE)
    @Operation(description = "Get namespace data for comparison page for the selected samples and values")
    @ApiResponse(responseCode = "200", description = "OK",
        content = @Content(array = @ArraySchema(schema = @Schema(implementation = NamespaceData.class))))
    public ResponseEntity<List<NamespaceData>> getNamespaceDataForComparison(
        ...
        @Valid @RequestAttribute(required = false, value = "interceptedNamespaceComparisonFilter") NamespaceComparisonFilter interceptedNamespaceComparisonFilter,
            @Parameter(required = true, description = "List of SampleIdentifiers, list of values and a NamespaceAttribute")
            @Valid @RequestBody(required = false) NamespaceComparisonFilter namespaceComparisonFilter) {
        ...
        List<NamespaceData> namespaceDataList;
            ...
            namespaceDataList = namespaceDataService.fetchNamespaceDataForComparison(studyIds, sampleIds, namespaceAttribute, values);

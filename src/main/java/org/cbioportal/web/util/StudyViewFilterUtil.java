...
public class StudyViewFilterUtil {
  ...
  public void extractStudyAndSampleIds(
      List<SampleIdentifier> sampleIdentifiers, List<String> studyIds, List<String> sampleIds) {
  ...
  public void removeSelfFromGenomicDataFilter(
      String hugoGeneSymbol, String profileType, StudyViewFilter studyViewFilter) {
  ...
  public void removeSelfFromMutationDataFilter(
      String hugoGeneSymbol,
      String profileType,
      MutationOption categorization,
      StudyViewFilter studyViewFilter) {
    if (studyViewFilter != null && studyViewFilter.getMutationDataFilters() != null) {
      studyViewFilter
          .getMutationDataFilters()
          .removeIf(
              f ->
                  f.getHugoGeneSymbol().equals(hugoGeneSymbol)
                      && f.getProfileType().equals(profileType)
  ...
  public ClinicalDataBin dataBinToClinicalDataBin(
      ClinicalDataBinFilter attribute, DataBin dataBin) {
        ...
        .collect(
            Collectors.groupingBy(
                sampleList -> {
                  return sampleList
                      .getStableId()
                      .replace(sampleList.getCancerStudyIdentifier() + "_", "");
                }));
  }

  public <S> Integer getFilteredCountByDataEquality(
  ...
  public List<ClinicalDataCountItem> getClinicalDataCountsFromCustomData(
      Collection<CustomDataSession> customDataSessions,
      Map<String, SampleIdentifier> filteredSamplesMap,
      List<Patient> patients) {
    int totalSamplesCount = filteredSamplesMap.keySet().size();
    int totalPatientsCount = patients.size();

    return customDataSessions.stream()
        .map(
            customDataSession -> {
              Map<String, List<CustomDataValue>> groupedDatabyValue =
                  customDataSession.getData().getData().stream()
                      .filter(
                          datum ->
                              filteredSamplesMap.containsKey(
                                  getCaseUniqueKey(datum.getStudyId(), datum.getSampleId())))
              ...
              List<ClinicalDataCount> clinicalDataCounts =
                  groupedDatabyValue.entrySet().stream()
                      .map(
                          entry -> {
                            long count =
                      ...
                      .filter(
                          c ->
                              !c.getValue().equalsIgnoreCase("NA")
                                  && !c.getValue().equalsIgnoreCase("NAN")
                                  && !c.getValue().equalsIgnoreCase("N/A"))
                      .collect(Collectors.toList());

              int totalCount =
                  clinicalDataCounts.stream().mapToInt(ClinicalDataCount::getCount).sum();
              int naCount = 0;
  ...
  public boolean isSingleStudyUnfiltered(StudyViewFilter filter) {
    return filter.getStudyIds() != null
        ...
        && (filter.getGenericAssayDataFilters() == null
            || filter.getGenericAssayDataFilters().isEmpty())
    ...
    if (CollectionUtils.isNotEmpty(sampleAttributeIds)) {
      ...
      combinedResult.addAll(
          filterClinicalDataByStudyAndSampleAndAttribute(
              unfilteredClinicalDataForSamples, sampleIdToStudyId, sampleAttributeIdLookup));
    }
    ...
    if (CollectionUtils.isNotEmpty(patientAttributeIds)) {
      ...
      combinedResult.addAll(
          filterClinicalDataByStudyAndPatientAndAttribute(
              unfilteredClinicalDataForPatients, patientIdToStudyId, patientAttributeIdLookup));
    }

    if (CollectionUtils.isNotEmpty(conflictingPatientAttributes)) {
      // create lookups for faster filtering
      Map<String, Boolean> conflictingPatientAttributeIdLookup =
          listToMap(conflictingPatientAttributes);
  ...
  public List<GeneFilterQuery> cleanQueryGeneIds(List<GeneFilterQuery> geneQueries) {
    List<String> hugoGeneSymbols =
        geneQueries.stream().map(GeneFilterQuery::getHugoGeneSymbol).collect(Collectors.toList());
  ...
  public List<StructuralVariantFilterQuery> resolveEntrezGeneIds(
      List<StructuralVariantFilterQuery> structVarQueries) {

    List<String> hugoGeneSymbols =
        structVarQueries.stream()
    ...
    structVarQueries.forEach(
        structVarQuery -> {
          structVarQuery
              .getGene1Query()
              .setEntrezId(
                  symbolToEntrezGeneId.getOrDefault(
                      structVarQuery.getGene1Query().getHugoSymbol(), null));
          structVarQuery
              .getGene2Query()
              .setEntrezId(
                  symbolToEntrezGeneId.getOrDefault(
                      structVarQuery.getGene2Query().getHugoSymbol(), null));
        });
  ...
  private List<Binnable> filterClinicalDataByStudyAndPatientAndAttribute(
    ...
    return clinicalData.stream()
        .filter(
            d ->
                patientToStudyId
                        .getOrDefault(generatePatientToStudyKey(d), "")
                        .equals(d.getStudyId())
                    && attributeIdLookup.getOrDefault(d.getAttrId(), false))
        .collect(Collectors.toList());
  }
  ...
  public List<ClinicalData> transformSampleIdentifiersToClinicalData(
      List<SampleIdentifier> sampleIdentifiers, String attributeId, String attributeValue) {

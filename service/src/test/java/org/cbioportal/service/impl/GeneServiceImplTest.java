...
public class GeneServiceImplTest extends BaseServiceImplTest {
    ...
    public void getAllGenes() throws Exception {
        ...
        List<Gene> result = geneService.getAllGenes(
            KEYWORD,
            ALIAS,
            PROJECTION,
            PAGE_SIZE,
    ...
    public void getMetaGenes() throws Exception {
        ...
        Mockito
            .when(geneRepository.getMetaGenes(null, ALIAS))
    ...
    public void getGeneByEntrezGeneIdNotFound() throws Exception {
        Mockito
            .when(geneRepository.getGeneByEntrezGeneId(ENTREZ_GENE_ID_1))
            .thenReturn(null);
    ...
    public void getGeneByHugoGeneSymbolNotFound() throws Exception {
        Mockito
            .when(geneRepository.getGeneByHugoGeneSymbol(HUGO_GENE_SYMBOL))
            .thenReturn(null);
    ...
    public void getAliasesOfGeneByEntrezGeneId() throws Exception {
        ...
        Mockito
            .when(
                geneRepository.getAliasesOfGeneByEntrezGeneId(ENTREZ_GENE_ID_1)
            )
            .thenReturn(expectedAliases);
        List<String> result = geneService.getAliasesOfGene(
            ENTREZ_GENE_ID_1.toString()
        );
    ...
    public void getAliasesOfGeneByHugoGeneSymbol() throws Exception {
        ...
        Mockito
            .when(
                geneRepository.getAliasesOfGeneByHugoGeneSymbol(
                    HUGO_GENE_SYMBOL
                )
            )
            .thenReturn(expectedAliases);
    ...
    public void fetchGenes() throws Exception {
        ...
        Mockito
            .when(
                geneRepository.fetchGenesByHugoGeneSymbols(
                    Arrays.asList(HUGO_GENE_SYMBOL),
                    PROJECTION
                )
            )
            .thenReturn(expectedGeneList);

        List<Gene> result = geneService.fetchGenes(
            geneIds,
            GENE_ID_TYPE,
            PROJECTION
        );
    ...
    public void fetchMetaGenes() throws Exception {
        ...
        Mockito
            .when(
                geneRepository.fetchMetaGenesByHugoGeneSymbols(
                    Arrays.asList(HUGO_GENE_SYMBOL)
                )
            )

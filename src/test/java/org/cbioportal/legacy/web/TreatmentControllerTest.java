...
public class TreatmentControllerTest {
  ...
  @MockitoBean private StudyViewFilterUtil studyViewFilterUtil;
  ...
  public void getAllPatientTreatments() throws Exception {
    Mockito.when(studyViewFilterApplier.apply(Mockito.any())).thenReturn(sampleIdentifiers);
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.post("/api/treatments/patient")
                .with(csrf())
  ...
  public void getAllSampleTreatments() throws Exception {
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.post("/api/treatments/sample")
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(studyViewFilter)))
        .andExpect(MockMvcResultMatchers.status().isOk())
  ...
  public void getContainsTreatmentData() throws Exception {
    ...
    Mockito.when(treatmentService.containsTreatmentData(Mockito.anyList(), Mockito.any()))
        .thenReturn(true);

    mockMvc
        .perform(
            MockMvcRequestBuilders.post("/api/treatments/display-patient")
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)

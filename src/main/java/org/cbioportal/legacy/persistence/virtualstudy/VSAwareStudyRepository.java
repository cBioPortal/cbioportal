...
public class VSAwareStudyRepository implements StudyRepository {
    ...
    public VSAwareStudyRepository(VirtualStudyService virtualStudyService, StudyRepository studyRepository) {
        this.virtualStudyService = virtualStudyService;
        this.studyRepository = studyRepository;
    }

    @Override
    public List<CancerStudy> getAllStudies(String keyword, String projection, Integer pageSize, Integer pageNumber, String sortBy, String direction) {
        List<CancerStudy> materialisedStudies = studyRepository.getAllStudies(keyword, projection, null, null, null, null);
        List<CancerStudy> virtualStudies = virtualStudyService.getPublishedVirtualStudies(keyword).stream()
            .map(virtualStudyService::toCancerStudy).toList();
    ...
    private Comparator<CancerStudy> composeComparator(String sortBy, String direction) {
        ...
        Comparator<CancerStudy> result = switch (s) {

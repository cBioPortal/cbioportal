...
public class DiscreteDataTypeSetSpec extends DataTypeSpec {
     ...
     * @param validValues1
     */
    public DiscreteDataTypeSetSpec(
        GeneticDataTypes theGeneticDataType,
        GeneticTypeLevel validValues1
    )
        ...
        if (validValues1.getTheGeneticDataType().equals(theGeneticDataType)) {
            ...
            throw new IllegalArgumentException(
                validValues1 + " is not a level for " + theGeneticDataType
            );
        }
    }
     ...
     * @return
     */
    public static DiscreteDataTypeSetSpec discreteDataTypeSetSpecGeneratorByLevelCode(
        String theGeneticDataTypeNameString,
        String levelCodeString
    ) {
        try {
            return new DiscreteDataTypeSetSpec(
                theGeneticDataTypeNameString,
                Integer.parseInt(levelCodeString)
            );
     ...
     * @return
     */
    public static DiscreteDataTypeSetSpec discreteDataTypeSetSpecGeneratorByLevelName(
        String levelName
    ) {
        if (levelName == null) {
            return null;
        ...
        GeneticTypeLevel theGeneticTypeLevel = GeneticTypeLevel.findDataTypeLevel(
            levelName
        );

        // verify that levelName maps to a level within a discrete datatype
        if (
            theGeneticTypeLevel != null &&
            theGeneticTypeLevel
            ...
            return new DiscreteDataTypeSetSpec(
                theGeneticTypeLevel.getTheGeneticDataType(),
                theGeneticTypeLevel
            );
        } else {
            return null;
     ...
     * @return
     */
    public static DiscreteDataTypeSetSpec specificMutationDataTypeSetSpecGenerator(
        String specificMutation
    ) {
        DiscreteDataTypeSetSpec ret = new DiscreteDataTypeSetSpec(
    ...
    public static GeneticDataTypes findDataType(String name)
        throws IllegalArgumentException {
        return DataTypeSpec.genericFindDataType(
            name,
     ...
     * method.
     */
    public static DiscreteDataTypeSetSpec newInstance(
        DiscreteDataTypeSetSpec aDiscreteDataTypeSetSpec
    ) {
        if (null == aDiscreteDataTypeSetSpec) {
            return null;
        }
        DiscreteDataTypeSetSpec theNewDiscreteDataTypeSetSpec = new DiscreteDataTypeSetSpec(
            aDiscreteDataTypeSetSpec.getTheGeneticDataType()
        );
    ...
    private static Pattern SPECIFIC_POSITION_MUTATION_PATTERN = Pattern.compile(
        "[^0-9]*([0-9]+)"
    );
    private static Pattern SPECIFIC_RANGE_DELETION_PATTERN = Pattern.compile(
        "[^0-9]*([0-9]+)[^0-9]+([0-9]+)[^0-9]*"
    );
    ...
    private boolean satisfySpecificMutation(String specificMutations) {
        ...
        for (String specificMutationUpper : specificMutationsUpper.split(",")) {
            ...
            for (String mutationPattern : mutationPatterns) {
                Matcher spm = SPECIFIC_POSITION_MUTATION_PATTERN.matcher(
                    mutationPattern
                );
                if (spm.matches()) { // all mutations for a specific amino acid
                    String strPos = spm.group(1);
                    if (
                        specificMutationUpper.matches(
                            ".*[^0-9]+" + strPos + "[^0-9\\+\\-]*"
                        ) ||
                    ...
                    Matcher delm = SPECIFIC_RANGE_DELETION_PATTERN.matcher(
                        specificMutationUpper
                    );
                    if (delm.matches()) {
                ...
                // the mutation_type from database directly
                    ...
                    mutationPattern.equals("MS") ||
                    mutationPattern.equals("MISSENSE")
                ) {
                    if (specificMutationUpper.matches("[A-Z][0-9]+[A-Z]")) {
                        if (
                            specificMutationUpper.charAt(0) !=
                            specificMutationUpper.charAt(
                                specificMutationUpper.length() - 1
                            )
                        ) { //not silent
                            return true;
                        }
                    ...
                    mutationPattern.equals("NS") ||
                    mutationPattern.equals("NONSENSE")
                ) {
                    ...
                    mutationPattern.equals("FS") ||
                    mutationPattern.equals("FRAMESHIFT")
                ) {
                    if (specificMutationUpper.matches("[A-Z\\*][0-9]+FS")) {
                        return true;
                    }
                } else if (
                    mutationPattern.equals("IF") ||
                    mutationPattern.equals("INFRAME")
                ) {
                    if (
                        specificMutationUpper.matches(
                            ".+IN_FRAME_((INS)|(DEL)).*"
                    ...
                    mutationPattern.equals("SP") ||
                    mutationPattern.equals("SPLICE")
                ) {
                    if (
                        specificMutationUpper.matches("[A-Z][0-9]+_SPLICE") ||
    ...
    public void addLevel(Object value) {
        if (value instanceof GeneticTypeLevel) {
            ...
            theGeneticDataType == GeneticDataTypes.Mutation &&
    ...
    public void combine(DiscreteDataTypeSetSpec aDiscreteDataTypeSetSpec) {
        this.specifiedValues.addAll(
                aDiscreteDataTypeSetSpec.getSpecifiedValues()
            );
        this.mutationPatterns.addAll(
                aDiscreteDataTypeSetSpec.getMutationPatterns()
            );
    }

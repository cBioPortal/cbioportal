...
window.DataManagerForIviz = (function($, _) {
  ...
  content.util.sortByClinicalPriority = function(array) {
    if (_.isArray(array)) {
      array = array.sort(function(a, b) {
        var score = iViz.util.compareLists(a.priority, b.priority, false);
        if (score === 0) {
            ...
            .then(function(_geneticProfiles, _caseLists,
              ...
              var addAttr = function(data, group) {
                ...
                var datum = {
                  ...
                  description: '',
                  display_name: '',
                  priority: iViz.priorityManager.getDefaultPriority(3, data.attr_id)
                };
              ...
              _.each(_sampleAttributes, function(_metaObj) {
                ...
                if (!_.isArray(_metaObj.priority)) {
                  iViz.priorityManager
                    .setClinicalAttrPriority(_metaObj.attr_id, [0, 0, 0, Number(_metaObj.priority)]);
                  _metaObj.priority =
                    iViz.priorityManager
                      .getDefaultPriority(4, _metaObj.attr_id);
                }
                _metaObj.show = iViz.priorityManager.calcPriorityNumber(_metaObj.priority) !== 0;
                ...
                if (['CANCER_TYPE', 'CANCER_TYPE_DETAILED']
                  ...
                  _metaObj.priority =
                    iViz.priorityManager
                      .getDefaultPriority(1, _metaObj.attr_id);
                }
              });
              ...
              if (self.hasCnaSegmentData()) {
                ...
                _cnaAttrMeta.priority =
                  iViz.priorityManager
                    .getDefaultPriority(3, _cnaAttrMeta.attr_id);
                _cnaAttrMeta.show = iViz.priorityManager.calcPriorityNumber(_cnaAttrMeta.priority) !== 0;
              ...
              if (self.hasMutationData()) {
                ...
                _mutDataAttrMeta.priority =
                  iViz.priorityManager
                    .getDefaultPriority(3, _mutDataAttrMeta.attr_id);
                _mutDataAttrMeta.show = iViz.priorityManager.calcPriorityNumber(_mutDataAttrMeta.priority) !== 0;
              ...
              if (_hasDFS) {
                ...
                _dfsSurvivalAttrMeta.priority =
                  iViz.priorityManager
                    .getDefaultPriority(2, 'DFS_SURVIVAL');
                _dfsSurvivalAttrMeta.show = _dfsSurvivalAttrMeta.priority[1] !== 0;
              ...
              if (_hasOS) {
                ...
                _osSurvivalAttrMeta.priority =
                  iViz.priorityManager
                    .getDefaultPriority(2, 'OS_SURVIVAL');
                _osSurvivalAttrMeta.show = iViz.priorityManager.calcPriorityNumber(_osSurvivalAttrMeta.priority) !== 0;
              ...
              if (self.getCancerStudyIds().length > 1) {
                ...
                _patientAttributes.study_id = {
                  ...
                  keys: [],
                  numOfDatum: 0,
                  priority: iViz.priorityManager.getDefaultPriority(3, _id),
                  show: true,
                  attrList: [_id]
                };
                _patientAttributes.study_id.show = iViz.priorityManager.calcPriorityNumber(_patientAttributes.study_id.priority) !== 0;
              }
              ...
              if (_hasSampleAttrData.copy_number_alterations !== undefined) {
                ...
                _sampleAttributes.copy_number_alterations.show = iViz.priorityManager.calcPriorityNumber(_sampleAttributes.copy_number_alterations.priority) !== 0;
              }
                ...
                .then(function(_cnaFractionData, _mutationCountData) {
                  ...
                  if (_hasSampleAttrData.mutation_count !== undefined && _hasSampleAttrData.cna_fraction !== undefined) {
                    ...
                    _mutCntAttrMeta.priority =
                      iViz.priorityManager
                        .getDefaultPriority(2, 'MUT_CNT_VS_CNA');
                    _mutCntAttrMeta.show = iViz.priorityManager.calcPriorityNumber(_mutCntAttrMeta.priority) !== 0;
                  ...
                  if (_hasSampleAttrData.mutation_count !== undefined) {
                    ...
                    _MutationCountMeta.priority =
                      iViz.priorityManager
                        .getDefaultPriority(3, _MutationCountMeta.attr_id);
                    _MutationCountMeta.show = iViz.priorityManager.calcPriorityNumber(_MutationCountMeta.priority) !== 0;

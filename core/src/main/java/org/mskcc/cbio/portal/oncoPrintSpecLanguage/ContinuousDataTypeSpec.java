...
public class ContinuousDataTypeSpec extends DataTypeSpecInequality {
    ...
    public ContinuousDataTypeSpec(
        ...
        if (
            ...
            throw new IllegalArgumentException(
                "ContinuousDataTypeSpec only takes Continuous GeneticDataTypes"
            );
        }
     ...
     * @return
     */
    public static ContinuousDataTypeSpec continuousDataTypeSpecGenerator(
        String theGeneticDataTypeString,
        String theComparisonOpString,
        String theThresholdString
    ) {
        try {
            //out.println( "continuousDataTypeSpecGenerator" + ":" + theGeneticDataTypeString + ":" + theComparisonOpString + ":" + theThresholdString );
            GeneticDataTypes theGeneticDataType = ContinuousDataTypeSpec.findDataType(
                theGeneticDataTypeString
            );
            ComparisonOp theComparisonOp = ComparisonOp.convertCode(
                theComparisonOpString
            );
            if (null == theThresholdString) {
                return null;
            }
            float theThreshold = Float.parseFloat(theThresholdString);
            return new ContinuousDataTypeSpec(
                theGeneticDataType,
                theComparisonOp,
                theThreshold
            );
    ...
    public ContinuousDataTypeSpec(
        ...
        this(
            aContinuousDataTypeSpec.getTheGeneticDataType(),
            aContinuousDataTypeSpec.getComparisonOp(),
    ...
    public static ContinuousDataTypeSpec newInstance(
        ContinuousDataTypeSpec aContinuousDataTypeSpec
    ) {
    ...
    public static GeneticDataTypes findDataType(String name)
        throws IllegalArgumentException {
        return DataTypeSpecInequality.genericFindDataType(
            name,
     ...
     */
    public void combine(ContinuousDataTypeSpec aContinuousDataTypeSpec)
        throws IllegalArgumentException {
        if (
            !this.comparisonOp.getTheComparisonOpDirection()
                .equals(
                    aContinuousDataTypeSpec
                        .getComparisonOp()
                        .getTheComparisonOpDirection()
                )
        ) {
            throw new IllegalArgumentException(
                "the ContinuousDataTypeSpecs must have ComparisonOps with the same direction"
            );
        }
        switch (this.comparisonOp.getTheComparisonOpDirection()) {
            case Smaller:
                if (
                    (
                        (Float) aContinuousDataTypeSpec.getThreshold()
                    ).floatValue() >
                    ((Float) this.threshold).floatValue()
                ) {
                    ...
                    if (
                        this.comparisonOp.equals(ComparisonOp.LessEqual) ||
                        aContinuousDataTypeSpec
                            .getComparisonOp()
                            .equals(ComparisonOp.LessEqual)
                    ) {
            ...
            case Bigger:
                if (
                    ...
                    if (
                        this.comparisonOp.equals(ComparisonOp.GreaterEqual) ||
                        aContinuousDataTypeSpec
                            .getComparisonOp()
                            .equals(ComparisonOp.GreaterEqual)
                    ) {

...
public class MutationsJSON extends HttpServlet {
    ...
    protected void processRequest(
        ...
        String mutationProfileId = request.getParameter(
            PatientView.MUTATION_PROFILE
        );

        if (mutationProfileId != null) {
            // Get the Genetic Profile
            GeneticProfile mutationProfile = DaoGeneticProfile.getGeneticProfileByStableId(
                mutationProfileId
            );
                ...
                try {
                    cancerStudy =
                        DaoCancerStudy.getCancerStudyByInternalId(
                            mutationProfile.getCancerStudyId()
                        );
                    if (
                        cancerStudy != null &&
                        accessControl
                        ...
                        if (cmd != null) {
                            if (cmd.equalsIgnoreCase(GET_CONTEXT_CMD)) {
                                processGetMutationContextRequest(
                                    request,
                                    response
                                );
                                return;
    ...
    private void processGetSmgRequest(
        HttpServletRequest request,
        HttpServletResponse response
    )
        throws ServletException, IOException {
        ...
        try {
            ...
            if (mutationProfile != null) {
                ...
                smgs =
                    mutationModelConverter.convertSignificantlyMutatedGeneToMap(
                        mutationRepositoryLegacy.getSignificantlyMutatedGenes(
                            profileId,
                            null,
                            selectedCaseList,
                            2,
                            DEFAULT_THRESHOLD_NUM_SMGS
                        )
                    );
        ...
        for (Map.Entry<Long, Map<String, String>> entry : smgs.entrySet()) {
            ...
            List<String> sampleStableIds = InternalIdUtil.getStableSampleIds(
                sampleInternalIds
            );
    ...
    private void processGetMutationsRequest(
        HttpServletRequest request,
        HttpServletResponse response
    )
        throws ServletException, IOException {
        String[] samples = request
            .getParameter(PatientView.SAMPLE_ID)
            .split(" +");
        String mutationProfileId = request.getParameter(
            PatientView.MUTATION_PROFILE
        );
        ...
        if (
            drugType != null &&
            drugType.equalsIgnoreCase(PatientView.DRUG_TYPE_FDA_ONLY)
        ) {
            fdaOnly = true;
            cancerDrug = false;
        }
        ...
        try {
            ...
            if (mutationProfile != null) {
                ...
                mutations =
                    mutationModelConverter.convert(
                ...
                if (!mutations.isEmpty()) {
                    cosmic =
                        DaoCosmicData.getCosmicForMutationEvents(mutations);
                    ...
                    keywordContextMap =
                        getKeywordContextMap(concatEventIds, profileId);
                    ...
                    if (mrnaProfileId != null && sample != null) { // only if there is only one tumor
                        mrnaContext =
                            getMrnaContext(sample, mutations, mrnaProfileId);
                    }
                    if (cnaProfileId != null && samples.length == 1) { // only if there is only one tumor
                        cnaContext =
                            getCnaContext(sample, mutations, cnaProfileId);
                    }
                }
            }
        } catch (DaoException ex) {
        ...
        for (ExtendedMutation mutation : mutations) {
            ...
            exportMutation(
                data,
                mapMutationEventIndex,
                mutation,
                cancerStudy,
                drugs.get(mutation.getEntrezGeneId()),
                geneContextMap.get(mutation.getGeneSymbol()),
                mutation.getKeyword() == null
                    ? 1
    ...
    private void processGetMutationContextRequest(
        HttpServletRequest request,
        HttpServletResponse response
    )
        throws ServletException, IOException {
    ...
    private void processCountMutationsRequest(
        HttpServletRequest request,
        HttpServletResponse response
    )
        throws ServletException, IOException {
        ...
        mutationProfile =
            DaoGeneticProfile.getGeneticProfileByStableId(mutationProfileId);
        if (strSampleIds != null) {
            List<String> stableSampleIds = Arrays.asList(
                strSampleIds.split("[ ,]+")
            );
            sampleIds =
    ...
    private Map<String, Integer> convertMapSampleKeys(
        Map<Integer, Integer> mutationEventCounts
    ) {
    ...
    private Map<Long, Set<String>> getDrugs(
        String eventIds,
        int profileId,
        boolean fdaOnly,
        boolean cancerDrug
        ...
        List<Integer> result = mutationRepositoryLegacy.getGenesOfMutations(
            intEventIds
        );
        ...
        Map<Long, List<String>> map = daoDrugInteraction.getDrugs(
            genes,
            fdaOnly,
            cancerDrug
        );
    ...
    private Map<Long, String> getCnaContext(
        Sample sample,
        ...
        for (ExtendedMutation mutEvent : mutations) {
            ...
            String cna = daoGeneticAlteration.getGeneticAlteration(
                DaoGeneticProfile
                    .getGeneticProfileByStableId(cnaProfileId)
                    .getGeneticProfileId(),
    ...
    private Map<Long, Map<String, Object>> getMrnaContext(
        Sample sample,
        List<ExtendedMutation> mutations,
        String mrnaProfileId
    )
        throws DaoException {
        ...
        for (ExtendedMutation mutEvent : mutations) {
            ...
            Map<Integer, String> mrnaMap = daoGeneticAlteration.getGeneticAlterationMap(
                DaoGeneticProfile
                    .getGeneticProfileByStableId(mrnaProfileId)
                    .getGeneticProfileId(),
                gene
            );
    ...
    private Map<String, Integer> getGeneContextMap(
        String eventIds,
        int profileId,
        ...
        List<Integer> genes = mutationRepositoryLegacy.getGenesOfMutations(
            intEventIds
        );

        Map<Long, Integer> map = mutationModelConverter.convertMutatedGeneSampleCountToMap(
            mutationRepositoryLegacy.countSamplesWithMutatedGenes(
                profileId,
                genes
            )
        );
    ...
    private Map<String, Integer> getKeywordContextMap(
        String eventIds,
        int profileId
    )
        throws DaoException {
        ...
        Set<String> genes = new HashSet<>(
            mutationRepositoryLegacy.getKeywordsOfMutations(intEventIds)
        );
        return mutationModelConverter.convertKeywordSampleCountToMap(
            mutationRepositoryLegacy.countSamplesWithKeywords(
                profileId,
                new ArrayList<>(genes)
            )
        );
    ...
    private Map<String, Integer> addReadCountMap(
    ...
    private void exportMutation(
        Map<String, List> data,
        Map<Long, Integer> mapMutationEventIndex,
        ...
        String cna,
        List<String> mycancergenomelinks,
        Boolean isHotspot,
        DaoGeneOptimized daoGeneOptimized
    )
        ...
        if (ix != null) { // multiple samples
            List.class.cast(data.get("caseIds").get(ix))
                .add(
                    DaoSample
                        .getSampleById(mutation.getSampleId())
                        .getStableId()
                );
            addReadCountMap(
                Map.class.cast(data.get("alt-count").get(ix)),
                sample.getStableId(),
                mutation.getTumorAltCount()
            );
            addReadCountMap(
                Map.class.cast(data.get("ref-count").get(ix)),
                sample.getStableId(),
                mutation.getTumorRefCount()
            );
            addReadCountMap(
                Map.class.cast(data.get("normal-alt-count").get(ix)),
                sample.getStableId(),
                mutation.getNormalAltCount()
            );
            addReadCountMap(
                Map.class.cast(data.get("normal-ref-count").get(ix)),
                sample.getStableId(),
    ...
    private List<List> convertCosmicDataToMatrix(
        Set<CosmicMutationFrequency> cosmic
    ) {
        if (cosmic == null) {
            return null;
    ...
    private static final Map<Integer, Map<Long, Double>> mutSigMap = new HashMap<Integer, Map<Long, Double>>(); // map from cancer study id // to map from gene to Q-value

    private static Double getMutSigQValue(int cancerStudyId, long entrez)
        throws DaoException {
        ...
        synchronized (mutSigMap) {
            ...
            if (mapGeneQvalue == null) {
                ...
                for (MutSig ms : DaoMutSig.getAllMutSig(cancerStudyId)) {
                    ...
                    mapGeneQvalue.put(
                        ms.getCanonicalGene().getEntrezGeneId(),
                        qvalue
                    );
                }
            }
    ...
    private static Map<Long, Double> getMutSig(int cancerStudyId)
        throws DaoException {
        Map<Long, Double> mapGeneQvalue;
        synchronized (mutSigMap) {
            mapGeneQvalue = mutSigMap.get(cancerStudyId);

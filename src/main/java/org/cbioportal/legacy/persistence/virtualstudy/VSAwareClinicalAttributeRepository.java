...
public class VSAwareClinicalAttributeRepository implements ClinicalAttributeRepository {
    ...
    public VSAwareClinicalAttributeRepository(VirtualStudyService virtualStudyService, ClinicalAttributeRepository clinicalAttributeRepository) {
    ...
    public List<ClinicalAttribute> getAllClinicalAttributes(String projection, Integer pageSize, Integer pageNumber, String sortBy, String direction) {
        List<ClinicalAttribute> materialisedClinicalAttributes = clinicalAttributeRepository.getAllClinicalAttributes(projection, null, null, null, null);
        ...
        List<ClinicalAttribute> virtualClinicalAttributes = virtualStudyService.getPublishedVirtualStudies().stream().flatMap(virtualStudy -> {
            ...
            return clinicalAttributeRepository.getClinicalAttributeCountsBySampleIds(studyIds, sampleIds).stream().filter(c -> c.getCount() > 0).map(c -> {
    ...
    private Comparator<ClinicalAttribute> composeComparator(String sortBy, String direction) {
        ...
        Comparator<ClinicalAttribute> result = switch (ca) {
    ...
    private static ClinicalAttribute mergeClinicalAttribute(List<ClinicalAttribute> clinicalAttributes, CancerStudy virtualCancerStudy) {
    ...
    public BaseMeta getMetaClinicalAttributes() {
        ...
        meta.setTotalCount(getAllClinicalAttributes(Projection.ID.name(), null, null, null, null).size());
        return meta;
    }
    ...
    public ClinicalAttribute getClinicalAttribute(String studyId, String clinicalAttributeId) {
        ...
        return getAllClinicalAttributes(Projection.DETAILED.name(), null, null, null, null).stream()
            .filter(clinicalAttribute -> clinicalAttribute.getAttrId().equals(clinicalAttributeId) && clinicalAttribute.getCancerStudyIdentifier().equals(studyId))
            .findFirst()
            .orElse(null);
    }

    @Override
    public List<ClinicalAttribute> getAllClinicalAttributesInStudy(String studyId, String projection, Integer pageSize, Integer pageNumber, String sortBy, String direction) {
        //TODO Optimize!
    ...
    public BaseMeta getMetaClinicalAttributesInStudy(String studyId) {
        BaseMeta meta = new BaseMeta();
        meta.setTotalCount(getAllClinicalAttributesInStudy(studyId, Projection.ID.name(), null, null, null, null).size());
        return meta;
    }
    ...
    public List<ClinicalAttribute> fetchClinicalAttributes(List<String> studyIds, String projection) {
        ...
        return getAllClinicalAttributes(projection, null, null, null, null).stream()
            .filter(clinicalAttribute -> studyIds.contains(clinicalAttribute.getCancerStudyIdentifier()))
            .toList();
    }
    ...
    public List<ClinicalAttribute> getClinicalAttributesByStudyIdsAndAttributeIds(List<String> studyIds, List<String> attributeIds) {
        //TODO Optimize!
        return getAllClinicalAttributes(Projection.DETAILED.name(), null, null, null, null).stream()
            .filter(clinicalAttribute -> attributeIds.contains(clinicalAttribute.getAttrId()) && studyIds.contains(clinicalAttribute.getCancerStudyIdentifier()))
            .toList();
    }
}

...
public class GetAlterationDataJSON extends HttpServlet {
    ...
    protected void doPost(
        ...
        try {
            ...
            List<String> stableSampleIds = CoExpUtil.getSampleIds(
                sampleSetId,
                patientIdsKey
            );
            List<Integer> sampleIds = InternalIdUtil.getInternalSampleIds(
                final_gp.getCancerStudyId(),
                stableSampleIds
            );
            ...
            for (String geneId : geneIdList) {
                ...
                for (int i = 0; i < sampleIds.size(); i++) {
                    if (
                        ...
                        _datum.put(
                            "value",
                            Float.parseFloat(tmpProfileDataArr.get(i))
                        );
                        _geneArr.add(_datum);
                    }
                }
            ...
            ((ObjectNode) _result).put(
                    "profile_name",
                    final_gp.getProfileName()
                );
            ((ObjectNode) _result).put(
                    "profile_description",
                    final_gp.getProfileDescription()
                );

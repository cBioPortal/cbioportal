...
public class GeneticProfileReader {
     ...
     * @throws DaoException
     */
    public static GeneticProfile loadGeneticProfile(File file)
        ...
        if (existingGeneticProfile != null) {
            if (!existingGeneticProfile.getDatatype().equals("MAF")) {
                ...
                throw new RuntimeException(
                    "Error: genetic_profile record found with same Stable ID as the one used in your data:  " +
                    existingGeneticProfile.getStableId() +
                    ". Remove the existing genetic_profile record first."
                );
            } else if (geneticProfile.getDatatype().equals("FUSION")) {
                String svStableId = existingGeneticProfile
                    .getStableId()
                    .replace("mutations", "fusion");
                ...
                GeneticProfile existingSVGeneticProfile = DaoGeneticProfile.getGeneticProfileByStableId(
                    svStableId
                );
                if (existingSVGeneticProfile == null) {
                    GeneticProfile gp = new GeneticProfile(geneticProfile);
                    gp.setGeneticAlterationType(
                ...
                geneticProfile.setGeneticProfileId(
        ...
        if (
            ...
            (
                ...
                geneticProfile
                    .getAllOtherMetadataFields()
                    .getProperty("source_stable_id") !=
                null
            )
        ) {
        ...
        if (
            ...
            geneticProfile.setGenericAssayType(
                geneticProfile.getOtherMetaDataField("generic_assay_type")
            );
            // if genericAssayType is TREATMENT_RESPONSE, validate if pivotThreshold and sortOrder exist
            if (
                geneticProfile
                    .getGenericAssayType()
                    .equals("TREATMENT_RESPONSE")
            ) {
                validateTreatmentResponse(geneticProfile, file);
            }
            // add pivotThreshold and sortOrder if exist
            if (
                geneticProfile.getOtherMetaDataField("pivot_threshold_value") !=
                null
            ) {
                geneticProfile.setPivotThreshold(
                    Float.parseFloat(
                        geneticProfile.getOtherMetaDataField(
            ...
            if (
                geneticProfile.getOtherMetaDataField("value_sort_order") != null
            ) {
                geneticProfile.setSortOrder(
                    geneticProfile.getOtherMetaDataField("value_sort_order")
                );
            }
        }
        ...
        if (geneticProfileLink != null) {
            ...
            int geneticProfileId = DaoGeneticProfile
                .getGeneticProfileByStableId(geneticProfile.getStableId())
                .getGeneticProfileId();
        ...
        // Get ID
        GeneticProfile gp = DaoGeneticProfile.getGeneticProfileByStableId(
            geneticProfile.getStableId()
        );
    ...
    private static GeneticProfileLink createGeneticProfileLink(
        GeneticProfile geneticProfile
    ) {
        GeneticProfileLink geneticProfileLink = new GeneticProfileLink();

        // Set `REFERRED_GENETIC_PROFILE_ID`
        String referredGeneticProfileStableId = parseStableId(
            geneticProfile.getAllOtherMetadataFields(),
            "source_stable_id"
        );
        if (referredGeneticProfileStableId == null) {
            throw new RuntimeException(
                "'source_stable_id' is required in meta file for " +
                geneticProfile.getStableId()
            );
        }
        GeneticProfile referredGeneticProfile = DaoGeneticProfile.getGeneticProfileByStableId(
            referredGeneticProfileStableId
        );
        geneticProfileLink.setReferredGeneticProfileId(
            referredGeneticProfile.getGeneticProfileId()
        );
        ...
        if (
            Arrays
                .asList("P-VALUE", "Z-SCORE")
                .contains(geneticProfile.getDatatype())
        ) {
    ...
    private static void validateGenesetProfile(
        GeneticProfile geneticProfile,
        File file
    )
        throws DaoException {
        ...
        if (genesetVersion == null) {
            throw new RuntimeException(
                "Attempted to import GENESET_SCORE data, but all gene set tables are empty.\n" +
            ...
            geneticProfile.getOtherMetaDataField("geneset_def_version") == null
        ) {
            throw new RuntimeException(
                "Missing geneset_def_version property in '" +
                file.getPath() +
            ...
            !geneticProfile
                .getOtherMetaDataField("geneset_def_version")
                .equals(genesetVersion)
        ) {
            throw new RuntimeException(
                "'geneset_def_version' property (" +
                geneticProfile.getOtherMetaDataField("geneset_def_version") +
                ") in '" +
                file.getPath() +
                "' differs from database version (" +
                genesetVersion +
                ")."
            );
        }
     ...
     */
    private static void validateTreatmentResponse(
        GeneticProfile geneticProfile,
        File file
    ) {
        if (
            ...
            "Missing `pivot_threshold_value` property in '" +
            file.getPath() +
            "' meta data file."
        );
        ...
        if (
            ...
            "Missing `value_sort_order` property in '" +
            file.getPath() +
            "' meta data file."
        );
    }
     ...
     * Check whether required columns for assay response data were defined in the meta file
     */
    private static void validateGenericAssay(
        GeneticProfile geneticProfile,
        File file
    ) {
        if (
            geneticProfile.getOtherMetaDataField("generic_assay_type") == null
        ) throw new RuntimeException(
            "Missing `generic_assay_type` property in '" +
            file.getPath() +
            "' meta data file."
        );
    }
     ...
     * @throws DaoException
     */
    public static GeneticProfile loadGeneticProfileFromMeta(File file)
        throws IOException, DaoException {
        ...
        String profileDescription = properties.getProperty(
            "profile_description"
        );
        String geneticAlterationTypeString = properties.getProperty(
        ...
        if (geneticAlterationTypeString == null) {
            throw new IllegalArgumentException(
                "genetic_alteration_type is not specified."
            );
        } else if (datatype == null) {
            datatype = "";
        }
        boolean showProfileInAnalysisTab = true;
        String showProfileInAnalysisTabStr = properties.getProperty(
            "show_profile_in_analysis_tab"
        );
        if (
            showProfileInAnalysisTabStr != null &&
            showProfileInAnalysisTabStr.equalsIgnoreCase("FALSE")
        ) {
            showProfileInAnalysisTab = false;
        }

        profileDescription = profileDescription.replaceAll("\t", " ");
        GeneticAlterationType alterationType = GeneticAlterationType.valueOf(
            geneticAlterationTypeString
        );
    ...
    private static String parseStableId(
        Properties properties,
        String stableIdPropName
    ) {
        ...
        if (stableId.equals(cancerStudyIdentifier + "_fusion")) {
            ...
            if (existingGeneticProfile == null) {
                throw new IllegalArgumentException(
                    "Wrong order: FUSION data should be loaded after MUTATION data"
                );
            }
     ...
     * @throws Exception
     */
    public static Set<String> getVariantClassificationFilter(File file)
        throws Exception {
        Properties properties = new TrimmedProperties();
        properties.load(new FileInputStream(file));
        String variantClassificationFilter = properties.getProperty(
            "variant_classification_filter"
        );
        if (variantClassificationFilter != null) {
            Set<String> filteredMutations = new HashSet<String>();
            for (String mutation : (
                Arrays.asList(variantClassificationFilter.split(","))
            )) {

...
public class DataBinner {
  ...
  public <T extends DataBinFilter> List<DataBin> calculateDataBins(
    ...
    return calculateDataBins(
        dataBinFilter, clinicalDataType, clinicalData, ids, DEFAULT_DISTINCT_VALUE_THRESHOLD);
  }
    ...
    if (0 < uniqueValues.size() && uniqueValues.size() <= distinctValueThreshold) {
      ...
      if (DataBinFilter.BinMethod.CUSTOM == binMethod && customBins != null) {
        ...
        List<BigDecimal> bins =
            this.dataBinHelper.generateBins(
        ...
        List<BigDecimal> boundaries =
            this.dataBinHelper.calcQuartileBoundaries(sortedNumericalValues);
  ...
  public DataBin calcNaDataBin(
      List<Binnable> clinicalData, ClinicalDataType clinicalDataType, List<String> ids) {
    DataBin bin = new DataBin();

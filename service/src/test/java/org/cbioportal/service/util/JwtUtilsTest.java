...
public class JwtUtilsTest {
    ...
    public void validateBadSignatureTokenTest()
        ...
        String badSignatureToken =
            token.substring(0, finalDividerIndex + 1) + badSignature;
    ...
    public void extractSubjectTest() throws InvalidDataAccessTokenException {
        ...
        if (
            extractedSubject.isEmpty() || !extractedSubject.equals(TEST_SUBJECT)
        ) {
    ...
    @Test
    public void extractExpirationDateTest()
        throws InvalidDataAccessTokenException {
        Date now = new Date();
        ...
        if (
            ...
            Assert.fail(
                "extracted expiration date is not in the expected range"
            );
        }
    }
    ...
    public void extractPropertiesTest() throws InvalidDataAccessTokenException {
        String token = jwtUtils.createToken(TEST_SUBJECT).getToken();
        Map<String, Object> extractedProperties = jwtUtils.extractProperties(
            token
        );
        if (
            extractedProperties == null ||
            extractedProperties.keySet().size() < 3
        ) {
            Assert.fail(
                "extracted properties is not large enough (at least 3 keys were expected)"
            );
        }
    }
}

...
import org.cbioportal.legacy.web.util.CustomDatatype;
...
public class CustomDataFilterUtil {
  ...
  public List<String> extractInvolvedCancerStudies(final StudyViewFilter studyViewFilter) {
    ...
    } else if (studyViewFilter.getSampleIdentifiers() != null
  ...
  public List<CustomSampleIdentifier> extractCustomDataSamples(
      final StudyViewFilter studyViewFilter) {
    ...
    final List<CustomSampleIdentifier> customSamplesFromProperty =
        studyViewFilter.getCustomDataFilters().stream()
            .flatMap(
                filter -> {
  ...
  private List<CustomSampleIdentifier> extractCustomDataSamplesWithSession(
      final StudyViewFilter studyViewFilter) {
                ...
                customDataSession
                    ...
                    .forEach(
                        datum -> {
                          ...
                          CustomSampleIdentifier customSampleIdentifier =
                              new CustomSampleIdentifier();
                          ...
                          customDataByStudySampleSession.put(
                              datum.getStudyId(),
                              datum.getSampleId(),
    ...
    customSampleIdentifiers.forEach(
        customSampleIdentifier -> {
          int equalityFilterCount =
              studyViewFilterUtil.getFilteredCountByDataEquality(
                  equalityFilters,
                  customDataByStudySampleSession,
                  customSampleIdentifier.getSampleId(),
                  customSampleIdentifier.getStudyId(),
                  false);
          int intervalFilterCount =
              getFilteredCountByDataInterval(
                  intervalFilters,
                  customDataByStudySampleSession,
                  customSampleIdentifier.getSampleId(),
                  customSampleIdentifier.getStudyId());
  ...
  private List<CustomSampleIdentifier> extractCustomDataSamplesWithoutSession(
      final StudyViewFilter studyViewFilter,
      List<CustomSampleIdentifier> customSamplesFromProperty) {
    List<CustomSampleIdentifier> customSampleIdentifiers =
        new ArrayList<>(customSamplesFromProperty);
    ...
    studyViewFilter
        ...
        .forEach(
            filter -> {
              if (filter.getDatatype().equals(CustomDatatype.STRING.name())) {
                equalityFilters.add(filter);
              } else {
                intervalFilters.add(filter);
              }
                ...
                filter
                    ...
                    .forEach(
                        datum -> {
                          ...
                          customDataByStudySampleName.put(
                              datum.getStudyId(),
                              datum.getSampleId(),
    ...
    customSampleIdentifiers.forEach(
        customSampleIdentifier -> {
          int equalityFilterCount =
              getFilteredCountByDataEqualityWithStudySampleNameMap(
                  equalityFilters,
                  customDataByStudySampleName,
                  customSampleIdentifier.getSampleId(),
                  customSampleIdentifier.getStudyId(),
                  false);
          int intervalFilterCount =
              getFilteredCountByDataIntervalWithStudySampleNameMap(
                  intervalFilters,
                  customDataByStudySampleName,
                  customSampleIdentifier.getSampleId(),
                  customSampleIdentifier.getStudyId());
  ...
  private Integer getFilteredCountByDataEqualityWithStudySampleNameMap(
    ...
    for (ClinicalDataFilter s : attributes) {
      List<String> filteredValues =
          s.getValues().stream().map(DataFilterValue::getValue).collect(Collectors.toList());
  ...
  private <S> Integer getFilteredCountByDataInterval(
      List<ClinicalDataFilter> attributes,
      MultiKeyMap<String, S> clinicalDataMap,
      String entityId,
      String studyId) {
    int count = 0;
  ...
  private Integer getFilteredCountByDataIntervalWithStudySampleNameMap(
      List<ClinicalDataFilter> attributes,
      MultiKeyMap<String, String> clinicalDataMap,
      String entityId,
      String studyId) {
  ...
  private Range<BigDecimal> calculateRangeValueForFilter(DataFilterValue filterValue) {
    ...
    // Guard start != null: DataFilterValue supports open-ended intervals where only end
    // is set; end.compareTo(null) throws NPE whereas the old end.equals(null) returned false.
  ...
  public List<ClinicalDataCountItem> getCustomDataCounts(
      List<SampleIdentifier> filteredSampleIdentifiers,
      Map<String, CustomDataSession> customDataSessions) {
    Map<String, SampleIdentifier> customSamplesMap =
    ...
    long patientCustomDataSessionsCount =
        customDataSessions.values().stream()
            .filter(customDataSession -> customDataSession.getData().getPatientAttribute())
            .count();
    ...
    return studyViewFilterUtil.getClinicalDataCountsFromCustomData(
        customDataSessions.values(), customSamplesMap, patients);
  }
}

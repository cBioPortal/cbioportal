...
public class ClinicalAttributeUtil {
    ...
    public void extractCategorizedClinicalAttributes(
        ...
        Map<String, Map<Boolean, List<ClinicalAttribute>>> groupedAttributesByIdAndType = clinicalAttributes
            ...
            .collect(
                Collectors.groupingBy(
                    ...
                    Collectors.groupingBy(
                        ClinicalAttribute::getPatientAttribute
                    )
                )
            );
                    ...
                    if (entry.getValue().keySet().size() == 1) {
                        entry
                            ...
                            .forEach(
                                x -> {
                                    if (x.getKey()) {
                                        patientAttributeIdsSet.add(
                                            entry.getKey()
                                        );
                                    } else {
                                        sampleAttributeIdsSet.add(
                                            entry.getKey()
                                        );
                                    }
                                }
                        ...
                        entry
                            ...
                            .forEach(
                                x -> {
                                    if (x.getKey()) {
                                        conflictingPatientAttributeIdsSet.add(
                                            entry.getKey()
                                        );
                                    } else {
                                        sampleAttributeIdsSet.add(
        ...
        conflictingPatientAttributeIds.addAll(
            conflictingPatientAttributeIdsSet
        );
    }
}

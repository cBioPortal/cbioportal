...
public class CoExpressionServiceImpl implements CoExpressionService {
    ...
    public List<CoExpression> getCoExpressions(
        ...
        MolecularProfile molecularProfileB = molecularProfileService.getMolecularProfile(
            molecularProfileIdB
        );
        Boolean isMolecularProfileBOfGenesetType = molecularProfileB
        ...
        List<? extends MolecularData> finalmolecularDataListA = molecularDataListA
            .stream()
            .filter(p -> sharedSamples.contains(p.getSampleId()))
            .collect(Collectors.toList());
        List<? extends MolecularData> finalmolecularDataListB = molecularDataListB
            .stream()
            .filter(p -> sharedSamples.contains(p.getSampleId()))
            .collect(Collectors.toList());

        computedCoExpressions =
            computeCoExpressionsFromMolecularData(
                finalmolecularDataListB,
                isMolecularProfileBOfGenesetType,
                finalmolecularDataListA,
                geneticEntityId,
                threshold,
                molecularProfileIdB
            );
    ...
    public List<CoExpression> getCoExpressions(
        ...
        List<String> sampleIds = sampleListRepository.getAllSampleIdsInSampleList(
            sampleListId
        );
    ...
    public List<CoExpression> fetchCoExpressions(
        ...
        if (geneticEntityType.equals(EntityType.GENE)) {
            List<Integer> queryGeneticEntityIds = Arrays.asList(
            ...
            List<String> queryGeneticEntityIds = Arrays.asList(
                queryGeneticEntityId
            );
            maItr =
                genesetDataService.getGenesetAlterations(
                    molecularProfileId,
                    queryGeneticEntityIds
                );
        }
        ...
        String commaSeparatedSampleIdsOfMolecularProfile = molecularDataRepository.getCommaSeparatedSampleIdsOfMolecularProfile(
            molecularProfileId
        );
        List<Integer> internalSampleIds = Arrays
            .stream(commaSeparatedSampleIdsOfMolecularProfile.split(","))
        ...
        MolecularProfile molecularProfile = molecularProfileService.getMolecularProfile(
            molecularProfileId
        );
        List<String> studyIds = new ArrayList<>();
        sampleIds.forEach(
            s -> studyIds.add(molecularProfile.getCancerStudyIdentifier())
        );
        ...
        for (Integer internalSampleId : internalSampleIds) {
            if (selectedSampleIdsMap.containsKey(internalSampleId)) {
                includedIndexes.add(
                    internalSampleIdToIndexMap.get(internalSampleId)
                );
            }
        }

        Boolean isMolecularProfileBOfGenesetType = molecularProfile
            .getMolecularAlterationType()
            .equals(MolecularProfile.MolecularAlterationType.GENESET_SCORE);
        ...
        List<String> queryValues = Arrays.asList(
            queryMolecularDataList.getSplitValues()
        );
        List<String> includedQueryValues = includedIndexes
            .stream()
            .map(index -> queryValues.get(index))
            .collect(Collectors.toList());
        ...
        for (MolecularAlteration ma : maItr) {
            ...
            List<String> internalValues = new ArrayList<>(
                Arrays.asList(ma.getSplitValues())
            );
            List<String> values = includedIndexes
                .stream()
                .map(index -> internalValues.get(index))
                .collect(Collectors.toList());
            CoExpression ce = computeCoExpressions(
                entityId,
                values,
                includedQueryValues,
                isMolecularProfileBOfGenesetType,
    ...
    public List<CoExpression> fetchCoExpressions(
        ...
        if (isMolecularProfileBOfGenesetType) {
            molecularDataListB =
                genesetDataService
                    .fetchGenesetData(molecularProfileIdB, sampleIds, null)
                    .stream()
                    .collect(Collectors.toList());
        } else {
            molecularDataListB =
    ...
    private List<CoExpression> computeCoExpressionsFromMolecularData(
        ...
        Map<String, List<MolecularData>> molecularDataMapA = molecularDataListA
            .stream()
            .collect(Collectors.groupingBy(MolecularData::getStableId));
        Map<String, List<MolecularData>> molecularDataMapB = molecularDataListB
            .stream()
            .collect(Collectors.groupingBy(MolecularData::getStableId));
        ...
        List<? extends MolecularData> finalMolecularDataListA = (List<? extends MolecularData>) molecularDataMapA.remove(
            queryGeneticEntityId
        );
        if (molecularDataMapB.get(queryGeneticEntityId) != null) {
            List<? extends MolecularData> finalMolecularDataListB = (List<? extends MolecularData>) molecularDataMapB.remove(
                queryGeneticEntityId
            );
        ...
        List<String> valuesB = finalMolecularDataListA
            .stream()
            .map(g -> g.getValue())
            .collect(Collectors.toList());
        for (String entityId : molecularDataMapB.keySet()) {
            List<String> internalValues = molecularDataMapB
                .get(entityId)
                .stream()
                .map(g -> g.getValue())
                .collect(Collectors.toList());
            CoExpression co = computeCoExpressions(
                entityId,
                internalValues,
                valuesB,
                isMolecularProfileBOfGenesetType,
    ...
    private CoExpression computeCoExpressions(
        String entityId,
        List<String> valuesA,
        List<String> valuesB,
        Boolean isMolecularProfileBOfGenesetType,
        ...
        for (int i = 0; i < valuesBCopy.size(); i++) {
            if (
                !NumberUtils.isNumber(valuesBCopy.get(i)) ||
        ...
        double[] valuesBNumber = valuesBCopy
            .stream()
            .mapToDouble(Double::parseDouble)
            .toArray();
        double[] valuesANumber = valuesACopy
            .stream()
            .mapToDouble(Double::parseDouble)
            .toArray();
        ...
        SpearmansCorrelation spearmansCorrelation = new SpearmansCorrelation(
            (new Array2DRowRealMatrix(arrays, false)).transpose()
        );

        double spearmansValue = spearmansCorrelation.correlation(
            valuesBNumber,
            valuesANumber
        );
        if (
            Double.isNaN(spearmansValue) || Math.abs(spearmansValue) < threshold
        ) {
            return null;
        }
        coExpression.setSpearmansCorrelation(
            BigDecimal.valueOf(spearmansValue)
        );

        RealMatrix resultMatrix = spearmansCorrelation
            .getRankCorrelation()

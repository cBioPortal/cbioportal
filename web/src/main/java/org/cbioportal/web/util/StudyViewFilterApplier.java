...
public class StudyViewFilterApplier {
    ...
    public StudyViewFilterApplier(
        ...
        this.clinicalDataEqualityFilterApplier =
            clinicalDataEqualityFilterApplier;
        this.clinicalDataIntervalFilterApplier =
    ...
    public List<SampleIdentifier> apply(
        ...
        if (!CollectionUtils.isEmpty(clinicalDataFilters)) {
            List<String> attributeIds = clinicalDataFilters
                .stream()
            ...
            Map<String, ClinicalAttribute> clinicalAttributeMap = clinicalAttributes
                .stream()
                .collect(
                    Collectors.toMap(
        ...
        if (!CollectionUtils.isEmpty(studyViewFilter.getGeneFilters())) {
            Set<String> molecularProfileIds = studyViewFilter
                .getGeneFilters()
                .stream()
                .flatMap(
                    geneFilter -> geneFilter.getMolecularProfileIds().stream()
                )
                .collect(Collectors.toSet());

            List<MolecularProfile> molecularProfiles = molecularProfileService.getMolecularProfiles(
                new ArrayList<String>(molecularProfileIds),
                "SUMMARY"
            );
    ...
    private List<SampleIdentifier> filterByProfiled(
        List<SampleIdentifier> sampleIdentifiers,
        Boolean criteria,
        ...
        List<String> firstMutationProfileIds = molecularProfileGetter.apply(
            studyIds,
            sampleIds
        );
        List<GenePanelData> genePanelDataList = genePanelService
            ...
            .stream()
            .filter(g -> g.getProfiled() == criteria)
            .collect(Collectors.toList());
    ...
    private List<SampleIdentifier> filterMutatedOrFusionGenes(
        List<GeneFilter> genefilters,
        Map<String, MolecularProfile> molecularProfileMap,
        MolecularAlterationType MolecularAlterationFilterType,
        List<SampleIdentifier> sampleIdentifiers
    ) {
        for (GeneFilter genefilter : genefilters) {
            List<MolecularProfile> filteredMolecularProfiles = genefilter
                ...
                .map(
                    molecularProfileId ->
                        molecularProfileMap.get(molecularProfileId)
                )
                .collect(Collectors.toList());

            Set<MolecularAlterationType> alterationTypes = filteredMolecularProfiles
                .stream()
                .map(MolecularProfile::getMolecularAlterationType)
                .collect(Collectors.toSet());

            if (
                alterationTypes.size() == 1 &&
                alterationTypes.iterator().next() ==
                MolecularAlterationFilterType
            ) {
                    ...
                    for (int i = 0; i < studyIds.size(); i++) {
                        ...
                        if (mapByStudyId.containsKey(studyId)) {
                            molecularProfileIds.add(
                                mapByStudyId.get(studyId).get(0).getStableId()
                            );
                        } else {
    ...
    private List<SampleIdentifier> filterCNAGenes(
        List<GeneFilter> cnaGenes,
        Map<String, MolecularProfile> molecularProfileMap,
        List<SampleIdentifier> sampleIdentifiers
    ) {
        for (GeneFilter copyNumberGeneFilter : cnaGenes) {
            List<MolecularProfile> filteredMolecularProfiles = copyNumberGeneFilter
                .getMolecularProfileIds()
                .stream()
                .map(
                    molecularProfileId ->
            ...
            if (
                ...
                for (List<SingleGeneQuery> geneQueries : copyNumberGeneFilter.getSingleGeneQueries()) {
                    ...
                    List<DiscreteCopyNumberData> resultList = DiscreteCopyNumberEventType
                        .HOMDEL_AND_AMP.getAlterationTypes()
                        .stream()
                        .flatMap(
                            alterationType -> {
                                List<SingleGeneQuery> filteredGeneQueries = geneQueries
                                    .stream()
                                    .filter(
                                        geneQuery ->
                                            geneQuery
                                ...
                                List<String> hugoGeneSymbols = filteredGeneQueries
                                    .stream()
                                    .map(SingleGeneQuery::getHugoGeneSymbol)
                                    .collect(Collectors.toList());

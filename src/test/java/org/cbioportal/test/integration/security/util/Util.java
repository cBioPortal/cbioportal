...
public class Util {
    ...
    new WebDriverWait(chromedriver, Duration.ofSeconds(20))
        .until(
            ExpectedConditions.presenceOfElementLocated(
                By.xpath("//span[.='Breast Invasive Carcinoma (TCGA,Nature 2012)']")));
  ...
  public static void testLoginAndVerifyStudyNotPresent(String cbioUrl, ChromeDriver chromeDriver) {
    ...
    Assertions.assertThrows(
        ...
        () ->
            chromeDriver.findElement(
                By.xpath("//span[.='Adrenocortical Carcinoma (TCGA, Provisional)']")),
        "Study could not be found on the landing page. Permissions are not correctly passed from IDP to client.");
  }

  public static void testDownloadOfflineToken(String cbioUrl, ChromeDriver chromeDriver)
      throws Exception {
    performLogin(cbioUrl, chromeDriver);
    Assertions.assertDoesNotThrow(
    ...
    var file =
        new File(
            String.format("%s/cbioportal_data_access_token.txt", ContainerConfig.DOWNLOAD_FOLDER));
  ...
  private static void performLogin(String url, RemoteWebDriver driver) {
    ...
    new WebDriverWait(driver, Duration.ofSeconds(20))
        .until(
            ExpectedConditions.presenceOfElementLocated(
                By.xpath("//*[@data-test='cancerTypeListContainer']")));
  }
}

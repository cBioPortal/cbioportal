...
import static org.cbioportal.legacy.web.parameter.DataBinFilter.*;
...
public class DataBinnerTest {
        ...
        .map(
            datum ->
                studyViewFilterUtil.getCaseUniqueKey(
                    datum.getStudyId(), getPatientIds ? datum.getPatientId() : datum.getSampleId()))
  ...
  public void testLinearDataBinnerWithRangeAndCustomBins() {
    ...
    clinicalDataBinFilter.setCustomBins(
        Arrays.asList(50.0, 60.0, 70.0).stream()
  ...
  public void testStaticDataBinnerFilter() {
    ...
    List<String> unfilteredPatientIds = getCaseIds(unfilteredClinicalData, true);
    List<DataBin> unfilteredDataBins =
        dataBinner.calculateDataBins(
    ...
    List<Binnable> filteredClinicalData =
        unfilteredClinicalData.subList(0, 108); // (0, 60] interval
    List<String> filteredPatientIds = getCaseIds(filteredClinicalData, true);
    List<DataBin> filteredDataBins =
  ...
  public void testLinearDataBinnerWithNumberOfBoneScans() {
    ...
    List<DataBin> dataBinsWithDefaultThreshold =
        dataBinner.calculateDataBins(
            clinicalDataBinFilter, ClinicalDataType.PATIENT, clinicalData, patientIds);
    ...
    List<DataBin> dataBinsWithThreshold5 =
        dataBinner.calculateDataBins(
  ...
  public void testLinearDataBinnerWithAgeAtWhichSequencingWasReported() {
    ...
    // even if we have 90 and 100 they will not be used because we have special values like ">89" in
    // our data
  ...
  public void testLinearDataBinnerWithPediatricAge() {
    ...
    clinicalDataBinFilter.setBinMethod(BinMethod.CUSTOM);
    clinicalDataBinFilter.setCustomBins(
        Arrays.asList(18.0, 20.0, 30.0, 40.0, 50.0, 60.0, 70.0, 80.0, 90.0).stream()
            .map(item -> BigDecimal.valueOf(item))
            .collect(Collectors.toList()));
  ...
  public void testLinearDataBinnerWithPediatricAgeCustomBinsTest2() {
    ...
    clinicalDataBinFilter.setBinMethod(BinMethod.CUSTOM);
    clinicalDataBinFilter.setCustomBins(
        Arrays.asList(18.0, 25.0, 30.0, 35.0).stream()
            .map(item -> BigDecimal.valueOf(item))
            .collect(Collectors.toList()));
  ...
  public void testLinearDataBinnerWithPediatricAgeCustomBinsTwoBoundaries() {
    ...
    clinicalDataBinFilter.setBinMethod(BinMethod.CUSTOM);
    clinicalDataBinFilter.setCustomBins(
        Arrays.asList(20.0, 50.0).stream()
            .map(item -> BigDecimal.valueOf(item))
            .collect(Collectors.toList()));
  ...
  public void testLinearDataBinnerQuartileBinsLowComplexitySeriesQ1Q2Q3Identical() {
    ...
    List<DataBin> expected = Arrays.asList(createDataBin("<=", null, "250", 50));
  ...
  public void testLinearDataBinnerQuartileBinsLowComplexitySeriesQ2Q3Identical() {
    ...
    List<DataBin> expected =
        Arrays.asList(createDataBin("<=", null, "15", 6), createDataBin(null, "15", "250", 17));
  ...
  public void testLinearDataBinnerWithPredefinedAttribute() {
    ...
    clinicalDataBinFilter.setBinMethod(BinMethod.CUSTOM);
    clinicalDataBinFilter.setCustomBins(
        Arrays.asList(1.0, 2.0, 5.0, 10.0, 30.0).stream()
            .map(item -> BigDecimal.valueOf(item))
            .collect(Collectors.toList()));
    ...
    List<DataBin> dataBins =
        dataBinner.calculateDataBins(
            clinicalDataBinFilter, ClinicalDataType.SAMPLE, clinicalData, sampleIds);
  ...
  public void testLinearDataBinnerWithAlwaysZeroIQR() {
    ...
    List<DataBin> dataBins =
        dataBinner.calculateDataBins(
            clinicalDataBinFilter, ClinicalDataType.SAMPLE, clinicalData, sampleIds, 5);
    ...
    Assert.assertEquals(
        "all values should be included in a single bin",
        values.length,
        dataBins.get(0).getCount().intValue());
  }
  ...
  public void testScientificDataBinner() {
    ...
    Assert.assertEquals(
        new BigDecimal("1.0e-5")
            .compareTo(dataBins.get(2).getEnd().round(new MathContext(5, RoundingMode.CEILING))),
        0);
    ...
    Assert.assertEquals(
        new BigDecimal("1.0e-5")
            .compareTo(dataBins.get(3).getStart().round(new MathContext(5, RoundingMode.CEILING))),
        0);
    Assert.assertEquals(1, dataBins.get(3).getCount().intValue());
    ...
    IntStream.range(0, expected.size())
        .forEach(
            i -> {
              ...
              Assert.assertTrue(
                  "Element " + i + " is not correct.", new ReflectionEquals(e).matches(o));
            });
  }
}

...
public class DaoSample {
    ...
    private static void cacheSample(Sample sample, int cancerStudyId) {
        ...
        Map<String, Sample> samples = byInternalPatientAndStableSampleId.get(
            sample.getInternalPatientId()
        );
        if (samples == null) {
            samples = new HashMap<String, Sample>();
            byInternalPatientAndStableSampleId.put(
                sample.getInternalPatientId(),
                samples
            );
        }
        if (samples.containsKey(sample.getStableId())) {
            System.err.println(
                "Something is wrong: there are two samples of " +
                sample.getStableId() +
                " in the same patient."
            );
        }
    ...
    public static List<Sample> getSamplesByPatientId(int internalPatientId) {
        return (
                byInternalPatientAndStableSampleId.isEmpty() ||
                !byInternalPatientAndStableSampleId.containsKey(
                    internalPatientId
                )
            )
            ? Collections.<Sample>emptyList()
            : new ArrayList<Sample>(
                byInternalPatientAndStableSampleId
                    .get(internalPatientId)
                    .values()
            );
    }

    public static Sample getSampleByPatientAndSampleId(
        int internalPatientId,
        String stableSampleId
    ) {
        Map<String, Sample> samples = byInternalPatientAndStableSampleId.get(
            internalPatientId
        );
    ...
    public static List<Sample> getSamplesByCancerStudy(int cancerStudyId) {
        Map<String, Sample> samples = byCancerStudyIdAndStableSampleId.get(
            cancerStudyId
        );
        if (samples == null) {
     ...
     * @return
     */
    public static List<String> getSampleStableIdsByCancerStudy(
        int cancerStudyId
    ) {
    ...
    public static Sample getSampleByCancerStudyAndSampleId(
        int cancerStudyId,
        String stableSampleId
    ) {
        return getSampleByCancerStudyAndSampleId(
            cancerStudyId,
            stableSampleId,
            true
        );

...
public class ContainerConfig {
  ...
  public static final int CBIO_PORT = 8080;
  public static final int SESSION_SERVICE_PORT = 5000;
  public static final int MONGO_PORT = 27017;

  public static final String MONGO_NETWORK = "mongo-network";
  public static final int MOCKSERVER_PORT = 8085;
  public static final String DOWNLOAD_FOLDER = "/tmp/browser_downloads";
  ...
  static {
    ...
    mongoContainer =
        new GenericContainer(DockerImageName.parse(MONGO_IMAGE_VERSION))
    ...
    String mongoConnectionString =
        String.format(
            "-Dspring.data.mongodb.uri=mongodb://%s:%s/session-service", MONGO_NETWORK, MONGO_PORT);
    sessionServiceContainer =
        new GenericContainer(DockerImageName.parse(SESSION_IMAGE_VERSION))
            .withNetwork(mongoNetwork)
            .withAccessToHost(true)
            .withEnv("SERVER_PORT", "5000")
            .withEnv("JAVA_OPTS", mongoConnectionString);
    sessionServiceContainer.setPortBindings(
        ImmutableList.of(String.format("%s:5000", SESSION_SERVICE_PORT)));

    keycloakContainer =
        new KeycloakContainer(KEYCLOAK_IMAGE_VERSION)
    ...
    mockServerContainer = new GenericContainer(MOCKSERVER_IMAGE_VERSION).withExposedPorts(1080);
    mockServerContainer.setPortBindings(
        ImmutableList.of(String.format("%s:1080", MOCKSERVER_PORT)));

    mysqlContainer =
        (MySQLContainer)
            new MySQLContainer(MYSQL_IMAGE_VERSION)
                .withClasspathResourceMapping(
                    "cgds.sql", "/docker-entrypoint-initdb.d/a_schema.sql", BindMode.READ_ONLY)
                .withClasspathResourceMapping(
                    "seed_mini.sql", "/docker-entrypoint-initdb.d/b_seed.sql", BindMode.READ_ONLY)
  ...
  public static class MyOAuth2ResourceServerKeycloakInitializer
      extends OAuth2ResourceServerKeycloakInitializer {
    @Override
    public void initialize(ConfigurableApplicationContext configurableApplicationContext) {
      super.initializeImpl(configurableApplicationContext, keycloakContainer);
    }
  ...
  public static class PortInitializer
      implements ApplicationContextInitializer<ConfigurableApplicationContext> {
    @Override
    public void initialize(ConfigurableApplicationContext applicationContext) {
      TestPropertyValues values = TestPropertyValues.of("server.port=" + CBIO_PORT);

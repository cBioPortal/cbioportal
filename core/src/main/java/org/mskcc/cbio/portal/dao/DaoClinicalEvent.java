...
public final class DaoClinicalEvent {
    ...
    public static int addClinicalEvent(ClinicalEvent clinicalEvent) {
        if (!MySQLbulkLoader.isBulkLoad()) {
            throw new IllegalStateException(
                "Only buld load mode is allowed for importing clinical events"
            );
        }

        MySQLbulkLoader
            .getMySQLbulkLoader("clinical_event")
            .insertRecord(
                ...
                clinicalEvent.getStopDate() == null
                    ? null
                    : clinicalEvent.getStopDate().toString(),
    ...
    private static int addClinicalEventData(ClinicalEvent clinicalEvent) {
        ...
        for (Map.Entry<String, String> entry : clinicalEvent
            .getEventData()
            .entrySet()) {
            MySQLbulkLoader
                .getMySQLbulkLoader("clinical_event_data")
                .insertRecord(
    ...
    public static List<ClinicalEvent> getClinicalEvent(int patientId)
        throws DaoException {
        return getClinicalEvent(patientId, null);
    }

    public static List<ClinicalEvent> getClinicalEvent(
        int patientId,
        String eventType
    )
        throws DaoException {
        ...
        try {
            ...
            if (eventType == null) {
                pstmt =
                    con.prepareStatement(
                        "SELECT * FROM clinical_event WHERE PATIENT_ID=?"
                    );
            } else {
                pstmt =
                    con.prepareStatement(
                        "SELECT * FROM clinical_event WHERE PATIENT_ID=? AND EVENT_TYPE=?"
                    );
            }
            ...
            if (!clinicalEvents.isEmpty()) {
                pstmt =
                    con.prepareStatement(
                        "SELECT * FROM clinical_event_data WHERE CLINICAL_EVENT_ID IN (" +
    ...
    private static ClinicalEvent extractClinicalEvent(ResultSet rs)
        throws SQLException {
        ...
        clinicalEvent.setStartDate(
            JdbcUtil.readLongFromResultSet(rs, "START_DATE")
        );
        clinicalEvent.setStopDate(
            JdbcUtil.readLongFromResultSet(rs, "STOP_DATE")
        );
     ...
     * @throws DaoException
     */
    public static boolean timeEventsExistForPatient(int patientId)
        throws DaoException {
        Connection con = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        try {
            ...
            pstmt =
                con.prepareStatement(
                    "SELECT EXISTS(SELECT 1 FROM `clinical_event` WHERE `PATIENT_ID`=?)"
                );
            pstmt.setInt(1, patientId);
            rs = pstmt.executeQuery();
    ...
    public static void deleteByCancerStudyId(int cancerStudyId)
        throws DaoException {
        Connection con = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        try {
            ...
            pstmt =
                con.prepareStatement(
                    "DELETE FROM clinical_event_data WHERE CLINICAL_EVENT_ID IN " +

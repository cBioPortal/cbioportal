...
public class OncotateTool {
    ...
    public OncotateTool(File inputMafFile, File outputMafFile)
        ...
        System.out.println(
            "Total Number of Records Processed:  " + numRecordsProcessed
        );
    ...
    private void outputFileNames(File inputMafFile, File outputMafFile) {
        System.out.println(
            "Reading MAF From:  " + inputMafFile.getAbsolutePath()
        );
        System.out.println(
            "Writing new MAF To:  " + outputMafFile.getAbsolutePath()
        );
    }

    private void conditionallyAbort(int numRecordsProcessed) {
        if (
            MAX_NUM_RECORDS_TO_PROCESS > 0 &&
            numRecordsProcessed > MAX_NUM_RECORDS_TO_PROCESS
        ) {
            throw new IllegalStateException(
                "Aborting at " + MAX_NUM_RECORDS_TO_PROCESS + " records"
            );
        }
    }
    ...
    private void writeHeaders(String headerLine, FileWriter writer)
        throws IOException {
    ...
    private void conditionallyOncotateRecord(
        MafRecord mafRecord,
        FileWriter writer
    )
        throws IOException, DaoException {
    ...
    private void oncotateRecord(MafRecord mafRecord, FileWriter writer)
        throws IOException, DaoException {
        ...
        if (tumorAllele != null) {
            String coords = createCoordinates(
                chr,
                start,
                end,
                refAllele,
            ...
            OncotatorRecord oncotatorRecord = oncotatorService.getOncotatorRecord(
                chr,
                start,
                end,
                refAllele,
    ...
    private void writeOncotatorResults(
        Writer writer,
    ...
    private String createCoordinates(
        ...
        return (
            chr + "_" + start + "_" + end + "_" + refAllele + "_" + tumorAllele
        );
    }
    ...
    public static void main(String[] args) {
        if (args.length < 2) {
            System.out.println(
                "command line usage:  oncotateMaf.pl <file.maf> <file_output.maf>"
            );
            System.exit(1);
        }
        Date start = new Date();

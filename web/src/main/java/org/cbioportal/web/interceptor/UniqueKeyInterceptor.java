...
public class UniqueKeyInterceptor
    extends AbstractMappingJacksonResponseBodyAdvice {
    private static final Base64.Encoder BASE64_ENCODER = Base64
        .getEncoder()
        .withoutPadding();
            ...
            for (Object object : list) {
                if (object instanceof Alteration) {
                    ...
                    alteration.setUniqueSampleKey(
                        calculateBase64(
                            alteration.getSampleId(),
                            alteration.getStudyId()
                        )
                    );
                    alteration.setUniquePatientKey(
                        calculateBase64(
                            alteration.getPatientId(),
                            alteration.getStudyId()
                        )
                    );
                    ...
                    structuralVariant.setUniqueSampleKey(
                        calculateBase64(
                            ...
                            structuralVariant.getStudyId()
                        )
                    );
                    structuralVariant.setUniquePatientKey(
                        calculateBase64(
                            structuralVariant.getPatientId(),
                            structuralVariant.getStudyId()
                        )
                    );
                }
    ...
    private String calculateBase64(String firstInput, String secondInput) {
        return BASE64_ENCODER.encodeToString(
            (firstInput + DELIMITER + secondInput).getBytes()
        );
    }
}

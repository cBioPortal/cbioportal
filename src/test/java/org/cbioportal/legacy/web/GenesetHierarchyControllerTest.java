...
public class GenesetHierarchyControllerTest {
  ...
  @MockitoBean private GenesetHierarchyService genesetHierarchyService;
  ...
  public void fetchGenesetHierarchyInfo() throws Exception {
    ...
    Mockito.when(
            genesetHierarchyService.fetchGenesetHierarchyInfo(
                Mockito.anyString(), Mockito.anyInt(), Mockito.anyDouble(), Mockito.anyDouble()))
        .thenReturn(genesetHierarchyInfoList);

    mockMvc
        .perform(
            MockMvcRequestBuilders.post("/api/geneset-hierarchy/fetch")
                .with(csrf())
        ...
        .andExpect(
            MockMvcResultMatchers.jsonPath("$[2].genesets[0].representativeScore").value(0.1))
        .andExpect(
            MockMvcResultMatchers.jsonPath("$[2].genesets[0].representativePvalue").value(0.054))
        ...
        .andExpect(
            MockMvcResultMatchers.jsonPath("$[2].genesets[1].representativeScore").value(0.8))
        .andExpect(
            MockMvcResultMatchers.jsonPath("$[2].genesets[1].representativePvalue").value(0.04))
        ...
        .andExpect(
            MockMvcResultMatchers.jsonPath("$[3].genesets[0].representativeScore").value(0.8))
        .andExpect(
            MockMvcResultMatchers.jsonPath("$[3].genesets[0].representativePvalue").value(0.04));
  }

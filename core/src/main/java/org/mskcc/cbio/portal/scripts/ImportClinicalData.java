...
public class ImportClinicalData extends ConsoleRunnable {
    ...
    public static enum MissingAttributeValues {
        ...
        MissingAttributeValues(String propertyName) {
    ...
    public void importData() throws Exception {
        ...
        if (patientIdIndex < 0) {
            ...
            throw new RuntimeException(
                "Aborting owing to failure to find " +
        ...
        if (
            getAttributesType() ==
            ImportClinicalData.AttributeTypes.SAMPLE_ATTRIBUTES ||
            getAttributesType() ==
            ImportClinicalData.AttributeTypes.MIXED_ATTRIBUTES
        ) {
            DaoPatient.createSampleCountClinicalData(
    ...
    private List<ClinicalAttribute> grabAttrs(BufferedReader buff)
        ...
        if (line.startsWith(METADATA_PREFIX)) {
            ...
            switch (this.attributesType) {
                ...
                case MIXED_ATTRIBUTES:
                    ...
                    for (String attributeTypeVal : attributeTypes) {
                        if (
                            !attributeTypeVal.equalsIgnoreCase(
                                AttributeTypes.PATIENT_ATTRIBUTES.toString()
                            ) &&
                            !attributeTypeVal.equalsIgnoreCase(
                                AttributeTypes.SAMPLE_ATTRIBUTES.toString()
                            )
                        ) {
                            throw new RuntimeException(
                                "Invalid value for attributeType: " +
                                attributeTypeVal +
                                ". Check the header rows of your data file."
                            );
                        }
                    }
                    break;
            ...
            if (
                displayNames.length != colnames.length ||
                ...
                throw new DaoException(
                    "attribute and metadata mismatch in clinical staging file. All lines in header and data rows should have the same number of columns."
                );
            }
        } else {
        ...
        for (int i = 0; i < colnames.length; i += 1) {
            ...
            if (
                attr.getAttrId().equals(PATIENT_ID_COLUMN_NAME) ||
                attr.getAttrId().equals(SAMPLE_ID_COLUMN_NAME)
            ) {
                continue;
            }
            ClinicalAttribute attrInDb = DaoClinicalAttributeMeta.getDatum(
                attr.getAttrId(),
                cancerStudy.getInternalId()
            );
            if (attrInDb != null) {
    ...
    private String[] getFieldValues(
        ...
        if (fieldValues.length != columnAttrs.size()) {
            throw new IllegalArgumentException(
                "Number of columns in line is not as expected. Expected: " +
                columnAttrs.size() +
                " columns, found: " +
                fieldValues.length +
                ", for line: " +
    ...
    private boolean addDatum(
        String[] fields,
        List<ClinicalAttribute> columnAttrs,
        MultiKeyMap attributeMap
    )
        throws Exception {
        int sampleIdIndex = findSampleIdColumn(columnAttrs);
        String stableSampleId = (sampleIdIndex >= 0)
            ? fields[sampleIdIndex]
            : "";
        stableSampleId = StableIdUtil.getSampleId(stableSampleId);
        int patientIdIndex = findPatientIdColumn(columnAttrs);
        String stablePatientId = (patientIdIndex >= 0)
            ? fields[patientIdIndex]
            : "";
        ...
        if (sample != null) {
            ...
            if (!this.isSupplementalData()) {
                throw new RuntimeException(
                    "Error: Sample " +
                    stableSampleId +
                    " found to be duplicated in your file."
                );
            } else {
            ...
            if (patient != null) {
                ...
                internalPatientId =
                    (patientIdIndex >= 0)
                        ? addPatientToDatabase(fields[patientIdIndex])
                        : -1;
            }
            // sample is new, so attempt to add to DB
            internalSampleId =
                (stableSampleId.length() > 0)
        ...
        if (internalSampleId != -1) {
            ...
            internalPatientId =
                DaoPatient
                    .getPatientByCancerStudyAndPatientId(
                        cancerStudy.getInternalId(),
                        stablePatientId
                    )
                    .getInternalId();
            if (
                internalPatientId !=
                DaoSample.getSampleById(internalSampleId).getInternalPatientId()
            ) {
                throw new RuntimeException(
                    "Error: Sample " +
                    stableSampleId +
                    " was previously linked to another patient, and not to " +
                    stablePatientId
                );
            }
            numSamplesProcessed++;
        ...
        for (int lc = 0; lc < fields.length; lc++) {
            ...
            boolean isPatientAttribute = columnAttrs
                .get(lc)
                .isPatientAttribute();
            if (isPatientAttribute && internalPatientId != -1) {
                ...
                if (
                    !attributeMap.containsKey(
                        internalPatientId,
                        columnAttrs.get(lc).getAttrId()
                    )
                ) {
                    addDatum(
                        internalPatientId,
                    ...
                    throw new RuntimeException(
                        "Error: Duplicated patient in file"
                    );
                }
                ...
                // NOT recommended! TODO - change this to an Exception instead of just a warning.
                    ...
                    !attributeMap
                        .get(internalPatientId, columnAttrs.get(lc).getAttrId())
                        .equals(fields[lc])
                ) {
                    ProgressMonitor.logWarning(
                        "Error: Duplicated patient " +
                        stablePatientId +
                        " with different values for patient attribute " +
                        columnAttrs.get(lc).getAttrId() +
                        "\n\tValues: " +
                        attributeMap.get(
                            internalPatientId,
                ...
                if (
                    ...
                    throw new RuntimeException(
                        "Error: Duplicated sample in file"
                    );
                } else if (
                    !attributeMap
                        .get(internalSampleId, columnAttrs.get(lc).getAttrId())
                        .equals(fields[lc])
                ) {
                    ProgressMonitor.logWarning(
                        "Error: Duplicated sample " +
                        stableSampleId +
                        " with different values for sample attribute " +
                        columnAttrs.get(lc).getAttrId() +
                        "\n\tValues: " +
                        attributeMap.get(
                            internalSampleId,
    ...
    private int addPatientToDatabase(String patientId) throws Exception {
        ...
        if (validPatientId(patientId)) {
            ...
            if (
                getAttributesType() ==
                ImportClinicalData.AttributeTypes.PATIENT_ATTRIBUTES
            ) {
                ...
                if (
                    ...
                    throw new RuntimeException(
                        "Something has gone wrong. Patient " +
                        patientId +
                        " already has clinical data loaded."
                    );
                }
    ...
    private int addSampleToDatabase(
        String sampleId,
        String[] fields,
        List<ClinicalAttribute> columnAttrs
    )
        ...
        String sampleTypeStr = (sampleTypeIndex != -1)
            ? fields[sampleTypeIndex]
            : null;
        if (sampleTypeStr != null) {
            // want to match Sample.Type enum names
            sampleTypeStr =
                sampleTypeStr.trim().toUpperCase().replaceAll(" ", "_");
        }
        Sample.Type sampleType = Sample.Type.has(sampleTypeStr)
            ? Sample.Type.valueOf(sampleTypeStr)
            : null;
        ...
        if (validSampleId(sampleId) && !StableIdUtil.isNormal(sampleId)) {
            ...
            String stablePatientId = getStablePatientId(
                sampleId,
                fields,
                columnAttrs
            );
            if (validPatientId(stablePatientId)) {
                ...
                if (patient == null) {
                    ...
                    patient =
                        DaoPatient.getPatientByCancerStudyAndPatientId(
                            cancerStudy.getInternalId(),
                            stablePatientId
                        );
                }
    ...
    private String getStablePatientId(
        String sampleId,
        String[] fields,
        List<ClinicalAttribute> columnAttrs
    ) {
        Matcher tcgaSampleBarcodeMatcher = StableIdUtil.TCGA_PATIENT_BARCODE_FROM_SAMPLE_REGEX.matcher(
            sampleId
        );
        if (tcgaSampleBarcodeMatcher.find()) {
            ...
            int patientIdIndex = findAttributeColumnIndex(
                PATIENT_ID_COLUMN_NAME,
                columnAttrs
            );
            if (patientIdIndex >= 0) {
    ...
    public void run() {
        try {
            ...
            OptionSpec<String> study = parser
                .accepts("study", "cancer study id")
                .withOptionalArg()
                .describedAs("study")
                .ofType(String.class);
            OptionSpec<String> attributeFlag = parser
                .accepts("a", "(deprecated) Flag for using MIXED_ATTRIBUTES")
                .withOptionalArg()
                .describedAs("a")
                .ofType(String.class);
            OptionSpec<String> relaxedFlag = parser
                .accepts(
                    "r",
            ...
            if (cancerStudy == null) {
                throw new IllegalArgumentException(
                    "Unknown cancer study: " + cancerStudyStableId
                );
            }
            ProgressMonitor.setCurrentMessage(
                "Reading data from:  " + clinical_f.getAbsolutePath()
            );
            ...
            if (
                ...
                ProgressMonitor.setCurrentMessage(
                    "Total number of patient specific clinical attributes added:  " +
            ...
            if (
                ...
                ProgressMonitor.setCurrentMessage(
                    "Total number of sample specific clinical attributes added:  " +
            ...
            ProgressMonitor.setCurrentMessage(
                "Total number of attribute values skipped because of empty value:  " +

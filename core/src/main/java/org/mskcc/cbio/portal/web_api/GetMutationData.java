...
public class GetMutationData {
     ...
     */
    public List<ExtendedMutation> getMutationData(
        GeneticProfile profile,
        List<String> geneList,
        Set<String> sampleIdSet,
        XDebug xdebug
    )
        throws DaoException {
        ...
        if (geneList.size() > 0) {
            ...
            List<Integer> internalSampleIds = InternalIdUtil.getInternalSampleIds(
                profile.getCancerStudyId(),
                new ArrayList<String>(sampleIdSet)
            );
            ...
            for (Long entrezID : entrezIDList) {
                ...
                for (ExtendedMutation mutation : tempmutationList) {
                    ...
                    if (
                        internalSampleIds.contains(mutation.getSampleId())
    ...
    public static String getProfileData(
        ...
        if (geneticProfile == null) {
            buf
                .append("No genetic profile available for ")
                .append(WebService.GENETIC_PROFILE_ID)
                .append(":  ")
                .append(geneticProfileId)
                .append(".\n");
            return buf.toString();
        }

        List<Integer> internalSampleIds = targetSampleList == null
            ? null
        ...
        //  Output Actual Data
        List<Gene> geneList = WebApiUtil.getGeneList(
            targetGeneList,
        ...
        buf
            .append("# DATA_TYPE\t ")
            .append(geneticProfile.getProfileName())
            .append("\n");
        ...
        for (Gene gene : geneList) {
            ...
            List<ExtendedMutation> mutationList = mutationModelConverter.convert(
                mutationRepositoryLegacy.getMutations(
                    (int) canonicalGene.getEntrezGeneId(),
            ...
            for (ExtendedMutation mutation : mutationList) {
                ...
                if (
                    ...
                    buf
                        .append(getAlleleCount(mutation.getTumorRefCount()))
                        .append(TAB);
                    buf
                        .append(getAlleleCount(mutation.getTumorAltCount()))
                        .append(TAB);
                    buf
                        .append(getAlleleCount(mutation.getNormalRefCount()))
                        .append(TAB);
                    buf
                        .append(getAlleleCount(mutation.getNormalAltCount()))
                        .append(TAB);
